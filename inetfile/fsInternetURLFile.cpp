...
fsInternetURLFile::~fsInternetURLFile()
{
	...
	if (m_bAutoDelSession && m_pSession) delete m_pSession;
...
{
#ifndef NOCURL
	if (m_bUseFile2) return m_ifile2.Read(pBuffer, dwToRead, pdwRead);
#else
	if (m_bUseFile2) return IR_E_NOTIMPL;
#endif

	if (m_pFile == NULL) return IR_FILENOTOPENED;
...
{
#ifndef NOCURL
	if (m_bUseFile2) return m_ifile2.GetFileSize();
#endif

	if (m_pFile == NULL) return _UI64_MAX;
...
void fsInternetURLFile::Close()
{
	...
	if (m_pFile) m_pFile->CloseHandle();
}
...
fsInternetResult fsInternetURLFile::Open(INTERNET_SCHEME scheme, LPCSTR pszHostName, LPCSTR pszUser, LPCSTR pszPassword,
                                         INTERNET_PORT port, LPCSTR pszPath, UINT64 uStartPosition,
                                         BOOL bSendHTTPBasicAuthImmediately)
{
	return OpenEx(scheme, pszHostName, pszUser, pszPassword, port, pszPath, uStartPosition,
	              bSendHTTPBasicAuthImmediately, _UI64_MAX);
}
...
#ifndef NOCURL
	if (m_bUseFile2) m_ifile2.StopDownloading();
#endif

	if (m_pFile) m_pFile->CloseHandle();

	if (m_pServer) m_pServer->CloseHandle();

	if (m_bAutoDelSession) m_pSession->CloseHandle();
...
{
#ifndef NOCURL
	return m_bUseFile2 ? m_ifile2.GetLastModifiedDate(pDate) : m_pFile->GetLastModifiedDate(pDate);
#else
	if (m_bUseFile2 || !m_pFile) return FALSE;
	return m_pFile->GetLastModifiedDate(pDate);
#endif
}
...
{
#ifndef NOCURL
	if (m_bUseFile2) return m_ifile2.IsResumeSupported();
#endif

	if (m_pFile == NULL) return RST_UNKNOWN;
...
LPCSTR fsInternetURLFile::GetSuggestedFileName()
{
	...
	{
#ifndef NOCURL
		return m_bUseFile2 ? m_ifile2.get_SuggestedFileName() : m_pFile->GetSuggestedFileName();
#else
...
fsInternetResult fsInternetURLFile::QuerySize(INTERNET_SCHEME scheme, LPCSTR pszHostName, LPCSTR pszUser,
                                              LPCSTR pszPassword, INTERNET_PORT port, LPCSTR pszPath,
                                              BOOL bSendHTTPBasicAuthImmediately)
{
...
void fsInternetURLFile::SetupProxyForFile2()
{
	...
	if (lstrcmpi(psz1, "Internet Explorer") == 0)
	{
		CRegKey key;
		if (ERROR_SUCCESS !=
		    key.Open(HKEY_CURRENT_USER, "Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings", KEY_READ))
			return;
	...
	if (psz1 && *psz1) m_ifile2.set_Proxy(psz1, psz2, psz3);
}
...
fsInternetResult fsInternetURLFile::OpenEx(INTERNET_SCHEME scheme, LPCSTR pszHostName, LPCSTR pszUser,
                                           LPCSTR pszPassword, INTERNET_PORT port, LPCSTR pszPath,
                                           UINT64 uStartPosition, BOOL bSendHTTPBasicAuthImmediately,
                                           UINT64 uUploadPartSize, UINT64 uUploadTotalSize)
{
	fsInternetResult ir;
	...
	case INTERNET_SCHEME_HTTPS:
		...
		m_pFile = &m_httpFile;
		if (bSendHTTPBasicAuthImmediately) FormHttpBasicAuthHdr(pszUser, pszPassword);
		break;
	...
	if (bUseFile2 == false)
	{
		if (uUploadTotalSize == _UI64_MAX)
		{
			...
			if (scheme == INTERNET_SCHEME_HTTP || scheme == INTERNET_SCHEME_HTTPS)
			{
				if (m_pFile->GetFileSize() != _UI64_MAX && m_pFile->GetFileSize() >= 2 * 1024 * 1024 * 1024ui64)
					bUseFile2 = true;
			}
		}
	}
	...
	_lUseFile2:
		...
		m_ifile2.Initialize();
		if (m_strInterface.IsEmpty() == false) m_ifile2.setInterface(m_strInterface);
		...
		if (pszUser && *pszUser) m_ifile2.set_Auth(pszUser, pszPassword);
		...
		if (m_httpFile.get_UseCookie())
		{
			...
			else
			{
				...
				*szCookie = 0;
				...
				if (*szCookie) m_ifile2.set_Cookie(szCookie);
			}
		}

		if (m_httpFile.GetPostData()) m_ifile2.set_PostData(m_httpFile.GetPostData());
...
{
	if (m_bUseFile2) return IR_ERROR;
...
{
	if (pszUser == NULL || *pszUser == 0) return;
	...
	lstrcat(szLogin, ":");
	if (pszPassword) lstrcat(szLogin, pszPassword);
...
fsInternetResult fsInternetURLFile::OpenAnotherRequestOnServer(LPCSTR pszPath, UINT64 uStartPosition,
                                                               UINT64 uUploadPartSize, UINT64 uUploadTotalSize)
{
	fsInternetResult ir;

	if (m_pFile == NULL) return IR_NOTINITIALIZED;
