...
fsInternetResult fsInternetURLFileDownloader::Download(LPCSTR pszUrl)
{
	...
	ir = Get_File()->Open(url.GetInternetScheme(), url.GetHostName(), url.GetUserName(), url.GetPassword(),
	                      url.GetPort(), url.GetPath(), 0);
	if (ir != IR_SUCCESS) return ir;
	...
	if (ir == IR_SUCCESS) Event(UFDE_DONE);
...
{
	if (m_pfnEvents) m_pfnEvents(ev, m_lpEvParam);
}
