...
{
	StopDownloading();
	for (int i = 0; m_bDownloading && i < 33; i++) Sleep(33 * 3);

	if (m_hevReadDataReq) CloseHandle(m_hevReadDataReq);
	if (m_hevReadDataDone) CloseHandle(m_hevReadDataDone);
}
...
void fsInternetFile2::set_URL(LPCSTR pszURL)
{
	...
	if (strstr(pszURL, ".megashares.com/") != NULL) m_bDoPause = TRUE;
}
...
fsInternetResult fsInternetFile2::StartDownloading()
{
	if (m_bDownloading == false)
	{
		...
		DWORD dw;
		CloseHandle(CreateThread(NULL, 0, _threadDownload, this, 0, &dw));

		while (m_bAnswrFromServRcvd == false) Sleep(10);

		if (m_irLastError == IR_SUCCESS) ExtractFileInfoFromResponse();

		if (m_strHttpHeader.IsEmpty() == FALSE) Dialog(IFDD_FROMSERVER, m_strHttpHeader);
...
DWORD WINAPI fsInternetFile2::_threadDownload(LPVOID lp)
{
	...
	try
	{
		...
		if (pthis->m_bDoPause) Sleep(1500);
...
fsInternetResult fsInternetFile2::Read(LPBYTE pBuffer, DWORD dwToRead, DWORD* pdwRead)
{
	...
	if (dwToRead == 0) return IR_SUCCESS;

	if (m_bDownloading == false) return m_irLastError;
	...
	if (m_ReadReq.nDataLeft == dwToRead) return m_irLastError != IR_SUCCESS ? m_irLastError : IR_ERROR;
...
{
	curl_easy_setopt(m_curl, CURLOPT_HTTP_VERSION, bUse ? CURL_HTTP_VERSION_1_1 : CURL_HTTP_VERSION_1_0);
}
...
{
	char sz[1000];
	sprintf_s(sz, sizeof(sz), "%s:%s", pszUser, pszPwd);
...
void fsInternetFile2::ExtractFileInfoFromResponse()
{
	if (m_curl != NULL)
	{
		...
		if (m_uStartPos != 0) m_enRST = RST_PRESENT;
	}
	else
	{
		m_uFileSize = 0;
...
size_t fsInternetFile2::_WriteHeader(void* ptr, size_t size, size_t nmemb, void* stream)
{
	...
	// Clamp to buffer capacity to prevent buffer overrun
	size_t copyLen = totalSize < sizeof(sz) - 1 ? totalSize : sizeof(sz) - 1;
	strncpy_s(sz, sizeof(sz), (LPCSTR)ptr, copyLen);
	sz[copyLen] = 0;
