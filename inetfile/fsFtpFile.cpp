...
{
	if (m_sFile == UINT(-1)) return IR_NOTINITIALIZED;

	try
	{
		*pdwRead = recv(m_sFile, (char*)pBuffer, dwToRead, 0);
	}
	catch (...)
	{
		return IR_ERROR;
	...
	if (*pdwRead == 0 || *pdwRead == DWORD(SOCKET_ERROR)) return fsWSAErrorToIR();
...
{
	if (m_sFile != UINT(-1)) closesocket(m_sFile);

	if (m_sRcv != UINT(-1)) closesocket(m_sRcv);
...
SHORT fsFtpFile::OpenSocket()
{
	...
	if (_portStart >= IPPORT_RESERVED + 10000) _portStart = IPPORT_RESERVED + 10;
	...
	CHAR ipAddr[100];
	sprintf_s(ipAddr, sizeof(ipAddr), "%d,%d,%d,%d,%d,%d", (int)(BYTE)he->h_addr_list[0][0],
	          (int)(BYTE)he->h_addr_list[0][1], (int)(BYTE)he->h_addr_list[0][2], (int)(BYTE)he->h_addr_list[0][3],
...
fsInternetResult fsFtpFile::FtpError()
{
	...
	if (ir == IR_EXTERROR) ReceiveExtError();
	return ir;
}
...
BOOL fsFtpFile::ParseSIZE()
{
	...
	if (uSize > 1000ui64 * 1024 * 1024 * 1024) return FALSE;
...
fsInternetResult fsFtpFile::PASV_ConnectSocket()
{
	...
	do
	{
		...
		if (pszPORT == NULL) return IR_ERROR;
	...
	for (int i = 0; i < 6; i++)
	{
		...
		while (*pszPORT >= '0' && *pszPORT <= '9') pszPORT++;
		while (*pszPORT && (*pszPORT < '0' || *pszPORT > '9')) pszPORT++;

		if (*pszPORT == 0 && i != 5) return IR_ERROR;
	}
	...
	if (m_sFile == -1) return fsWSAErrorToIR();

	if (connect(m_sFile, (sockaddr*)&addr, sizeof(addr)) == -1) return fsWSAErrorToIR();
...
fsInternetResult fsFtpFile::QuerySize(LPCSTR pszFilePath)
{
	...
	char szFile[10000];
	strcpy_s(szFile, sizeof(szFile), pszFilePath);
	...
	if (pszFile)
	{
		if (pszFile - pszFilePath)
		{
			...
			sprintf_s(szCmd, sizeof(szCmd), "CWD %s", szFile);
			...
			if (!FtpCommand(hServer, FALSE, m_dwTransferType, szCmd, NULL, NULL))
				...
				strcpy_s(szFile, sizeof(szFile), pszFile + 1);
	...
	if (m_bDontUseLIST == FALSE) bListOK = Send_LIST(szCmd, szFile);
	...
	if (bListOK == FALSE)
	{
		...
		sprintf_s(szCmd, sizeof(szCmd), "SIZE %s", szFile);
		Dialog(IFDD_TOSERVER, szCmd);
		if (FtpCommand(hServer, FALSE, FTP_TRANSFER_TYPE_BINARY, szCmd, NULL, NULL)) ParseSIZE();
		DialogFtpResponse();
...
{
	sprintf_s(pszCmd, 10000, "LIST %s", pszFile);
	Dialog(IFDD_TOSERVER, pszCmd);

	WIN32_FIND_DATA wfd;

	HINTERNET hFind =
	    FtpFindFirstFile(m_pServer->GetHandle(), pszFile, &wfd, INTERNET_FLAG_NO_CACHE_WRITE | INTERNET_FLAG_RELOAD, 0);
...
fsInternetResult fsFtpFile::OpenEx(LPCSTR pszFilePath, UINT64 uStartPos, UINT64, UINT64 uUploadTotalSize)
{
	...
	if (m_bDontUseLIST == FALSE && uStartPos == 0) bListOK = Send_LIST(szCmd, szFile);
	...
	sprintf_s(szCmd, sizeof(szCmd), "TYPE %c", m_dwTransferType & FTP_TRANSFER_TYPE_ASCII ? 'A' : 'I');
	Dialog(IFDD_TOSERVER, szCmd);
	...
	if (bPassive)
	{
		...
		// fsInternetResult
		ir = PASV_ConnectSocket();

		if (ir != IR_SUCCESS) return ir;
	}
	else
	{
		if (0 == OpenSocket()) return IR_ERROR;

		sprintf_s(szCmd, sizeof(szCmd), "PORT %s", m_strPORT.pszString);
		Dialog(IFDD_TOSERVER, szCmd);
		if (!FtpCommand(hServer, FALSE, m_dwTransferType, szCmd, NULL, NULL))
		{
			DialogFtpResponse();
	...
	if (uUploadTotalSize == _UI64_MAX)
	{
		if (uStartPos)
		{
			sprintf_s(szCmd, sizeof(szCmd), "REST %I64u", uStartPos);
			Dialog(IFDD_TOSERVER, szCmd);

			if (!FtpCommand(hServer, FALSE, m_dwTransferType, szCmd, NULL, NULL)) m_enRST = RST_NONE;
			DialogFtpResponse();

			if (m_enRST == RST_NONE) return IR_RANGESNOTAVAIL;
		...
		sprintf_s(szCmd, sizeof(szCmd), "RETR %s", szFile);
	}
	else
	{

		if (uStartPos)
			sprintf_s(szCmd, sizeof(szCmd), "APPE %s", szFile);
		else
			sprintf_s(szCmd, sizeof(szCmd), "STOR %s", szFile);
	}
	...
	if (bPassive == FALSE)
	{
		...
		if (m_sFile == UINT(-1)) return fsWSAErrorToIR();
	}
...
fsInternetResult fsFtpFile::Write(LPBYTE pBuffer, DWORD dwToWrite, DWORD* pdwWritten)
{
	...
	try
	{
		*pdwWritten = send(m_sFile, (char*)pBuffer, dwToWrite, 0);
	}
	catch (...)
	{
		return IR_ERROR;
	...
	if (*pdwWritten == 0 || *pdwWritten == DWORD(SOCKET_ERROR)) return fsWSAErrorToIR();

	return IR_SUCCESS;
}

BOOL fsFtpFile::FtpCommand(HINTERNET hConnect, BOOL fExpectResponse, DWORD dwFlags, LPCSTR pszCommand,
