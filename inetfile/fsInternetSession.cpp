...
fsInternetResult fsInternetSession::Create(LPCSTR pszAgent, fsInternetAccessType accType, LPCSTR pszProxy)
{
	...
	case IAT_MANUALPROXY:
		...
		if (pszProxy == NULL || *pszProxy == 0) return IR_INVALIDPARAM;
		m_strProxy = pszProxy;
		break;
	...
	m_hSession =
	    InternetOpen(pszAgent, dwAccessType, pszProxy, dwAccessType == INTERNET_OPEN_TYPE_PROXY ? "<local>" : NULL, 0);

	if (m_hSession == NULL) return fsWinInetErrorToIR();
...
{
	if (m_strProxyUser.Length() == 0) return IR_SUCCESS;

	if (!InternetSetOption(hInetHandle, INTERNET_OPTION_PROXY_USERNAME, (LPVOID)m_strProxyUser,
	                       m_strProxyUser.Length() + 1))
		return fsWinInetErrorToIR();

	if (!InternetSetOption(hInetHandle, INTERNET_OPTION_PROXY_PASSWORD, (LPVOID)m_strProxyPassword,
