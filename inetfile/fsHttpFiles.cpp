...
fsInternetResult fsHttpFiles::BuildFileList()
{
	...
	CHAR szUrl[10000];
	DWORD dwLen = 10000;
	url.Create(INTERNET_SCHEME_HTTP, m_pServer->GetServerName(), m_pServer->GetServerPort(), NULL, NULL, m_strPath,
	           szUrl, &dwLen);
	m_strFullPath = szUrl;
	...
	for (UINT i = 0; i < cUrls && m_bAbort == FALSE; i++)
	{
		...
		fsURL _url;
		if (_url.Crack(pszUrl, FALSE) == IR_SUCCESS)
		{
			INTERNET_SCHEME scheme = _url.GetInternetScheme();
		...
		if (file.strName.Length() == 0) continue;
...
fsInternetResult fsHttpFiles::GetUrlInfo(LPCSTR pszUrl, fsFileInfo* pInfo)
{
	...
	if (pInfo->bFolder) return IR_SUCCESS;
	...
	{
		ir = fsHttpOpenUrl(pszUrl, url.GetUserName(), url.GetPassword(), &m_hUI_server, &m_hUI_file, &pszNewUrl,
		                   &bRedirInner);
...
void fsHttpFiles::CalcUrl(fsFileInfo* pInfo, LPCSTR pszSomeUrl)
{
	...
	if (ir != IR_SUCCESS && m_pszBaseURL && *m_pszBaseURL)
	{
		...
		if (strResUrl[strResUrl.Length() - 1] != '/' && strResUrl[strResUrl.Length() - 1] != '\\') strResUrl += "/";
	...
	if (ir == IR_SUCCESS)
	{
		if (fsIsServersEqual(url1.GetHostName(), url2.GetHostName(), FALSE) &&
		    url1.GetInternetScheme() == url2.GetInternetScheme() && url1.GetPort() == url2.GetPort())
		{
...
{
	if (m_pfnEvents) m_pfnEvents(this, enEvent, m_lpEventsParam);
}
