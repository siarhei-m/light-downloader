...
fsInternetResult fsLocalFile::Open(LPCSTR pszFilePath, UINT64 uStartPos)
{
	...
	m_hFile = CreateFile(strPath, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL);

	if (m_hFile == INVALID_HANDLE_VALUE) return fsWinErrorToIR();
	...
	if (uStartPos)
	{
		if (!inetfile::fsSetFilePointer(m_hFile, (__int64)uStartPos, FILE_BEGIN)) return IR_RANGESNOTAVAIL;
	}
...
{
	if (FALSE == ReadFile(m_hFile, pBuffer, dwSize, pdwRead, NULL)) return fsWinErrorToIR();
...
fsInternetResult fsLocalFile::QuerySize(LPCSTR pszFilePath)
{
	...
	if (hFile == INVALID_HANDLE_VALUE) return fsWinErrorToIR();
...
{
	if (uUploadTotalSize != _UI64_MAX) return IR_ERROR;
