...
fsInternetResult fsURL::Crack(LPCSTR pszUrl, BOOL bCheckScheme)
{
	...
	{
		strUrl = pszUrl + 1;
		if (strUrl[0] == 0) return IR_BADURL;
	...
	if ((pszUrl[0] == '\\' && pszUrl[1] == '\\'))
	{
		...
		strcpy_s(m_szPath, sizeof(m_szPath), pszUrl);
		strcpy_s(m_szScheme, sizeof(m_szScheme), "file");
		*m_szUser = 0;
		*m_szPassword = 0;
		m_url.nPort = 0;
	}
	else
	{
		...
		else
		{
			...
			strcpy_s(pszCanUrl, urlLen, pszUrl);
		}
		...
		if (!InternetCrackUrl(pszCanUrl, urlLen, _strnicmp(pszCanUrl, "ftp://", 6) == 0 ? ICU_DECODE : 0, &m_url))
		{
			delete pszCanUrl;
			return fsWinInetErrorToIR();
		}
	...
	if (m_url.nScheme == INTERNET_SCHEME_FILE)
	{
...

		{
			...
			if (iPathStart == strlen(m_szPath)) return IR_BADURL;
			strncpy_s(m_szHost, sizeof(m_szHost), m_szPath + 2, iPathStart - 2);
			m_szHost[iPathStart - 2] = 0;
			memmove(m_szPath, m_szPath + iPathStart, strlen(m_szPath + iPathStart) + 1);
		}
	}
...
fsInternetResult fsURL::Create(INTERNET_SCHEME nScheme, LPCTSTR lpszHostName, INTERNET_PORT nPort, LPCTSTR lpszUserName,
                               LPCTSTR lpszPassword, LPCTSTR lpszUrlPath, LPSTR lpszUrl, DWORD* pdwUrlLength)
{
	if (!lpszUrl) return IR_ERROR;
	...
	*lpszUrl = 0;
	...
	if (lpszUserName && *lpszUserName == 0) lpszUserName = NULL;

	if (lpszPassword && *lpszPassword == 0) lpszPassword = NULL;
	...
	if (!InternetCreateUrl(&m_url, 0, lpszUrl, pdwUrlLength)) return fsWinInetErrorToIR();
...
void fsURL::FixWinInetBug()
{
	...
	strcat_s(m_szPassword, sizeof(m_szPassword), "@");
	strncat_s(m_szPassword, sizeof(m_szPassword), m_szHost, psz - m_szHost);
	memmove(m_szHost, psz + 1, strlen(psz + 1) + 1);
}
...
void fsURL::Encode(LPCSTR psz, LPSTR sz)
{
	while (*psz)
	{
		...
		if (c == ':' || c == '@' || c == '%')
		{
			sprintf_s(sz2, sizeof(sz2), "%%%x", (int)(BYTE)c);
		}
		else
		{
			sz2[0] = c;
