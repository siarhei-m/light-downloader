...
LPCSTR fsHTMLParser::ParseTag(LPCSTR pszTag)
{
	...
	if (i == sizeof(apszTags) / sizeof(LPCSTR)) pszTag = ParseTag_Unknown(pszTag);
...
LPCSTR fsHTMLParser::Parse_HTMLGiving(LPCSTR pszGiving, LPSTR* ppszValue, LPCSTR pszAddEnds)
{
	...
	*ppszValue = NULL;
	...
	char szEnd[100];
	strcpy_s(szEnd, sizeof(szEnd), pszEnd);
	if (pszAddEnds) strcat_s(szEnd, sizeof(szEnd), pszAddEnds);

	pszGiving = fsStrGetStrUpToChar(pszGiving, szEnd, ppszValue);

	if (pszGiving == NULL) return NULL;

	if (*(pszGiving - 1) == '>' || *(pszGiving - 1) == ' ') pszGiving--;
...
{
	if (strcmp(m_vUrls[iIndex], pszNewUrl) == 0) return;
...
{
	if (strcmp(m_vImgs[iIndex], pszNewImg) == 0) return;
...
LPCSTR fsHTMLParser::ParseTag_Link(LPCSTR pszTag, fsHTMLParser* pThis)
{
	...
	if (bLinkAdded) pThis->m_vLRTs.add(lrt);
...
{
	if (strcmp(m_vLinkUrls[iIndex], pszNewUrl) == 0) return;
...
LPCSTR fsHTMLParser::ParseTag_Meta_Content(LPCSTR pszTag, fsMetaHttpEquivType mhet)
{
	...
	default:
		...
		if (pszContent) delete[] pszContent;
	}
...
{
	if (strcmp(m_vFrameUrls[iIndex], pszNewUrl) == 0) return;
