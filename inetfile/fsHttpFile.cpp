...
fsHttpFile::~fsHttpFile()
{
	if (m_pszReferer) delete[] m_pszReferer;

	if (m_pszCookies) delete[] m_pszCookies;

	if (m_pszPostData) delete[] m_pszPostData;
...
fsInternetResult fsHttpFile::Read(LPBYTE pBuffer, DWORD dwToRead, DWORD* pdwRead)
{
	...
	if (!InternetReadFile(m_hFile, pBuffer, dwToRead, pdwRead)) return fsWinInetErrorToIR();
...
{
	if (bUse)
		strcpy_s(m_pszHttpVersion, 100, "HTTP/1.1");
	else
		strcpy_s(m_pszHttpVersion, 100, "HTTP/1.0");
}
...
void fsHttpFile::SetReferer(LPCSTR pszReferer)
{
	...
	if (pszReferer)
	{
		...
		strcpy_s(m_pszReferer, strlen(pszReferer) + 1, pszReferer);
	}
}
...
{
	if (m_hFile == NULL) return IR_NOTINITIALIZED;
...
fsInternetResult fsHttpFile::ProcessRangesResponse()
{
	...
	if (strncmp(sz, "bytes", 5)) return bAcceptRanges ? IR_DOUBTFUL_RANGESRESPONSE : IR_RANGESNOTAVAIL;

	int pos = 0;
	while (sz[pos++] != ' ');

	if (isdigit(sz[pos]) == false) return IR_RANGESNOTAVAIL;

	UINT64 first = (UINT64)_atoi64(sz + pos);

	while (sz[pos] >= '0' && sz[pos] <= '9') pos++;
	pos++;

	UINT64 last = (UINT64)_atoi64(sz + pos);

	if (last < first) return IR_RANGESNOTAVAIL;
	...
	if (m_uFileSize < last) return IR_RANGESNOTAVAIL;
...
void fsHttpFile::RetreiveSuggFileName()
{
	...
	if (FALSE == HttpQueryInfo(m_hFile, HTTP_QUERY_CONTENT_DISPOSITION, sz, &dwFL, NULL)) return;
...
{
	if (!m_pServer) return IR_NOTINITIALIZED;

	HINTERNET hServer = m_pServer->GetHandle();

	if (!hServer) return IR_NOTINITIALIZED;

	CloseHandle();

	if (lstrlen(pszFilePath) > 9000) return IR_BADURL;

	if (cTryings > 1) return IR_WININETUNKERROR;

	DWORD dwFlags = m_dwFlags;
	if (m_pszCookies) dwFlags |= INTERNET_FLAG_NO_COOKIES;
	if (m_bEnableAutoRedirect == FALSE) dwFlags |= INTERNET_FLAG_NO_AUTO_REDIRECT;

	LPCSTR ppszAcceptedTypes[2] = {"*/*", NULL};
	...
	m_hFile = HttpOpenRequest(
	    hServer, pszVerb, pszFilePath, m_pszHttpVersion, m_pszReferer, (LPCSTR*)ppszAcceptedTypes,
	    dwFlags | INTERNET_FLAG_RELOAD | INTERNET_FLAG_PRAGMA_NOCACHE | INTERNET_FLAG_NO_CACHE_WRITE, NULL);

	if (m_hFile == NULL) return fsWinInetErrorToIR();
	...
	if (uStartPos) sprintf_s(szHdr, sizeof(szHdr), "Range: bytes=%I64u-\r\n", uStartPos);

	if (m_pszCookies) sprintf_s(szHdr + lstrlen(szHdr), sizeof(szHdr) - lstrlen(szHdr), "Cookie: %s\r\n", m_pszCookies);

	if (m_pszPostData) strcat_s(szHdr, sizeof(szHdr), "Content-Type: application/x-www-form-urlencoded\r\n");

	if (m_pszAdditionalHeaders) strcat_s(szHdr, sizeof(szHdr), m_pszAdditionalHeaders);

	if (cTryings == 0)
	{
		char szReq[90000];
		sprintf_s(szReq, sizeof(szReq), "%s %s %s\r\nReferer: %s", pszVerb, pszFilePath, m_pszHttpVersion,
		          m_pszReferer ? m_pszReferer : "-");

		if (*szHdr)
		{
			strcat_s(szReq, sizeof(szReq), "\r\n");
			strcat_s(szReq, sizeof(szReq), szHdr);
		...
		if ((dwFlags & INTERNET_FLAG_NO_COOKIES) == 0)
		{
			...
			url.Create(m_dwFlags & INTERNET_FLAG_SECURE ? INTERNET_SCHEME_HTTPS : INTERNET_SCHEME_HTTP,
			           m_pServer->GetServerName(), m_pServer->GetServerPort(), NULL, NULL, pszFilePath, szUrl, &dw);
			...
			*szCookie = 0;
			...
			{
				strcat_s(szReq, sizeof(szReq), "\r\n");
				strcat_s(szReq, sizeof(szReq), "Cookie: ");
				strcat_s(szReq, sizeof(szReq), szCookie);
			}
		}

		strcat_s(szReq, sizeof(szReq), "\r\nHost: ");
		strcat_s(szReq, sizeof(szReq), m_pServer->GetServerName());

		if (m_pszPostData)
		{
			strcat_s(szReq, sizeof(szReq), "\r\n");
			strcat_s(szReq, sizeof(szReq), m_pszPostData);
		}
	...
	if (!HttpSendRequest(m_hFile, *szHdr ? szHdr : NULL, (UINT)-1, m_pszPostData,
	                     m_pszPostData ? lstrlen(m_pszPostData) : 0))
	{
	...
	if (dwStatusCode < 200 || dwStatusCode >= 300)
	{
		...
		if (ir == IR_NEEDREDIRECT)
		{
			...
			HttpQueryInfo(m_hFile, HTTP_QUERY_LOCATION, NULL, &dwNeedLen, NULL);
			...
			if (::GetLastError() == ERROR_INSUFFICIENT_BUFFER)
			{
				...
				if (m_pszLastError == NULL) return IR_OUTOFMEMORY;
				if (!HttpQueryInfo(m_hFile, HTTP_QUERY_LOCATION, m_pszLastError, &dwNeedLen, NULL))
	...
	if (HttpQueryInfo(m_hFile, HTTP_QUERY_CONTENT_LENGTH, szContLen, &dwLen, NULL))
	{
		...
		if (iSize < 0) return IR_SERVERUNKERROR;
	...
	if (uStartPos)
	{
		...
		if (ir == IR_RANGESNOTAVAIL) return ir;
	}
	else
	{
		CHAR sz[10000];
	...
	if (HttpQueryInfo(m_hFile, HTTP_QUERY_LAST_MODIFIED | HTTP_QUERY_FLAG_SYSTEMTIME, &time, &dwTL, NULL))
	{
...
void fsHttpFile::IgnoreSecurityProblems()
{
	...
	InternetQueryOption(m_hFile, INTERNET_OPTION_SECURITY_FLAGS, (LPVOID)&dwFlags, &dwBuffLen);

	dwFlags |= SECURITY_FLAG_IGNORE_UNKNOWN_CA | SECURITY_FLAG_IGNORE_WRONG_USAGE | SECURITY_FLAG_IGNORE_REVOCATION |
	           SECURITY_FLAG_IGNORE_REDIRECT_TO_HTTPS | SECURITY_FLAG_IGNORE_REDIRECT_TO_HTTP |
	           SECURITY_FLAG_IGNORE_CERT_DATE_INVALID | SECURITY_FLAG_IGNORE_CERT_CN_INVALID;

	InternetSetOption(m_hFile, INTERNET_OPTION_SECURITY_FLAGS, &dwFlags, sizeof(dwFlags));
}
...
void fsHttpFile::SetAdditionalHeaders(LPCSTR pszAdditionalHeaders)
{
	...
	if (pszAdditionalHeaders && *pszAdditionalHeaders)
	{
		...
		if (l < 2) return;
		if (pszAdditionalHeaders[l - 2] != '\r') return;
...
fsInternetResult fsHttpFile::OpenEx(LPCSTR pszFilePath, UINT64 uStartPos, UINT64 uUploadPartSize,
                                    UINT64 uUploadTotalSize)
{
	if (uUploadTotalSize == _UI64_MAX) return Open_imp(pszFilePath, uStartPos, 0);

	if (uStartPos + uUploadPartSize > uUploadTotalSize) return IR_INVALIDPARAM;
	...
	m_hFile = HttpOpenRequest(hServer, "POST", strFilePath, m_pszHttpVersion, NULL, (LPCSTR*)ppszAcceptedTypes,
	...
	{
		if (*szHdr) lstrcat(szHdr, "\r\n");
		sprintf_s(szHdr + lstrlen(szHdr), sizeof(szHdr) - lstrlen(szHdr), "Range: bytes=%I64u-%I64u/%I64u", uStartPos,
	...
	{
		if (*szHdr) lstrcat(szHdr, "\r\n");
		sprintf_s(szHdr + lstrlen(szHdr), sizeof(szHdr) - lstrlen(szHdr), "Cookie: %s", m_pszCookies);
	}
...
fsInternetResult fsHttpFile::Write(LPBYTE pBuffer, DWORD dwSize, DWORD* pdwWritten)
{
	if (m_hFile == NULL)
	{
		if (pdwWritten) *pdwWritten = 0;
		return IR_NOTINITIALIZED;
	}

	if (m_uLeftToUpload == 0) return IR_S_FALSE;

	DWORD dwWritten;

	BOOL bRet = InternetWriteFile(m_hFile, pBuffer, dwSize, &dwWritten);

	if (pdwWritten) *pdwWritten = dwWritten;

	if (!bRet) return fsWinInetErrorToIR();
	...
	if (m_uLeftToUpload == 0)
	{
		if (m_bUseMultipart)
		{
			...
			str += "--\r\n";
			DWORD dw;
			if (FALSE == InternetWriteFile(m_hFile, str, str.GetLength(), &dw)) return fsWinInetErrorToIR();
		}
...
fsString fsHttpFile::GetCookiesFromResponse()
{
	...
	if (FALSE == HttpQueryInfo(m_hFile, HTTP_QUERY_SET_COOKIE, sz, &dw, NULL)) return "";
