...
fsInternetResult fsHttpOpenUrl(LPCSTR pszUrl, LPCSTR pszUser, LPCSTR pszPassword, fsHttpConnection* pServer,
                               fsHttpFile* pFile, LPSTR* ppRedirectedUrl, BOOL* pbRedirInner)
{
	fsURL url;
	fsInternetResult ir;
...
fsInternetResult fsHttpOpenPath(LPCSTR pszPath, fsHttpConnection* pServer, fsHttpFile* pFile, LPSTR* ppRedirectedUrl,
                                BOOL* pbRedirInner)
{
	fsInternetResult ir;
	...
	*ppRedirectedUrl = NULL;
	...
	if (ir != IR_SUCCESS)
	{
		if (ir == IR_NEEDREDIRECT)
		{
			...
			if (u.Crack(pFile->GetLastError()) != IR_SUCCESS) bRelUrl = TRUE;
			...
			*pbRedirInner = TRUE;
			...
			if (bRelUrl)
			{
				if (*pFile->GetLastError() != '/' && *pFile->GetLastError() != '\\')
				{
					...
					while (strUrl[len - 1] != '/' && strUrl[len - 1] != '\\') len--;
					strUrl[len] = 0;
			...
			if (*ppRedirectedUrl == NULL)
			{
				...
				strcpy_s(*ppRedirectedUrl, strUrl.Length() + 1, strUrl);
			}
	...
	*ppszFullUrl = NULL;
	...
	try
	{
		if (IR_SUCCESS != url.Crack(pszUrlCurrent, FALSE))
		{
			...
			{
				fsURL url2;
				if (IR_SUCCESS != url.Crack(pszUrlParent)) return FALSE;
				DWORD dwLen = nLenParent + nLenUrl + 1;
				url2.Create(url.GetInternetScheme(), url.GetHostName(), url.GetPort(), url.GetUserName(),
				            url.GetPassword(), pszUrlCurrent, *ppszFullUrl, &dwLen);
			}
			else
			{
				...
				if (pszPath == NULL) pszPath = strstr(*ppszFullUrl, ":\\\\");
				if (pszPath)
				{
					pszPath += 3;
				...
				if (pos < posmin)
					...
					while (pos > posmin && pszUrlParent[pos - 1] != '\\' && pszUrlParent[pos - 1] != '/') pos--;
					...
					if (pszUrlCurrent[i] == '.')
					{
						if (pszUrlCurrent[i + 1] == '.')
						{
							if (pszUrlCurrent[i + 2] == '\\' || pszUrlCurrent[i + 2] == '/')
							{
								if (pos > posmin) pos--;
								while (pos > posmin &&
								       ((*ppszFullUrl)[pos - 1] != '\\' && (*ppszFullUrl)[pos - 1] != '/'))
									pos--;
								i += 2;
							}
							else
		...
		strcpy_s(*ppszFullUrl, strlen(pszUrlCurrent) + 1, pszUrlCurrent);
		return FALSE;
	}
	catch (...)
	{
...
{
	if (pszServ1 == NULL || pszServ2 == NULL) return FALSE;
...
fsInternetResult fsDownloadFile(fsInternetURLFile* file, LPBYTE* ppBuf, UINT* puSize, BOOL* pbAbort)
{
	...
	if (uFileSize == _UI64_MAX) uMax = 100000;
	...
	*ppBuf = NULL;
	...
	do
	{
		if (uToRead > uFileSize - uPos) uToRead = UINT(uFileSize - uPos);
		if (uToRead == 0) break;
		...
		{
			cZeroReads++;
			if (cZeroReads < 3 && uFileSize != _UI64_MAX) dwRead = 1;
		}
		else
			cZeroReads = 0;
	} while (dwRead && *pbAbort == FALSE);

	if (*pbAbort) return IR_S_FALSE;

	if (uFileSize != _UI64_MAX && uFileSize != uPos) return IR_WININETUNKERROR;

	pBuf[uPos] = 0;

	*ppBuf = pBuf;

	if (puSize) *puSize = uPos;
