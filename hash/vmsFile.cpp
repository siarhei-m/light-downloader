...
void vmsFile::Open(LPCSTR pszFile, BOOL bRead)
{
	...
	m_hFile = CreateFile(pszFile, bRead ? GENERIC_READ : GENERIC_WRITE, bRead ? FILE_SHARE_READ : 0, NULL,
...
void vmsFile::Create(LPCSTR pszFile)
{
	...
	m_hFile = CreateFile(pszFile, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	if (m_hFile == INVALID_HANDLE_VALUE) throw "failed to create file";
}
...
{
	DWORD dw;
	if (FALSE == WriteFile(m_hFile, pb, dwSize, &dw, NULL)) throw "failed to write to file";
	return dw;
}
...
{
	DWORD dw;
	if (FALSE == ReadFile(m_hFile, lp, dwSize, &dw, NULL)) throw "failed to read from file";
	return dw;
}
