...
{
	TGroupLoadEventData() : pdgmGroupsMgr(0), pgtParent(0), bIsGroupLoaded(true) {}
...
class vmsDownloadsGroupsMgr : public vmsPersistObject
{
  public:
	...
	PDLDS_GROUPS_TREE Add(vmsDownloadsGroupSmartPtr pGroup, vmsDownloadsGroupSmartPtr pParentGroup,
	                      BOOL bKeepIdAsIs = FALSE, bool bDontQueryStoringGroupsInformation = false);
	PDLDS_GROUPS_TREE Add(vmsDownloadsGroupSmartPtr grp, PDLDS_GROUPS_TREE pParentGroup, BOOL bKeepIdAsIs = FALSE,
	                      bool bDontQueryStoringGroupsInformation = false);
	...
	void UpdateIdForNextGroup(UINT nId);

	vmsDownloadsGroupsMgr();
	virtual ~vmsDownloadsGroupsMgr();

  protected:
	void RebuildGroupsList(PDLDS_GROUPS_TREE pRoot,
	...
	void SaveGroupsTreeToBuffer(LPBYTE& pbtCurrentPos, LPBYTE pbtBuffer, DWORD dwBufferSizeSize, DWORD* pdwSizeReuiqred,
	                            PDLDS_GROUPS_TREE pRoot);
	void SaveGroupToBuffer(LPBYTE& pbtCurrentPos, LPBYTE pbtBuffer, DWORD dwBufferSizeSize, DWORD* pdwSizeReuiqred,
	...
	bool LoadGroupFromBuffer(LPBYTE& pbtCurrentPos, LPBYTE pbtBuffer, DWORD dwBufferSize,
	                         vmsDownloadsGroupSmartPtr pGroup);
