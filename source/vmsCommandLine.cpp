...
tstring getLongPathName(LPCTSTR ptsz)
{
	...
	{
		tstring tstr;
		if (SearchPath(ptsz, _T ("exe"), &tstr)) _tcscpy(tsz, tstr.c_str());
	}
	if (0 == GetLongPathName(ptsz, tsz, MAX_PATH)) return ptsz;
	return tsz;
}
...
void vmsCommandLine::fromString(LPCTSTR ptsz, bool bConvertToLongPath)
{
	...
	{
		ptsz++;
		while (*ptsz && *ptsz != '"') m_tstrExe += *ptsz++;
		if (*ptsz == 0)
		{
			m_tstrExe = _T ("");
			return;
	...
	else
	{
		...
		do
		{
			...
			if (*ptsz)
			{
				...
				if (dw != DWORD(-1) && (dw & FILE_ATTRIBUTE_DIRECTORY) == 0) break;
				...
				if (m_tstrExe.length() > 4 && _tcsicmp(m_tstrExe.c_str() + m_tstrExe.length() - 4, _T (".exe")) == 0)
					break;

				while (*ptsz == ' ') m_tstrExe += *ptsz++;
			}

		} while (*ptsz);
	}

	if (bConvertToLongPath) m_tstrExe = getLongPathName(m_tstrExe.c_str());
...
{
	if (m_tstrArgs.empty()) return m_tstrExe;

	if (m_tstrExe.empty()) return _T ("");
...
{
	if (m_tstrExe.empty()) return false;
	if (m_tstrExe.length() < 5) return false;
...
int vmsCommandLine::CompareEXEs(const vmsCommandLine& cl) const
{
	...
	if (ptsz1 && ptsz2) return _tcsicmp(ptsz1, ptsz2);

	if (ptsz1 == NULL && ptsz2 == NULL) return 0;
...
bool vmsCommandLine::Execute(DWORD dwFlags, LPDWORD pdwProcessExitCode) const
{
	...
	BOOL bOK = CreateProcess(NULL, (LPTSTR)toString().c_str(), NULL, NULL, FALSE, CREATE_DEFAULT_ERROR_MODE, NULL, NULL,
	                         &si, &pi);
	...
	if (!bOK)
	{
		...
		if (dwError == ERROR_NOT_FOUND || dwError == ERROR_PATH_NOT_FOUND)
		{
			if (dwFlags & SearchPathIfRequired)
			{
				...
				bOK = CreateProcess(NULL, (LPTSTR)tstrCmdLine.c_str(), NULL, NULL, FALSE, CREATE_DEFAULT_ERROR_MODE,
				                    NULL, NULL, &si, &pi);
			}
			else
			{
