...
CDlg_CheckFileIntegrity::CDlg_CheckFileIntegrity(CWnd* pParent) : CDialog(CDlg_CheckFileIntegrity::IDD, pParent)
{
...
BOOL CDlg_CheckFileIntegrity::OnInitDialog()
{
	...
	for (int i = 0; i < sizeof(ppszAlgs) / sizeof(LPCSTR); i++)
	{
		...
		if (strLastAlg.CompareNoCase(ppszAlgs[i]) == 0) m_wndAlgorithm.SetCurSel(i);
	}

	if (CB_ERR == m_wndAlgorithm.GetCurSel()) m_wndAlgorithm.SetCurSel(0);
	...
	if (m_strChecksum.IsEmpty() == FALSE)
	{
		...
		if (nAlg == HA_SHA2 && m_nSHA2Strength != 0) nAlg += 1000 + m_nSHA2Strength;
...
void CDlg_CheckFileIntegrity::OnOK()
{
	...
	if (m_nAlgorithm != HA_SHA2) m_nSHA2Strength = 0;

	GetDlgItemText(IDC_CHECKSUM, m_strChecksum);
	if (m_strChecksum.GetLength() == 0) return;

	CString str;
	m_wndAlgorithm.GetWindowText(str);
...
void CDlg_CheckFileIntegrity::PasteChecksum(bool bCheckValid)
{
	...
	if (psz)
	{
		if (bCheckValid)
		{
			...
			while (*psz2)
			{
				if (!((*psz2 >= '0' && *psz2 <= '9') || (*psz2 >= 'a' && *psz2 <= 'f') ||
				      (*psz2 >= 'A' && *psz2 <= 'F') || (*psz2 == ' ' || *psz2 == '-')))
					return;
				psz2++;
			}
		}
