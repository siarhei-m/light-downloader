...
{
	if ((m_dwFlags & SM_ALLOWBITMAPS) == 0) pszName = NULL;

	if (m_nCurrSkin == -1 || pszName == NULL)
		return (HBITMAP)LoadImage(m_hInst, MAKEINTRESOURCE(nID), IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION);

	HBITMAP hbm = (HBITMAP)LoadImage(NULL, m_vList[m_nCurrSkin].strSkinFolder + "\\" + pszName + ".bmp", IMAGE_BITMAP,
	                                 0, 0, LR_LOADFROMFILE);
...
{
	static UINT aIDs[] = {IDB_BACK,
	                      IDB_BACK_D,
	                      IDB_CHECKS,
	                      IDB_DOWNLOADSTASKS_SEL,
	                      IDB_DOWNLOADSTASKS,
	                      IDB_DOWNLOADSINFO,
	                      IDB_FILELIST,
	                      IDB_FILELIST_SEL,
	...
	{
		if (aIDs[i] == nID) return bmp_x(nID, ppszNames[i]);
	}
...
{
	static UINT aIDs[] = {
	    IDI_CHOOSEFOLDER, IDI_CREATEGROUP,      IDI_LOGO2,       IDI_LOGIN,        IDI_SETTIME, IDI_TOSEL, IDI_TOUNSEL,
	    IDI_TRAY,         IDI_TRAY_DOWNLOADING, IDI_TRAY_ERRORS, IDI_TRAY_UNKNOWN, IDI_TEST_OK,
	};

	static LPCSTR ppszNames[] = {
	    "choosefolder", "creategroup", "dropbox",   "login",    "settime",       "tosel",
	    "tounsel",      "tray",        "tray_down", "tray_err", "tray_starting", "check_ok",
	};
	...
	for (int i = 0; i < sizeof(aIDs) / sizeof(UINT); i++)
	{
		if (aIDs[i] == nID) return icon_x(nID, ppszNames[i], cx, cy);
	}
...
{
	if ((m_dwFlags & SM_ALLOWICONS) == 0) pszName = NULL;

	if (m_nCurrSkin == -1 || pszName == NULL) return (HICON)LoadIcon(m_hInst, MAKEINTRESOURCE(nID));

	HICON hico = (HICON)LoadImage(NULL, m_vList[m_nCurrSkin].strSkinFolder + "\\" + pszName + ".ico", IMAGE_ICON, cx,
	                              cy, LR_LOADFROMFILE);
