...
BOOL CDownloads_Info::Create(CWnd* pParent)
{
	...
	if (FALSE == CTabCtrl::Create(TCS_SINGLELINE | TCS_RIGHTJUSTIFY | TCS_FOCUSNEVER, rc, pParent, 0x5111))
		return FALSE;
...
int CDownloads_Info::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (!m_frame.Create(this)) return -1;

	if (!m_log.Create(&m_frame)) return -1;

	if (!m_progress.Create(&m_frame)) return -1;
...
{
	if (m_progress.IsWindowVisible()) m_progress.UpdateDownload();
}
