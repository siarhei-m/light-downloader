...
vmsTrafficUsageModeMgr::vmsTrafficUsageModeMgr(void) : m_dwState(0)
{
...
vmsTrafficUsageModeMgr::~vmsTrafficUsageModeMgr(void)
{
	...
	for (size_t i = 0; i < m_vpManageForSpeed.size(); i++) delete m_vpManageForSpeed[i];
}
...
void vmsTrafficUsageModeMgr::SetTumRawSettings(fsTUM enTUM, vmsTrafficUsageModeRawSettings& stgs)
{
	...
	{
		m_aTUM[enTUM] = stgs;
		if (enTUM == m_enCurrentTUM) BuildCurrentSettings();
	}
}

void vmsTrafficUsageModeMgr::SetTumRawSettingsPerDirection(fsTUM enTUM,
                                                           vmsTrafficUsageModeRawSettings::PerDirection& stgs,
                                                           bool bForDownload)
{
...
{
	if (FALSE == _App.Get_TUM(m_aTUM)) InitTUM();
...
void vmsTrafficUsageModeMgr::SetTUMManage(fsTUMManage enManage)
{
	...
	if (enManage == TM_OFF)
	{
		if (GetTUM() != m_enUserTUM) SetTUM(m_enUserTUM);
	}
}
...
void vmsTrafficUsageModeMgr::ApplySpeedLimit(bool bForDownload, UINT64 uLimit)
{
	...
	for (size_t i = 0; i < m_vpManageForSpeed.size(); i++)
	{
		for (size_t j = 0; j < m_vpManageForSpeed[i]->vItems.size(); j++)
		{
			if ((m_vpManageForSpeed[i]->vItems[j].state & ManageForSpeedItem::MSIS_PRIORITY_CHANGED) ||
			    (m_vpManageForSpeed[i]->vItems[j].getData(bForDownload).state &
			     ManageForSpeedItem::ManageForSpeedItemData::MSIDS_SPEED_CAN_BE_LIMITED_CHANGED))
	...
	for (int i = m_vpManageForSpeed.size() - 1; i >= 0; i--, bIsHighestPriorityItems = false)
	{
		ApplySpeedLimitForItems(bForDownload, m_vpManageForSpeed[i],
		                        ns.currentBandwidthExploreInLimitedMode.isExploring() ? UINT64_MAX : uLimit,
		                        bIsHighestPriorityItems, &uLimit);
	}
}

void vmsTrafficUsageModeMgr::ApplySpeedLimitForItems(bool bForDownload, ManageForSpeedItemsList* pItems, UINT64 uLimit,
                                                     bool bIsHighestPriorityItems, UINT64* puUnusedLimit)
{
	...
	{
		UINT64 uMaxLimitPerOne =
		    uLimit == UINT64_MAX ? UINT64_MAX : (uLimit - uEatenTotal) / (vProcessed.size() - cProcessed);
		bMaxLimitPerOneMustBeRecalculated = false;

		for (size_t i = 0; i < pItems->vItems.size(); i++)
		{
			if (vProcessed[i]) continue;
			...
			if (!uMaxLimitForThis) uMaxLimitForThis = 1;
			...
			if (pItem->pDldr->isIgnoreAllSpeedLimits(bForDownload)) uMaxLimitForThis = UINT64_MAX;
			if (uMaxLimitForThis < 100 && !pItem->pDldr->isResumeSupported(bForDownload)) uMaxLimitForThis = 100;
			...
			if (uCurrentLimit <= uMaxLimitForThis && uCurrentMaximumSpeed < uCurrentLimit / 1.1)
			{

				vLimits[i] = uMaxLimitForThis;
				uEatenTotal += uCurrentMaximumSpeed;
				if (uEatenTotal > uLimit) uEatenTotal = uLimit;
				vProcessed[i] = true;
				cProcessed++;
				bMaxLimitPerOneMustBeRecalculated = cProcessed != vProcessed.size();
				continue;
			...
			if (!bMaxLimitPerOneMustBeRecalculated) vLimits[i] = uMaxLimitForThis;
		}
	}
	...
	if (bIsHighestPriorityItems)
	{
		...
		if (ns.uCurrentBandwidth < uLimit2)
		{
			...
			if (uEatenTotal != UINT64_MAX && uEatenTotal > uLimit2) uEatenTotal = uLimit2;
		}
	}
	...
	if (uEatenTotal != UINT64_MAX)
	{
		...
		uEatenTotal *= 1.05;
		if (uEatenTotal > uLimit2) uEatenTotal = uLimit2;
		...
		if (uLimit2 != UINT64_MAX)
		{
			if (uEatenTotal != uLimit2)
			{
				...
				if (uLimit2 / delta(uLimit2, uEatenTotal) >= 10) uEatenTotal = uLimit2;
			}
		}
	}
	...
	*puUnusedLimit = uLimit2 > uEatenTotal ? uLimit2 - uEatenTotal : 0;
	...
	if (*puUnusedLimit == 0) pItems->state |= ManageForSpeedItemsList::MSILS_USES_ALL_BANDWIDTH_PROVIDED;
...
DWORD WINAPI vmsTrafficUsageModeMgr::_threadManage(LPVOID lp)
{
	...
	while (WaitForSingleObject(pthis->m_hevShuttingDown, 1000) == WAIT_TIMEOUT)
	{
		...
		if (pthis->m_bRestrainAll) bDontUpdateSpeeds = true;
...
void vmsTrafficUsageModeMgr::ProcessManageForSpeedItems_checkStateChanges(bool bForDownload)
{
	for (size_t i = 0; i < m_vpManageForSpeed.size(); i++)
	{
		...
		for (size_t j = 0; j < pList->vItems.size(); j++)
		{
			...
			if (!(data.state & data.MSIDS_SPEED_CAN_BE_LIMITED) !=
			    !item.pDldr->isSpeedCanBeLimitedBySomeInternalReasons(bForDownload))
			{
...
void vmsTrafficUsageModeMgr::ProcessManageForSpeedItems_manageReMeasures_CurrentBandwidthInLimitedMode(
    bool bForDownload)
{
	NetworkStat& ns = refgetNetworkStat(bForDownload);
	...
	{
		if (m_enCurrentTUM == TUM_HEAVY || m_bRestrainAll || ns.uConnectionBandwidthByUser != UINT64_MAX) return;
		if (ns.tcHeavyModeWorkedEnoughUntil &&
		    vmsTickCount::GetTickCount64() - ns.tcHeavyModeWorkedEnoughUntil < 5 * 60 * 1000)
			return;
...
void vmsTrafficUsageModeMgr::ProcessManageForSpeedItems_manageReMeasures_CurrentBandwidth(bool bForDownload)
{
	...
	if (nCurrentMaxTotalSpeed == UINT64_MAX) return;

	if (ns.uCurrentBandwidth == UINT64_MAX || ns.uCurrentBandwidth < nCurrentMaxTotalSpeed ||
	...
	else
	{
		...
		if (uCurrentLimit == UINT64_MAX || uCurrentLimit == UINT_MAX) uCurrentLimit = ns.uConnectionBandwidth;
		bool bNeedExplore = ns.uCurrentBandwidth < uCurrentLimit * 0.9;
		if (bNeedExplore) ns.currentBandwidthExplorer.CheckNeedStartExplore();
	}
}

void vmsTrafficUsageModeMgr::ProcessManageForSpeedItems_manageReMeasures_ExtraBandwidthForHigherPriorityItems(
    bool bForDownload)
{
	...
	else
	{
		for (int i = m_vpManageForSpeed.size() - 1; i >= 0; i--)
		{
			...
			if (pList->state & ManageForSpeedItemsList::MSILS_USES_ALL_BANDWIDTH_PROVIDED) continue;
			if (pList->vTotalSpeed.getCurrentMaximumValue() == UINT64_MAX) continue;
			pList->totalSpeedExplorer.CheckNeedStartExplore();
			if (pList->totalSpeedExplorer.isExploring()) return;
		}
	}
}
...
{
	size_t i = 0;
	while (i < m_vpManageForSpeed.size() && m_vpManageForSpeed[i]->i8Priority <= i8Priority) i++;
...
void vmsTrafficUsageModeMgr::addManageForSpeedItem(vmsDownloaderWithNetworkUsageAdjustment* pItem)
{
	...
	{
		if (iPriorityIndex == -1) iPriorityIndex = GetPriorityListIndex(pItem->getPriority());
...
int vmsTrafficUsageModeMgr::findPriorityIndex(BYTE i8Priority) const
{
	for (size_t i = 0; i < m_vpManageForSpeed.size(); i++)
	{
		if (m_vpManageForSpeed[i]->i8Priority == i8Priority) return i;
		if (m_vpManageForSpeed[i]->i8Priority > i8Priority) return -1;
	}
	return -1;
}

int vmsTrafficUsageModeMgr::findManageForSpeedItemIndex(vmsDownloaderWithNetworkUsageAdjustment* pItem,
                                                        int* piPriorityIndex) const
{
	assert(piPriorityIndex != NULL);
	if (!piPriorityIndex) return -1;
	*piPriorityIndex = findPriorityIndex(pItem->getPriority());
	...
	for (size_t i = 0; i < m_vpManageForSpeed.size(); i++)
	{
		if (int(i) == *piPriorityIndex) continue;
...
void vmsTrafficUsageModeMgr::ProcessManageForSpeedItems_updateSpeeds(bool bForDownload)
{
	...
	for (size_t i = 0; i < m_vpManageForSpeed.size(); i++)
	{
		...
		for (size_t j = 0; j < pList->vItems.size(); j++)
		{
			...
			{
				if (!pItem->getData(bForDownload).vSpeed.empty()) pItem->getData(bForDownload).vSpeed.clear();
			}
		}
	...
	else
	{
		...
		else
		{
			...
			if (!ns.tcTrafficGoesInHeavyModeFrom) ns.tcTrafficGoesInHeavyModeFrom = tcNow;
			if (tcNow - ns.tcTrafficGoesInHeavyModeFrom > 3 * 60 * 1000) ns.tcHeavyModeWorkedEnoughUntil = tcNow;
		}
	}

	vmsAUTOLOCKSECTION(m_csMisc);
	if (bAtLeast1UsesInternet) ns.vTotalSpeed.AddValue(uTotalInternetSpeed);
	...
	if (uTotalMaxSpeed != UINT64_MAX)
	{
		if (ns.uConnectionBandwidth == UINT64_MAX)
		{
			...
			if (uTotalMaxSpeed) ns.bandwidthMeasureStart.cNonZeroTotalSpeeds++;
...
{
	vmsIpHelper iph;
	if (!iph.GetAdaptersInfo()) return;

	DWORD dwIfIndex;
	if (NO_ERROR != GetBestInterface(inet_addr("8.8.8.8"), &dwIfIndex)) return;

	PIP_ADAPTER_INFO pAdapter = iph.FindAdapterByIfIndex(dwIfIndex);
	if (!pAdapter) return;

	if (pAdapter->IpAddressList.IpAddress.String == m_strCurrentIP) return;

	if (!(m_dwState & TUMM_SPEEDS_INFO_CAN_BE_INCORRECT)) SaveCurrentAdapterInfo();
...
{
	if (!(m_dwState & TUMM_SPEEDS_INFO_CAN_BE_INCORRECT)) return;
	...
	if (!bAllUsesCurrentAdapter) return;
...
void vmsTrafficUsageModeMgr::BuildCurrentSettings_PerDirection(vmsTrafficUsageModeSettings& stgs, bool bForDownload)
{
	...
	if (uConnectionSpeed == UINT64_MAX) uConnectionSpeed = refgetNetworkStat(bForDownload).uConnectionBandwidth;
	...
	if (!s.uTrafficLimit) s.uTrafficLimit = 1;
	if (!s.uMaxTasks) s.uMaxTasks = 1;
	if (!s.uMaxConns) s.uMaxConns = 1;
	if (!s.uMaxConnsPS) s.uMaxConnsPS = 1;
}
