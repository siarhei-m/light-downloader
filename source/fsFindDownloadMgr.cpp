...
void fsFindDownloadMgr::ReadSettings()
{
	...
	if (m_pGroup == NULL) m_pGroup = _DldsGrps.FindGroup(GRP_OTHER_ID);
...
int fsFindDownloadMgr::GetElementCount()
{
	...
	if (m_dwFlags & SEARCHWH_LISTOFDLDS) cElements += _DldsMgr.GetCount();

	if (m_dwFlags & SEARCHWH_DELETED) cElements += _DldsMgr.Get_DeletedDownloadCount();
...
void fsFindDownloadMgr::FindNext_imp(BOOL* pbCancel, int* piProgress)
{
	do
	{
		...
		if (m_iIndex == -1 && NextFindSource(m_enWI) == FALSE) break;
	} while (m_iIndex == -1);
}
...
void fsFindDownloadMgr::FindNext_InListOfDlds(BOOL* pbCancel, int* piProgress)
{
	...
	for (int i = m_iIndex; i < cDlds; i++)
	{
		...
		if (piProgress) *piProgress = int((double)cDlds / cElems * 100.0);
	}
...
void fsFindDownloadMgr::FindNext_InDeleted(BOOL* pbCancel, int* piProgress)
{
	...
	if (m_dwFlags & SEARCHWH_LISTOFDLDS) cDidElems += _DldsMgr.GetCount();
	...
	for (int i = m_iIndex; i < cDlds; i++)
	{
		...
		if (piProgress) *piProgress = int((double)(cDlds + cDidElems) / cElems * 100.0);
	}
...
BOOL fsFindDownloadMgr::IsSatisfies(vmsDownloadSmartPtr dld)
{
	...
	{
		if (dld->pGroup != m_pGroup) return FALSE;
	}

	if (m_dwFlags & SEARCHU_FILESIZE)
	{
		UINT64 size = dld->pMgr->GetLDFileSize();
		if (size < m_uFileSizeFrom || size > m_uFileSizeTo) return FALSE;
	}

	if (m_dwFlags & SEARCHIN_COMMENT)
		if (fsStrStrNoCase(dld->strComment, m_strWhat)) return TRUE;

	if (m_dwFlags & SEARCHIN_LOCFILENAME)
	{
		if (fsStrStrNoCase(dld->pMgr->get_OutputFilePathName(), m_strWhat)) return TRUE;
	}

	if (m_dwFlags & SEARCHIN_URL)
	{
		if (fsStrStrNoCase(dld->pMgr->get_URL(), m_strWhat)) return TRUE;
	}
	...
	if (m_dwFlags & SEARCHIN_FILENAME)
	{
		...
		if (fsStrStrNoCase(sz, m_strWhat)) return TRUE;
	}
...
BOOL fsFindDownloadMgr::IsSatisfies(fsDLHistoryRecord* rec)
{
	if (m_dwFlags & SEARCHU_FILESIZE)
	{
		if (rec->uFileSize < m_uFileSizeFrom || rec->uFileSize > m_uFileSizeTo) return FALSE;
	}
	...
	if (m_dwFlags & SEARCHU_TIME)
	{
		...
		if (filter.IsSatisfies(rec) == FALSE) return FALSE;
	}

	if (m_dwFlags & SEARCHIN_COMMENT)
		if (fsStrStrNoCase(rec->strComment, m_strWhat)) return TRUE;

	if (m_dwFlags & SEARCHIN_LOCFILENAME)
		if (fsStrStrNoCase(rec->strSavedTo, m_strWhat)) return TRUE;

	if (m_dwFlags & SEARCHIN_URL)
	{
		if (fsStrStrNoCase(rec->strURL, m_strWhat)) return TRUE;
	}

	if (m_dwFlags & SEARCHIN_FILENAME)
	{
		if (fsStrStrNoCase(rec->strFileName, m_strWhat)) return TRUE;
	}
