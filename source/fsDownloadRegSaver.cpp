...
fsDLLoadResult fsDownloadRegSaver::Load(vmsDownloadList& vDownloads, LPCSTR pszFileName, BOOL bDontLoadIfTooLarge,
                                        fsDLLoadFromType lt, BOOL bErrIfNotExists)
{
	LPBYTE pBuffer = NULL;
	...
	fsString strFileName = (LPCSTR)theApp.m_strAppPath;
	...
	hFile = CreateFile(strFileName, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_ALWAYS, 0, NULL);
	...
	if (FALSE == ReadFile(hFile, &hdr, sizeof(hdr), &dw, NULL) || strnicmp(hdr.szSig, DLFILE_SIG, strlen(DLFILE_SIG)))
	{
		goto _lErr;
	}
	else
	{
		dwFileSize -= dw;
		wVer = hdr.wVer;
		if (wVer > DLFILE_CURRENT_VERSION) goto _lErr;
	}
	...
	if (!ReadFile(hFile, pBuffer, dwFileSize, &dw, NULL) || dw != dwFileSize) goto _lErr;
	CloseHandle(hFile);
	hFile = INVALID_HANDLE_VALUE;

	if (!tmp.loadFromStateBuffer(pBuffer, &dwFileSize, wVer)) goto _lErr;
...
_lErr:
	...
	if (lt != DLLFT_SAV) DeleteFile(strFileName);

	if (nlt == DLLFT_SAV) return DLLR_FAILED;
...
{
	if (!vDownloads.isDirty()) return TRUE;

	fsString strFileName = (LPCSTR)theApp.m_strAppPath;
	strFileName += pszFileName;
	strFileName += ".sav";

	fsString strFileNameBak = (LPCSTR)theApp.m_strAppPath;
	strFileNameBak += pszFileName;
	strFileNameBak += ".bak";

	HANDLE hFile = CreateFile(strFileNameBak, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_HIDDEN, NULL);
