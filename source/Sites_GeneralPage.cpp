...
CSites_GeneralPage::CSites_GeneralPage() : CPropertyPage(CSites_GeneralPage::IDD), m_bIsAddingSite(false)
{
...
BOOL CSites_GeneralPage::OnInitDialog()
{
	...
	for (int i = 0; i < _LastUrlPaths.GetRecordCount(); i++) m_wndSite.AddString(_LastUrlPaths.GetRecord(i));

	m_wndGroups.Fill();

	m_wndMaxConnsSpin.SetRange(1, UD_MAXVAL);

	if (m_pSite->strName != NULL) SetDlgItemText(IDC_SITE, m_pSite->strName);

	if (m_pSite->strUser)
	{
		CheckDlgButton(IDC_USELOGIN, BST_CHECKED);
		SetDlgItemText(IDC_USER, m_pSite->strUser);
		if (m_pSite->strPassword) SetDlgItemText(IDC_PASSWORD, m_pSite->strPassword);
	}
...
BOOL CSites_GeneralPage::OnApply()
{
	...
	if (IR_SUCCESS == url.Crack(strSite))
	{
		...
		if (*url.GetUserName())
		{
			...
			if (url.GetPassword()) SetDlgItemText(IDC_PASSWORD, url.GetPassword());
		}
		...
		if (m_advanced->IsUserChangedProtocols() == FALSE)
		{
			...
			if (!m_bIsAddingSite) _SitesMgr.setDirty();
			m_advanced->ToDialog();
		}
	...
	if (IsDlgButtonChecked(IDC_USEFTPPASSIVE) == BST_CHECKED) m_pSite->dwFtpFlags |= FTP_USEPASSIVEMODE;
	if (IsDlgButtonChecked(IDC_DONTSENDLIST2) == BST_CHECKED) m_pSite->dwFtpFlags |= FTP_DONTSENDLIST;
