...
CSites_AdvancedPage::CSites_AdvancedPage() : CPropertyPage(CSites_AdvancedPage::IDD), m_bIsAddingSite(false)
{
...
BOOL CSites_AdvancedPage::OnApply()
{
	...
	if (IsDlgButtonChecked(IDC_VALIDSUBDOMAINS) == BST_CHECKED) dwVF |= SITE_VALIDFOR_SUBDOMAINS;
...
{
	if (m_bDialogWasInited) OnApply();
}

void CSites_AdvancedPage::ToDialog()
{
	if (m_bDialogWasInited == FALSE) return;
