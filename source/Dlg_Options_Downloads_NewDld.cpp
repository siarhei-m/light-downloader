...
BOOL CDlg_Options_Downloads_NewDld::OnInitDialog()
{
	...
	case NGSW_CHOOSE_BY_EXT:
		if (_App.NewDL_GroupId() != (UINT)-1) _App.NewDL_GroupId((UINT)-1);
...
{
	CFolderBrowser* fb =
	    CFolderBrowser::Create(LS(L_CHOOSEOUTFOLDER), _DldsGrps.FindGroup(GRP_OTHER_ID)->strOutFolder, NULL, this);
	if (fb == NULL) return;

	CString str = fb->GetPath();
	if (str.Right(1) != '\\') str += '\\';

	if (MessageBox(LS(L_ALLGRPSFLDRSWBCHANGEDTOTHISFLDR), NULL, MB_YESNO) == IDNO) return;
