...
vmsPersistObject::vmsPersistObject()
    : m_bState(POS_NOTMODIFIEDSINCELASTSAVE), m_persistChildObjects(&m_csObj),
...
{
	if (m_bState & POS_DIRTY) return true;
	...
	{
		if (m_pPersistChildObjects->getPersistObject(i)->isDirty()) return true;
	}
...
void vmsPersistObject::onStateSavedSuccessfully()
{
	...
	if ((m_bState & POS_NOTMODIFIEDSINCELASTSAVE) != 0) m_bState &= ~POS_DIRTY;
...
bool vmsPersistObject::loadFromStateBuffer(LPBYTE pb, LPDWORD pdwSize, DWORD dwVer)
{
	...
	*pdwSize = 0;
	...
	dwSize = dwBufferSize;
	if (!loadObjectItselfFromStateBuffer(pb, &dwSize, dwVer)) return false;
...
{
	{
		...
		if (bSaveToStorage) m_bState |= POS_NOTMODIFIEDSINCELASTSAVE;
	}
