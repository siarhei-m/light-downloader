...
{
	m_general.m_pvDlds = m_connection.m_pvDlds = m_protocol.m_pvDlds = m_proxy.m_pvDlds = m_integrity.m_pvDlds =
	    m_misc.m_pvDlds = pvDlds;

	if (dwPages & DPS_SHP_GENERAL) AddPage(&m_general);
	...
	if (dwPages & DPS_SHP_CONNECTION) AddPage(&m_connection);

	if (dwPages & DPS_SHP_PROTOCOL) AddPage(&m_protocol);

	if (dwPages & DPS_SHP_PROXY) AddPage(&m_proxy);

	if (dwPages & DPS_SHP_INTEGRITY) AddPage(&m_integrity);

	if (dwPages & DPS_SHP_MISC) AddPage(&m_misc);
