...
BOOL CPlugFrame::Create(CWnd* pParent)
{
	...
	if (FALSE ==
	    CWnd::Create(AfxRegisterWndClass(CS_DBLCLKS, LoadCursor(NULL, IDC_ARROW), (HBRUSH)(COLOR_3DFACE + 1), NULL),
	                 NULL, WS_CHILD | WS_VISIBLE, rc, pParent, 0x111))
		return FALSE;
...
void CPlugFrame::OnPaint()
{
	...
	CPen pen(PS_SOLID, 1, m_clrBkCaption), penBorder(PS_SOLID, 2, GetSysColor(COLOR_3DDKSHADOW));
...
{
	if (m_wndPlugin) SetupPluginRect();
...
void CPlugFrame::SetupPluginRect()
{
	...
	::MoveWindow(m_wndPlugin, rcThis.left, rcThis.top, rcThis.right - rcThis.left, rcThis.bottom - rcThis.top, TRUE);
}
