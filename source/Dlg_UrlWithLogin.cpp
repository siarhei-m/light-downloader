...
CUrlWithLoginDlg::CUrlWithLoginDlg(CWnd* pParent) : CDialog(CUrlWithLoginDlg::IDD, pParent)
{
...
BOOL CUrlWithLoginDlg::OnInitDialog()
{
	...
	if (m_strUrl.GetLength() == 0)
	{
		...
		{
			fsURL url;
			if (url.Crack(pszUrl) == IR_SUCCESS && *url.GetHostName()) m_strUrl = pszUrl;
		}

		if (m_strUrl.GetLength() == 0) m_strUrl = "http://";
	}
