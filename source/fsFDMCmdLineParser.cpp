...
void fsFDMCmdLineParser::Parse(PerformTasksOfType enPTT)
{
	...
	HWND hWnd = NULL;
	if (m_bAnotherFDMStarted) hWnd = FindWindow(MY_CLASSNAME, NULL);

	if (FALSE == m_parser.Parse()) return;
...
void fsFDMCmdLineParser::ReadIntVector(LPCTSTR ptsz, std::vector<int>& v)
{
	...
	{
		tstring tstr;
		while (_istdigit(*ptsz)) tstr += *ptsz++;
		if (!tstr.empty()) v.push_back(_ttoi(tstr.c_str()));
		if (*ptsz++ != ',') return;
	}
}
