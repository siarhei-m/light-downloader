...
template <class T> void InitArray(CArray<T, T&>& arrDest)
{
...
template <class T> void CleanArray(CArray<T, T&>& arrDest)
{
	arrDest.RemoveAll();
	arrDest.FreeExtra();
}
...
template <class T> void CopyArray(CArray<T, T&>& arrDest, CArray<T, T&>& arrSrc)
{
	CleanArray(arrDest);
...
template <class T> void CopyArray(CArray<T, T&>& arrDest, const CArray<T, T&>& arrSrc)
{
	CleanArray(arrDest);
...
#define CHECK_BUFFER_BOUNDS(buff_size, size)                                                                           \
	if (pbtBuffer != NULL && (buff_size - (pbtCurrentPos - pbtBuffer) < size)) return;
#define BOOLEAN_CHECK_BUFFER_BOUNDS(buff_size, size)                                                                   \
	if (pbtBuffer != NULL && (buff_size - (pbtCurrentPos - pbtBuffer) < size)) return false;

void putStrToBuffer(LPCTSTR tszStr, LPBYTE& pbtCurrentPos, LPBYTE pbtBuffer, DWORD dwBufferSizeSize,
                    DWORD* pdwSizeReuiqred);
...
template <class T> bool getVarFromBuffer(T& t, LPBYTE& pbtCurrentPos, LPBYTE pbtBuffer, DWORD dwBufferSizeSize)
{
	if (pbtBuffer == 0 || pbtCurrentPos == 0) return false;
...
template <class T>
void putListToBuffer(fs::list<T>* pvList, LPBYTE& pbtCurrentPos, LPBYTE pbtBuffer, DWORD dwBufferSize,
                     DWORD* pdwSizeRequired)
{
	if (pvList == 0) return;
