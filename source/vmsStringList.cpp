...
BOOL vmsStringList::Save(HANDLE hFile)
{
	...
	if (FALSE == WriteFile(hFile, &c, sizeof(c), &dw, NULL)) return FALSE;

	for (int i = 0; i < c; i++)
	{
		if (FALSE == fsSaveStrToFile(get_String(i), hFile)) return FALSE;
	}
...
BOOL vmsStringList::Load(HANDLE hFile)
{
	...
	for (int i = 0; i < c; i++)
	{
		...
		if (FALSE == fsReadStrFromFile(&psz, hFile)) return FALSE;
