...
BOOL CDlg_Options_General_Misc::OnInitDialog()
{
	...
	CComboBox* pboxes[] = {&m_wndExitTout, &m_wndShutdownTout, &m_wndLaunchDldTout};

	for (UINT i = 0; i < sizeof(pboxes) / sizeof(CComboBox*); i++)
	{
		pboxes[i]->AddString(LS(L_DONTASKFORCONF));

		UINT aSecs[] = {5, 15, 30, 60};
		...
		{
		case 0:
			uTimeout = _App.ConfTimeout_Exit();
			break;
		case 1:
			uTimeout = _App.ConfTimeout_Shutdown();
			break;
		case 2:
			uTimeout = _App.ConfTimeout_LaunchDld();
			break;
		default:
			ASSERT(FALSE);
			uTimeout = 0;
