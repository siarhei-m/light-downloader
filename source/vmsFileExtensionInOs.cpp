...
{
	fsString strExt;
	strExt = appendDot ? _T(".") : _T("");
	strExt += pszExt;

	CRegKey key;
	if (ERROR_SUCCESS != key.Open(HKEY_CLASSES_ROOT, strExt, KEY_READ)) return "";
...
fsString vmsFileExtensionInOs::GetAssociation_2(LPCSTR pszProgId, LPCSTR pszVerb)
{
	...
	if (ERROR_SUCCESS != key.Open(HKEY_CLASSES_ROOT, str, KEY_READ)) return "";

	char sz[MY_MAX_PATH];
	DWORD dw = sizeof(sz);
	if (ERROR_SUCCESS != key.QueryValue(sz, NULL, &dw)) return "";
...
BOOL vmsFileExtensionInOs::SetAssociation_2(LPCSTR pszProgId, LPCSTR pszVerb, LPCSTR pszValue)
{
	...
	if (lErr != ERROR_SUCCESS) SetLastError(lErr);
...
BOOL vmsFileExtensionInOs::CreateAssociation(LPCSTR pszExt, LPCSTR pszProgId)
{
	...
	if (ERROR_SUCCESS != key.SetValue(pszProgId, NULL)) return FALSE;
...
LONG vmsFileExtensionInOs::OpenAssociationKey(CRegKey& key, const tstring& tstrExt, bool bCreateIfDoesNotExist,
                                              REGSAM samDesired)
{
