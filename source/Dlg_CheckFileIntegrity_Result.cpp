...
BOOL CDlg_CheckFileIntegrity_Result::OnInitDialog()
{
	...
	if (m_bResultOK == false)
	{
		str += LS(L_VALIDCHECKSUM);
		str += ": ";
		str += m_strValidHash;
		str += "\r\n";
		str += LS(L_RESULTCHECKSUM);
		str += ": ";
		str += m_strResultHash;
	}
...
void CDlg_CheckFileIntegrity_Result::ApplyLanguage()
{
	...
	if (m_bResultOK == false) SetDlgItemText(IDOK, LS(L_CANCEL));
...
{
	if (IsDlgButtonChecked(IDC_NEVERSHOWTHISIFTESTOK) == BST_CHECKED) _App.View_DontShowHashCheckResultIfOK(TRUE);
