...
BOOL CSheduler_Tasks::Create(CWnd* pParent)
{
	...
	if (FALSE == CListCtrl::Create(LVS_REPORT | LVS_NOSORTHEADER | LVS_SHOWSELALWAYS | LVS_OWNERDRAWFIXED, rc, pParent,
	                               0x7645329))
		return FALSE;
...
void CSheduler_Tasks::OnTasksDeletetask()
{
	...
	while (pos) vItems.add(GetNextSelectedItem(pos));
...
void CSheduler_Tasks::UpdateMenu(CMenu* pPopup)
{
	...
	else
	{
		...
		if (cItems > 1) pPopup->EnableMenuItem(ID_TASKS_PROPERTIES, MF_BYCOMMAND | MF_GRAYED);
	}
}
...
int CSheduler_Tasks::OnUpdateToolBar(UINT nID)
{
	...
	else
	{
		...
		while (pos)
		{
			...
			{
			case ID_TASKS_ENABLE:
				if (task->dwFlags & SCHEDULE_ENABLED) bEnabled = FALSE;
				break;

			case ID_TASKS_DISABLE:
				if ((task->dwFlags & SCHEDULE_ENABLED) == 0) bEnabled = FALSE;
				break;
			}
		}

		if (cItems > 1 && nID == ID_TASKS_PROPERTIES) bEnabled = FALSE;
	}
...
{
	int cItems = GetItemCount();
	while (cItems--) UpdateTask(cItems);
}
