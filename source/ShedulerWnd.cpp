...
BOOL CShedulerWnd::Create(CWnd* pParent)
{
	...
	if (FALSE ==
	    CWnd::Create(AfxRegisterWndClass(0, NULL, NULL, NULL), NULL, WS_CHILD | WS_VISIBLE, rc, pParent, 0x16721))
		return FALSE;
...
int CShedulerWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (FALSE == m_wndTasks.Create(this)) return -1;

	if (FALSE == m_wndLog.Create(this)) return -1;

	if (FALSE == m_wndSplitter.Create(AfxGetInstanceHandle(), m_hWnd, WST_HORIZONTAL)) return -1;
...
void CShedulerWnd::OnProperties(fsSchedule* task)
{
	...
	if (IDOK != _DlgMgr.DoModal(&sheet)) return;

	if (sheet.IsNeedRecalculateStartTime()) m_mgr.CalculateStartTime(task);
...
void CShedulerWnd::LoadTasks()
{
	...
	if (GetFileAttributes(strFile) == DWORD(-1)) return;
	...
	for (int i = 0; i < cEvents; i++) m_wndLog.AddRecord(m_evMgr.GetEvent(i));
}
...
{
	static fsSetImage images[] = {fsSetImage(ID_CREATENEWTASK, 0), fsSetImage(ID_TASKS_PROPERTIES, 1),
...
BOOL CShedulerWnd::ExitWhenDone(int* ppos)
{
	...
	do
	{
		...
		if (pos != -1 && m_mgr.GetTask(pos)->hts.enType == HTS_WHENDONE) break;
	} while (pos != -1);

	if (pos != -1) bEn = (m_mgr.GetTask(pos)->dwFlags & SCHEDULE_ENABLED) != 0;
...
{
	int pos;
	if (!bUse == !ExitWhenDone(&pos)) return;
	...
	if (bUse)
	{
		...
		fsSchedule& task = schScheduleParam.schTask;
		task.dwFlags = SCHEDULE_ENABLED;
		if (_App.WD_DisableAfterExec()) task.dwFlags |= SCHEDULE_AUTODIS;
...
void CShedulerWnd::TurnoffWhenDone(fsShutdownType enST, BOOL bUse)
{
	...
	int pos;
	if (!bUse == !TurnoffWhenDone(enST, &pos)) return;
...
void CShedulerWnd::ApplyLanguageToMenuView(CMenu* menu)
{
	...
	LPCSTR apszCmds[] = {LS(L_WTS),  LS(L_HTS),  LS(L_NEXTSTART),  LS(L_LASTSTART),
...
void CShedulerWnd::UpdateTasksListColMenu(CMenu* menu)
{
	...
	for (int i = 0; i < 4; i++)
	{
		if (m_wndTasks.IsColumnShown(i))
		{
			...
			if (bEn == FALSE) menu->EnableMenuItem(ID_TASKS_1 + i, MF_GRAYED);
		}
	}
}
...
void CShedulerWnd::UpdateLogColMenu(CMenu* menu)
{
	...
	for (int i = 0; i < 3; i++)
	{
		if (m_wndLog.IsColumnShown(i))
		{
			...
			if (bEn == FALSE) menu->EnableMenuItem(ID_LOG_1 + i, MF_GRAYED);
		}
	}
}
...
void CShedulerWnd::onChildObjectLoadFinished(vmsPersistObject* pObject, bool bResult)
{
	...
	if (pObject == &m_mgr)
	{
		if (_App.App_2_0_SchedulerFixPerformed() == FALSE)
		{
			...
			while (-1 != (pos = m_mgr.FindTask(WTS_SHUTDOWN))) m_mgr.DeleteTask(m_mgr.GetTask(pos));

			while (-1 != (pos = m_mgr.FindTask(WTS_EXIT))) m_mgr.DeleteTask(m_mgr.GetTask(pos));
		...
		for (int i = 0; i < cTasks; i++) m_wndTasks.AddTask(m_mgr.GetTask(i));
