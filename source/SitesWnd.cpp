...
BOOL CSitesWnd::Create(CWnd* pWndParent)
{
	...
	if (FALSE == CListCtrlEx::Create(LVS_REPORT | LVS_OWNERDRAWFIXED | LVS_NOSORTHEADER | LVS_SHOWSELALWAYS, rc,
	                                 pWndParent, 0x53459))
		return FALSE;
...
void CSitesWnd::UpdateMenu(CMenu* pMenu)
{
	...
	else
	{
		...
		if (m_bSelectedIsTemp) pMenu->CheckMenuItem(ID_SITES_TEMPRORARY, MF_BYCOMMAND | MF_CHECKED);
	}

	if (cSelected != 1) pMenu->EnableMenuItem(ID_SITES_PROPERTIES, MF_BYCOMMAND | MF_GRAYED);
...
void CSitesWnd::UpdateSite(fsSiteInfo* pSite)
{
	...
	if (pSite->cMaxConns != UINT_MAX)
		...
		str = "+";
	strConn.Format("%d/%s", pSite->cConnsNow, str.GetString());
	SetItemText(iItem, 1, strConn);

	SetItemText(iItem, 2, pSite->strUser == NULL ? "" : (LPCSTR)pSite->strUser);
...
void CSitesWnd::LoadSites()
{
	...
	try
	{
		...
		if (!_SitesMgr.loadFromStateBuffer(pbtBuffer, &dwRequiredSize, SITESFILE_CURRENT_VERSION)) // hdr.wVer
		{
			CloseHandle(hFile);
			_DldsMgr.RebuildServerList();
			return;
...
{
	if (dwWhat & 1) SaveSites();
	if (dwWhat & 2) SaveState("SitesList");
}
...
void CSitesWnd::OnForceUpdate()
{
	...
	for (int i = 0; i < cSites; i++) AddSiteToList(_SitesMgr.GetSite(i));
}
...
void CSitesWnd::UpdateSitesListColMenu(CMenu* menu)
{
	...
	for (int i = 0; i < 4; i++)
	{
		if (IsColumnShown(i))
		{
			...
			if (bEn == FALSE) menu->EnableMenuItem(ID_SITES_1 + i, MF_GRAYED);
		}
	}
}
