...
bool vmsSecurity::VerifySign(LPCTSTR ptszFile, LPCTSTR ptszPubKey)
{
	...
	HCRYPTKEY hKey;
	if (FALSE == CryptImportKey(hProv, abKey, dwKeySize, NULL, 0, &hKey)) return false;
	...
	while (pbSig != pbData && strncmp((char*)pbSig, FdmCryptSignatureMarker, nSigLen) != 0) pbSig--;
	if (pbData == pbSig)
	{
		delete[] pbData;
		return false;
	...
	HCRYPTHASH hHash;
	if (FALSE == CryptCreateHash(hProv, CALG_MD5, 0, 0, &hHash)) return false;

	if (FALSE == CryptHashData(hHash, pbData, pbSig - pbData, 0)) return false;

	BOOL bResult = CryptVerifySignature(hHash, pbSig + nSigLen, pbData + dwDataSize - pbSig - nSigLen, hKey, NULL, 0);
...
bool vmsSecurity::ExtractFileFromSignedFile(LPCTSTR ptszSrcFile, LPCTSTR ptszDstFile)
{
	...
	while (pbSig != pbData && strncmp((char*)pbSig, FdmCryptSignatureMarker, nSigLen) != 0) pbSig--;
	delete[] pbData;
	if (pbData == pbSig) return false;

	UINT nSignatureSize = dwDataSize - (pbSig - pbData);

	if (FALSE == CopyFile(ptszSrcFile, ptszDstFile, FALSE)) return false;
