...
{
	tstr = _T ("");
	while (isWhiteChar(*ptsz)) ptsz++;
	while (*ptsz && isWhiteChar(*ptsz) == false) tstr += *ptsz++;
}
...
{
	tstr = _T ("");
	while (*ptsz && *ptsz != tch && isWhiteChar(*ptsz) == false) tstr += *ptsz++;
}
...
void vmsStringParser::GetAlphaWord(LPCTSTR& ptsz, tstring& tstr)
{
	tstr = _T ("");
	while (isWhiteChar(*ptsz)) ptsz++;
	while (_istalpha(*ptsz)) tstr += *ptsz++;
}
