...
CCsvParser::CCsvParser() : m_nPos(0) {}
...
{
	if (nStartChar < 0 || nStartChar > sValue.GetLength() - 1) return -1;
...
void CCsvParser::decodeValue(bool bIsQuote, int nClosingQuotePos, CString& sValue)
{
	if (bIsQuote)
	{
		ASSERT((nClosingQuotePos == -1 || nClosingQuotePos >= 2) && (nClosingQuotePos <= sValue.GetLength() - 1));
		...
		if (nClosingQuotePos != -1)
		{
			...
			if (nClosingQuotePos <= sValue.GetLength() - 1) sSecondPart = sValue.Mid(nClosingQuotePos + 1);
		}
		else
		{
...
void CCsvParser::Sgets()
{
	...
	while ((c != '\n' || bIsField) && m_nPos <= m_sCsvContent.GetLength() - 1)
	{
		...
		if (bIsTextQulifierScope)
		{
			...
			{
				if (!bIsField) bIsTextQulifierScope = false;
			}
		}
		...
		++m_nPos;
		if (m_nPos <= m_sCsvContent.GetLength() - 1) c = m_sCsvContent[m_nPos];
	}
