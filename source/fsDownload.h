...
typedef DWORD (*fntDownloadsMgrEventFunc)(struct fsDownload* dld, struct fsDLHistoryRecord* phst,
                                          fsDownloadsMgrEvent ev, LPVOID);
...
struct fsDownload : public vmsObject, public vmsPersistObject
{
  ...
  public:
	...
	{
		if (pMgr) getPersistObjectChildren()->removePersistObject(pMgr);
		pMgr = a_pMgr;
		if (pMgr) getPersistObjectChildren()->addPersistObject(pMgr);
	}
	...
	void setStateFlags(DWORD _dwFlags)
	{
		LockStateSection(true);
		dwState |= _dwFlags;
		LockStateSection(false);
	}
	void removeStateFlags(DWORD _dwFlags)
	{
		LockStateSection(true);
		dwState &= ~_dwFlags;
		LockStateSection(false);
