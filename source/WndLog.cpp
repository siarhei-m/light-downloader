...
BOOL CWndLog::Create(CWnd* pParent)
{
	...
	if (FALSE ==
	    CListCtrl::Create(LVS_REPORT | LVS_NOSORTHEADER | LVS_SHOWSELALWAYS | LVS_OWNERDRAWFIXED, rc, pParent, 0x76329))
		return FALSE;
...
void CWndLog::OnForceUpdate()
{
	...
	if (m_pEvMgr)
	{
		...
		for (int i = 0; i < cEvents; i++) AddRecord(m_pEvMgr->GetEvent(i));
	}
}
