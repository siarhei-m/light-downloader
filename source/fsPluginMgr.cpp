...
BOOL fsPluginMgr::LoadPlugins(CWnd* pPluginsFrame)
{
	...
	if (FALSE == LoadBuiltIn()) return FALSE;

	for (int i = m_vWndPlugs.size() - 1; i; i--) ShowWindow(m_vWndPlugs[i].hWnd, SW_HIDE);
...
BOOL fsPluginMgr::OnAppExit(BOOL bQueryForExit)
{
	...
	if (m_bWasExit) return TRUE;

	if (bQueryForExit)
	{
		if (FALSE == QueryExit()) return FALSE;
	}
...
void fsPluginMgr::OnNewLng()
{
	for (int i = 0; i < m_vWndPlugs.size(); i++)
	{
		...
		if (plug->pfnSetLanguage) plug->pfnSetLanguage(WGL_UNKNOWN, plug->hMenuMain, plug->hMenuView);
		...
		for (int j = 0; j < cButtons; j++) ppszToolTips[j] = pButtons[j].pszToolTip;
...
BOOL fsPluginMgr::QueryExit()
{
	for (int i = 0; i < m_vWndPlugs.size(); i++)
	{
		...
		{
			if (m_vWndPlugs[i].pfnRTS() == FALSE) return FALSE;
		}
		else
		{
			if (0 != SendMessage(m_vWndPlugs[i].hWnd, WM_WGP_READYTOSHUTDOWN, 0, 0)) return FALSE;
		}
	}
