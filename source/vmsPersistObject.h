...
class vmsPersistObjectsStandardContainer : public vmsPersistObjectsContainerBase
{
  public:
	...
	void removePersistObject(vmsPersistObject* pObj)
	{
		...
		if (nIndex != size_t(-1)) removePersistObject(nIndex);
	}

	void removePersistObject(size_t nIndex)
	{
		vmsCriticalSectionAutoLock csal(m_pcsObj);
		if (nIndex >= m_vPersistObjects.size()) return;
	...
	size_t findPersistObjectIndex(vmsPersistObject* pObj)
	{
		...
		{
			if (m_vPersistObjects[i] == pObj) return i;
		}
		return size_t(-1);
	}
...
class vmsPersistObjectsArray : public vmsPersistObject, protected vmsPersistObjectsContainerBase
{
  public:
	...
	{
		assert(nIndex < m_vObj.size());
		if (nIndex >= m_vObj.size()) throw std::runtime_error("");
	...
	size_t size() const { return m_vObj.size(); }
	...
	void DeleteAll()
	{
		if (m_vObj.empty()) return;
		vmsCriticalSectionAutoLock csalStateGuardAutoLock(&m_csObj);
		m_vObj.clear();
		setDirty();
	}

	bool IsEmpty() const { return m_vObj.empty(); }

  public:
	vmsPersistObjectsArray() { m_pPersistChildObjects = this; }
