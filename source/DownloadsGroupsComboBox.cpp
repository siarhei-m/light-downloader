...
{
	m_wndTree.CreateEx(WS_EX_TOPMOST, WC_TREEVIEW, "",
	                   WS_POPUP | WS_BORDER | TVS_FULLROWSELECT | TVS_HASLINES | TVS_TRACKSELECT, CRect(0, 0, 0, 0),
	                   GetParent(), 0);
	...
	if (m_bHasAllGroupsItem) m_wndTree.Expand(hRoot, TVE_EXPAND);
	...
	if (i == m_vGroupItems.size()) m_wndTree.SelectItem(m_vGroupItems[0].hItem);
...
vmsDownloadsGroupSmartPtr CDownloadsGroupsComboBox::CreateNewGroup(vmsDownloadsGroupSmartPtr pParentGroup)
{
	...
	if (IDCANCEL == _DlgMgr.DoModal(&dlg)) return NULL;
...
void CDownloadsGroupsComboBox::RememberSelectedGroup()
{
	...
	if (pGroup != NULL) _App.CreateDld_IdOfLastGrpSelected(pGroup->nId);
}

BOOL CDownloadsGroupsComboBox::SelectGroupById(UINT nGroupId)
{
	if (nGroupId == (UINT)-1) return FALSE;
...
void CDownloadsGroupsComboBox::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (timeNow - m_timeLastHide > 300)
	{
		...
		if (m_wndTree.IsWindowVisible() == FALSE) ShowDropDown(TRUE);
	}
}
...
void CDownloadsGroupsComboBox::OnSetFocus(CWnd* pOldWnd)
{
	...
	if (pOldWnd != NULL && pOldWnd->m_hWnd == m_wndTree.m_hWnd) ShowDropDown(FALSE);
}
...
void CDownloadsGroupsComboBox::Tree_UpdateSelection(BOOL bPostNotificationMsg)
{
	...
	{
		GetParent()->PostMessage(WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), CBN_SELCHANGE), (LPARAM)m_hWnd);
	}
}
...
void CDownloadsGroupsComboBox::FillGroups(PDLDS_GROUPS_TREE pGroupsTree, HTREEITEM hRoot)
{
	for (int i = 0; i < pGroupsTree->GetLeafCount(); i++)
	{
		...
		item.hItem = m_wndTree.InsertItem(item.pGroup->nId != GRP_OTHER_ID ? (LPCSTR)item.pGroup->strName : LS(L_OTHER),
		                                  hRoot, TVI_SORT);
