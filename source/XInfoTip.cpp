...
void CXInfoTip::OnPaint()
{
	...
	if (m_hIcon != NULL) rc.left = rc.left + m_IconSize.cx + CX_ICON_MARGIN;
...
BOOL CXInfoTip::GetWindowRegion(CDC* pDC, HRGN* hRegion, CSize* Size)
{
	...
	if (m_hIcon != NULL) rcWnd.right = rcWnd.right + m_IconSize.cx + CX_ICON_MARGIN;
	if (rcWnd.Height() < m_IconSize.cy) rcWnd.bottom = rcWnd.top + m_IconSize.cy;
...
int CXInfoTip::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	CRect rc(0, 0, 0, 0);
	m_btnDSA.Create(LS(L_DONTSHOWTHISWINDOWAGAIN), BS_AUTOCHECKBOX | BS_CHECKBOX | WS_CHILD | WS_VISIBLE, rc, this,
	                IDC_DSA);
...
void CXInfoTip::OnTimer(UINT_PTR nIDEvent)
{
	HRGN hRegion;
	CSize WindowSize;
	CDC* pDC;
	...
	case timerShow:
		...
		SetWindowPos(&wndTop, m_ptOrigin.x - WindowSize.cx + CX_ROUNDED, m_ptOrigin.y - WindowSize.cy + CY_ROUNDED,
...
{
	if (m_nTimer == timerShowNAH) return;
