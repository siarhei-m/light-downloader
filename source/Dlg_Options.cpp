...
CDlg_Options::CDlg_Options(CWnd* pParent, LPCSTR pszStartPage) : CDialog(CDlg_Options::IDD, pParent)
{
	m_fntCaption.CreateFont(15, 0, 0, 0, FW_BOLD, 0, 0, 0, DEFAULT_CHARSET, 0, 0, 0, 0, _T ("Tahoma"));

	if (pszStartPage) m_strStartPage = pszStartPage;
...
BOOL CDlg_Options::OnInitDialog()
{
	...
	GetVersionEx(&ver);
	if (ver.dwMajorVersion > 5) SetTimer(1, 300, NULL);
...
void CDlg_Options::set_CurPage(LPOPTIONS_PAGES_TREE ptPage)
{
	...
	if (m_ptCurPage == ptPage) return;

	if (m_ptCurPage && m_ptCurPage->GetData().page) m_ptCurPage->GetData().page->ShowWindow(SW_HIDE);

	if (IsWindow(ptPage->GetData().page->m_hWnd) == FALSE) CreatePageDialog(ptPage->GetData().page);
...
void CDlg_Options::CreatePageDialog(CDlg_Options_Page* page)
{
	...
	CRect rc;
	GetDlgItem(IDC__DLGFRAME)->GetWindowRect(&rc);
	ScreenToClient(&rc);
...
{
	if (FALSE == OptionsTree_Apply()) return;
	CDialog::OnOK();
}

void CDlg_Options::FillPagesList(LPOPTIONS_PAGES_TREE ptRoot, HTREEITEM htRoot)
{
	if (!ptRoot) ptRoot = m_tPages;

	for (int i = 0; i < ptRoot->GetLeafCount(); i++)
	{
		LPOPTIONS_PAGES_TREE ptPage = ptRoot->GetLeaf(i);
		CString str =
		    ptPage->GetData().page ? ptPage->GetData().page->get_PageShortTitle() : ptPage->GetData().strJustCaption;
		HTREEITEM hPage = m_wndPagesList.InsertItem(TVIF_TEXT | TVIF_PARAM | TVIF_STATE, str, 0, 0, TVIS_EXPANDED,
		                                            TVIS_EXPANDED, (DWORD_PTR)ptPage, htRoot, TVI_LAST);
		if (!m_strStartPage.IsEmpty() && m_strStartPage == str) m_wndPagesList.Select(hPage, TVGN_CARET);
		if (ptPage->GetLeafCount()) FillPagesList(ptPage, hPage);
	}
...
BOOL CDlg_Options::OptionsTree_Apply(LPOPTIONS_PAGES_TREE ptRoot, bool* pbBrowserRestartRequired,
                                     CString* ptstrFdmElevateArgs)
{
	if (!ptRoot) ptRoot = m_tPages;

	bool bBrowserRestartRequired = false;
	if (!pbBrowserRestartRequired) pbBrowserRestartRequired = &bBrowserRestartRequired;
	CString tstrFdmElevateArgs;
	if (!ptstrFdmElevateArgs) ptstrFdmElevateArgs = &tstrFdmElevateArgs;
	...
	if (ptRoot == m_tPages)
	{
		if (bBrowserRestartRequired) MessageBox(LS(L_BROWSERRESTARTREQ));
...
void CDlg_Options::OnTimer(UINT_PTR nIDEvent)
{
	if (nIDEvent == 1)
	{
