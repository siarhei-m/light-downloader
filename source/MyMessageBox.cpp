...
CMyMessageBox::CMyMessageBox(CWnd* pParent) : CDialog(CMyMessageBox::IDD, pParent)
{
	m_hIcon = NULL;
	m_bChecked = false;
	m_bTopMost = false;
...
BOOL CMyMessageBox::OnInitDialog()
{
	...
	if (m_hIcon) m_wndIcon.SetIcon(m_hIcon);
	...
	if (m_strBtn2Text != "")
	{
		...
		if (m_strBtn3Text != "") dc->DrawText(m_strBtn3Text, &rcBtn3, DT_CALCRECT);
	}
	...
	rcCBText.right += 25;
	if (rcCBText.Width() > rcText.Width()) rcText.right = rcText.top + rcCBText.Width();
	...
	if (m_strCheckBoxText == "") rcDlg.bottom -= rcCheckbox.Height() + DX;

	if (m_hIcon == NULL) rcDlg.right -= rcIcon.Width() + DX;
	...
	if (DLG_WIDTH < BTNSW) DLG_WIDTH = BTNSW;
	...
	if (m_hIcon == NULL) m_wndIcon.ShowWindow(SW_HIDE);
