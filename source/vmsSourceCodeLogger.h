...
#define SCL_INITOBJECT(moduleName, logSizePerThread)                                                                   \
	vmsSourceCodeLogger _SourceCodeLoggerObject(moduleName, logSizePerThread)
...
#define LOG_TIME_BEGIN(name)                                                                                           \
	LARGE_INTEGER liQpCounts##name;                                                                                    \
	QueryPerformanceCounter(&liQpCounts##name)
#define LOG_TIME_ELAPSED(name, pszDesc)                                                                                \
	{                                                                                                                  \
		LARGE_INTEGER liTmp;                                                                                           \
		QueryPerformanceCounter(&liTmp);                                                                               \
		liTmp.QuadPart -= liQpCounts##name.QuadPart;                                                                   \
		LOG("%s (%s) taken: %d ms (%d counts)", #name, pszDesc, _SourceCodeLoggerObject.QpCountsToMilliseconds(liTmp), \
		    (int)liTmp.QuadPart);                                                                                      \
		LOG_TIME_RESET(name);                                                                                          \
	}
...
class vmsSourceCodeLogger
{
  public:
	...
	int QpCountsToMilliseconds(LARGE_INTEGER liCounts) { return liCounts.QuadPart * 1000 / m_liQpFrequency.QuadPart; }
	...
	void FlushBuffers()
	{
		...
		for (size_t i = 0; i < m_vThreadsLogs.size(); i++) FlushThreadLogBuffer(&m_vThreadsLogs[i]);
	}
	void log(LPCSTR psz, bool bAddNextLine = false)
	{
		threadCtx* thr = getCurrentThreadContext();
		if (thr == NULL) return;
		if (thr->bDisableLog) return;

		thr->strLog += psz;
		if (bAddNextLine) thr->strLog += "\r\n";
		    ...
		    (thr->strLog[thr->strLog.length() - 1] == '\n' || thr->strLog[thr->strLog.length() - 1] == '\r'))
		{
			...
			if (time.wSecond != thr->stLastTimeLogged.wSecond || time.wMinute != thr->stLastTimeLogged.wMinute ||
		...
		if (thr->strLog.length() > m_nBufSizePerThread) FlushThreadLogBuffer(thr);
	}
  ...
  protected:
	void InitializeLogsFolder()
	{
		...
		if (!ptszExeName++) return;
		TCHAR tsz2[MAX_PATH] = _T ("");
		GetTempPath(MAX_PATH, tsz2);
		if (tsz2[_tcslen(tsz2) - 1] != '\\') _tcscat(tsz2, _T ("\\"));
	...
	std::string StringFromError(DWORD dw)
	{
		...
		FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_ALLOCATE_BUFFER, NULL, dw, 0, (LPSTR)&psz, 0, NULL);

		if (psz)
		{
			while (*psz != 0 && (psz[strlen(psz) - 1] == '\n' || psz[strlen(psz) - 1] == '\r'))
				psz[strlen(psz) - 1] = 0;
			if (*psz && psz[strlen(psz) - 1] == '.') psz[strlen(psz) - 1] = 0;
			std::string str = psz;
			LocalFree(psz);
			return str;
		}
	...
	threadCtx* getCurrentThreadContext()
	{
		...
		if (n != -1) return &m_vThreadsLogs[n];
	...
	threadCtx* CreateCurrentThreadContext()
	{
		...
		tstr += tsz2;
		tstr += _T (".sclgr");
		t.hLogFile = CreateFile(tstr.c_str(), GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_ALWAYS,
		                        FILE_ATTRIBUTE_NORMAL, NULL);
		assert(t.hLogFile != INVALID_HANDLE_VALUE);
		if (t.hLogFile == INVALID_HANDLE_VALUE) return NULL;
	...
	{
		DWORD dw;
		if (FALSE == WriteFile(thr->hLogFile, thr->strLog.c_str(), thr->strLog.length(), &dw, NULL)) return;
		thr->strLog = "";
	}
	int findCurrentThreadIndex() const
	{
		...
		for (size_t i = 0; i < m_vThreadsLogs.size(); i++)
		{
			if (m_vThreadsLogs[i].dwThreadId == dwId) return i;
		}
...
class vmsSourceCodeLogger_Function
{
  public:
	...
	~vmsSourceCodeLogger_Function()
	{
		...
		LOG("%s has finished (run time: %d ms (%d counts)).", m_strDesc.c_str(),
...
class vmsSourceCodeLogger_DisableLogIf
{
  public:
	vmsSourceCodeLogger_DisableLogIf(bool b)
	{
		...
		if (m_bLogWasDisabledAlready == false && b) _SourceCodeLoggerObject.DisableLogForCurrentThread(true);
	}

	~vmsSourceCodeLogger_DisableLogIf()
	{
		if (m_bLogWasDisabledAlready == false) _SourceCodeLoggerObject.DisableLogForCurrentThread(false);
	}
