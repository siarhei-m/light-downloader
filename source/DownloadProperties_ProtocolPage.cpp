...
CDownloadProperties_ProtocolPage::CDownloadProperties_ProtocolPage()
    : CPropertyPage(CDownloadProperties_ProtocolPage::IDD)
{
...
LPCSTR _ppszAgentNames[] = {vmsFdmAppMgr::getAppAgentNameUf(), "Firefox 105.0", "Chrome 106.0", "Edge 106.0"};

LPCSTR _ppszAgentValues[] = {
    vmsFdmAppMgr::getAppAgentName(), "Mozilla/5.0 (Windows NT 6.3; Win64; x64; rv:105.0) Gecko/20100101 Firefox/105.0",
...
{
	if (IsDlgButtonChecked(IDC_USEHTTP11) == BST_INDETERMINATE) CheckDlgButton(IDC_USEHTTP11, BST_UNCHECKED);
...
{
	if (IsDlgButtonChecked(IDC_USECOOKIE) == BST_INDETERMINATE) CheckDlgButton(IDC_USECOOKIE, BST_UNCHECKED);
...
{
	if (IsDlgButtonChecked(IDC_USEPASSIVEMODE) == BST_INDETERMINATE) CheckDlgButton(IDC_USEPASSIVEMODE, BST_UNCHECKED);
...
{
	if (IsDlgButtonChecked(IDC_DONTSENDLIST) == BST_INDETERMINATE) CheckDlgButton(IDC_DONTSENDLIST, BST_UNCHECKED);
