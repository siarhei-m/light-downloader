		...
		if (pch && *(pch + 1) != 0)
		{
			*pch = 0;
			...
			bAdded = UINT_MAX != _pwndDownloads->CreateDownload(pszStartUrl, FALSE, NULL, NULL, bSilent,
			                                                    DWCD_NOFORCEAUTOLAUNCH, NULL, &params);
		}
		else
			bAdded = UINT_MAX != _pwndDownloads->CreateDownload(pszStartUrl, FALSE, NULL, NULL, bSilent);

		// BOOL bAdded = UINT_MAX != _pwndDownloads->CreateDownload(pszStartUrl, FALSE, NULL, NULL, bSilent);
		if (bAdded && bSilent) ShowTimeoutBalloon(pszStartUrl, LS(L_DOWNLOADADDED), NIIF_INFO, TRUE);
		free(pszStartUrl);
	}
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (FALSE == _TBMgr.Create(this)) return -1;
	...
	if (FALSE == _TBMgr.InsertGroup(btns, &bmp, &bmpd, sizeof(btns) / sizeof(wgTButtonInfo))) return -1;
	...
	if (!m_wndView.Create(AfxRegisterWndClass(0, LoadCursor(NULL, IDC_ARROW), (HBRUSH)(COLOR_3DFACE + 1), NULL), NULL,
	                      AFX_WS_DEFAULT_VIEW | WS_CHILD, CRect(0, 0, 0, 0), this, AFX_IDW_PANE_FIRST, NULL))
	{
	...
	if (_App.View_Toolbar() == FALSE)
	{
		...
		if (pBar != NULL) ShowControlBar(pBar, FALSE, FALSE);
	}
	...
	_TrayMgr.Create(m_hWnd, pnIcons, sizeof(pnIcons) / sizeof(UINT), vmsFdmAppMgr::getAppName(), WM_TRAYMSG);
...
{
	if (hSrcMenu == NULL) return;
	if (m_cMenuMainItemsAdded) return;
...
{
	if (onExit(m_bQueryForExit)) CFrameWnd::OnClose();
}
...
{
	if (_TumMgr.GetTUM() == TUM_HEAVY) return;
...
{
	if (_TumMgr.GetTUM() == TUM_LIGHT) return;
...
{
	if (_TumMgr.GetTUM() == TUM_MEDIUM) return;
...
{
	if (_DldsMgr.IsDeletingNow()) return;
...
LRESULT CMainFrame::OnTrayMsg(WPARAM, LPARAM lp)
{
	...
	{
	case WM_MOUSEMOVE:
		if (strlen(_TrayMgr.getTip())) _TrayMgr.setTip("");
...
void CMainFrame::OnSize(UINT nType, int cx, int cy)
{
	...
	{
		if (m_bMinToTray) ShowWindow(SW_HIDE);
	}
	else
	{
		CFrameWnd::OnSize(nType, cx, cy);
		if (nType == SIZE_MAXIMIZED || nType == SIZE_RESTORED) RecalcLayout();
	}
...
void CMainFrame::ShowTrayMenu(BOOL bModeTray)
{
	...
	fsSetImage img[] = {
	    fsSetImage(ID_DLD_CREATE, 18),  fsSetImage(ID_TUM_LIGHT, 0),  fsSetImage(ID_TUM_MEDIUM, 1),
	    fsSetImage(ID_TUM_HEAVY, 2),    fsSetImage(ID_STARTALL, 3),   fsSetImage(ID_STOPALL, 4),
	    fsSetImage(ID_PAUSEALLDLDS, 5), fsSetImage(ID_APP_ABOUT, 10),
	};

	m_odTrayMenu.SetImages(img, sizeof(img) / sizeof(fsSetImage));

	CString strSMW = LS(L_SHOWMAINWND);
	strSMW += '\t';
	CString strCND = LS(L_NEWDL);
	strCND += "\tIns";
	CString strL = LS(L_LIGHT);
	strL += "\tCtrl+1";
	CString strM = LS(L_MEDIUM);
	strM += "\tCtrl+2";
	CString strH = LS(L_HEAVY);
	strH += "\tCtrl+3";
	CString strExit = LS(L_EXIT);
	strExit += "\tAlt+X";
	CString sImpFromClip = LS(L_IMPORTURLSFROMCLIPBOARD);
	sImpFromClip += "\tCtrl+Shift+V";
...
{
	if (!m_imgsMenu.Create(16, 16, ILC_COLOR32 | ILC_MASK, 30, 5)) return;

	if (!m_dimgsMenu.Create(16, 16, ILC_COLOR32 | ILC_MASK, 30, 5)) return;

	UINT aImgs[] = {
	    IDB_TOOL0_16, IDB_TOOL_DLD_16, IDB_TOOL_DLD_16, IDB_TOOL_DLD_16, IDB_TOOL_SCH_16, IDB_TOOL_SITES_16,
	};

	UINT adImgs[] = {IDB_TOOL0_16_D,    IDB_TOOL_DLD_16_D, IDB_TOOL_DLD_16_D,
	                 IDB_TOOL_DLD_16_D, IDB_TOOL_SCH_16_D, IDB_TOOL_SITES_16_D};
...
void CMainFrame::OnTurnoffwhendone()
{
	...
	if (_App.WD_LastShutdownType_DontAsk() == FALSE)
	{
		...
		if (dlg.m_bRemember) _App.WD_LastShutdownType_DontAsk(TRUE);
	}
...
void CMainFrame::BuildSwitchToMenu()
{
	...
	for (int i = 0; i < _PluginMgr.GetPluginCount(); i++)
	{
		...
		if (pData) pData->iCheckImage = ODMENU_CHECKIMAGE_RADIO;
	}
}
...
{
	pCmdUI->SetCheck(pCmdUI->m_nID - ID_SWITCH_TO_START == (UINT)m_wndView.m_wndClient.m_plugs2.GetActivePlugin() ? 1
	                                                                                                              : 0);
}
...
void CMainFrame::RemovePluginMenuView()
{
	...
	if (m_cPluginMenuViewInserted == 0) return;
...
void CMainFrame::AddPluginMenuView(HMENU hMenuView, fsSetImage* pImages, int cImages, wgMenuViewItem* pItems,
                                   int cItems)
{
...
void CMainFrame::OnAltPlusDigit(UINT nID)
{
	...
	if (iPlug < _PluginMgr.GetPluginCount()) m_wndView.m_wndClient.m_plugs2.SetActivePlugin(iPlug);
}
...
void CMainFrame::OnViewToolbar()
{
	...
	CControlBar* pBar = GetControlBar(AFX_IDW_TOOLBAR);
	if (pBar != NULL) _App.View_Toolbar((pBar->GetStyle() & WS_VISIBLE) != 0);
}
...
void CMainFrame::OnViewStatusbar()
{
	...
	CControlBar* pBar = GetControlBar(AFX_IDW_STATUS_BAR);
	if (pBar != NULL) _App.View_Statusbar((pBar->GetStyle() & WS_VISIBLE) != 0);
}
...
void CMainFrame::ApplyLanguage()
{
	...
	LPCSTR ppszToolTips[] = {"", LS(L_LIGHT),        LS(L_MEDIUM),      LS(L_HEAVY),
...
void CMainFrame::ApplyLanguageToMenu()
{
	...
	fsSetText aMenuBarTexts[] = {
	    fsSetText(0, LS(L_FILE)),  fsSetText(1, LS(L_VIEW)), fsSetText(2, LS(L_OPTIONS)),
	    fsSetText(3, LS(L_TOOLS)), fsSetText(4, LS(L_HELP)),
	};
	m_odmenu.SetMenuItemsText(GetMenu(), aMenuBarTexts, sizeof(aMenuBarTexts) / sizeof(fsSetText), TRUE);

	CString strCut = LS(L_CUT), strCopy = LS(L_COPY), strPaste = LS(L_PASTE), strDldOpt = LS(L_SETTINGS),
	        strDldDef = LS(L_DLDDEFOPT), strPrgGen = LS(L_PRGGENSET), strLight = LS(L_LIGHT), strMedium = LS(L_MEDIUM),
	        strHeavy = LS(L_HEAVY), strExit = LS(L_EXIT), strExport = LS(L_EXPORTSETTINGS), strSaveAll = LS(L_SAVEALL),
	...
	m_odmenu.SetMenuItemsText(GetMenu()->GetSubMenu(0), aMenu0Texts, sizeof(aMenu0Texts) / sizeof(fsSetText), TRUE);
...
{
	fsSetImage images[] = {
	    fsSetImage(ID_DLDROPTIONS, 6), fsSetImage(ID_DLDDEFOPTIONS, 9),

	    fsSetImage(ID_TUM_LIGHT, 0),   fsSetImage(ID_TUM_MEDIUM, 1),    fsSetImage(ID_TUM_HEAVY, 2),
...
void CMainFrame::AddPluginMenuView(HMENU hMenu, fsSetImage* pImages, int cImages)
{
	...
	if (m_cMenuViewItemsAdded) return;

	m_cMenuViewItemsAdded =
	    m_odmenu.InsertMenu(CMenu::FromHandle(hMenu), PLUGIN_MENU_VIEW_POS, GetMenu()->GetSubMenu(1)); // 8 - 3
...
void CMainFrame::RemovePluginMenuView2()
{
	if (m_cMenuViewItemsAdded)
	{
		m_odmenu.RemoveMenu(PLUGIN_MENU_VIEW_POS + m_cMenuViewItemsAdded - 1, m_cMenuViewItemsAdded,
		                    GetMenu()->GetSubMenu(1)); // 8 - 3
...
{
	if (!CFrameWnd::OnQueryEndSession()) return FALSE;
...
{
	fsPluginInfo& plug = _PluginMgr.Plugin(m_wndView.m_wndClient.m_plugs2.GetActivePlugin());
...
{
	if (bCheckSettings && _App.Notif_UseBalloon() == FALSE) return;
...
void CMainFrame::RecalcLayout(BOOL bNotify)
{
	...
	if (IsWindow(_TBMgr) == FALSE || IsWindow(m_wndView) == FALSE) return;
...
void CMainFrame::OnProceedFurherInitialization()
{
	...
	for (i = 0; i < app->m_vUrlsToAdd.size(); i++)
	{
		...
		if (app->m_vUrlsToAdd[i].bForceFilename)
		{
			...
			// UINT CreateDownload(LPCSTR pszStartUrl, BOOL bReqTopMostDialog = FALSE, LPCSTR pszComment = NULL, LPCSTR
			// pszReferer = NULL,	BOOL bSilent = FALSE, 	DWORD dwForceAutoLaunch = DWCD_NOFORCEAUTOLAUNCH, BOOL*
			//pbAutoStart = NULL, vmsDWCD_AdditionalParameters* pParams = NULL, UINT* pRes = NULL
			//);
			bAdded = UINT_MAX != _pwndDownloads->CreateDownload(app->m_vUrlsToAdd[i].strUrl, FALSE, NULL, NULL, bSilent,
		...
		{
			bAdded =
			    UINT_MAX != _pwndDownloads->CreateDownload(app->m_vUrlsToAdd[i].strUrl, FALSE, NULL, NULL, bSilent);
		}

		if (bAdded && bSilent) ShowTimeoutBalloon(app->m_vUrlsToAdd[i].strUrl, LS(L_DOWNLOADADDED), NIIF_INFO, TRUE);
	}
...
void CMainFrame::OnImportlistofdownloads()
{
	...
	if (_DlgMgr.DoModal(&dlg) == IDCANCEL) return;
...
{
	if (!m_imgsTool.Create(_TB_SIZE_X, _TB_SIZE_Y, ILC_COLOR32 | ILC_MASK, 10, 5)) return;

	if (!m_dimgsTool.Create(_TB_SIZE_X, _TB_SIZE_Y, ILC_COLOR32 | ILC_MASK, 10, 5)) return;
...
void CMainFrame::OnFileImportImportlistofdownloadsfromclipboard()
{
	...
	while (*pszURLs)
	{
		while (mainfrm__is_separator(*pszURLs)) *pszURLs++;
		...
		while (*pszURLs)
		{
			if (mainfrm__is_separator(*pszURLs))
			{
				...
				while (mainfrm__is_separator(*psz)) psz++;
				if (*psz == 0) break;
				if (_strnicmp(psz, "http://", 7) == 0 || _strnicmp(psz, "https://", 8) == 0 ||
		...
		if (strURL != "")
		{
			if (pGrp == NULL)
			{
				...
				if (IDOK != _DlgMgr.DoModal(&dlg)) return;
				pGrp = dlg.m_pGroup;
			}
			...
			if (FALSE == _pwndDownloads->CreateDownloadWithDefSettings(dld, strURL)) continue;
	...
	if (v.size() != 0) _pwndDownloads->CreateDownloads(v, NULL);
}
...
BOOL CMainFrame::ImportListOfDownloads_FromURLListFile(LPCSTR pszFile)
{
	...
	do
	{
		...
		if (strUrl != "")
		{
			...
			{
				CDlg_ChooseGroup dlg;
				if (IDOK != _DlgMgr.DoModal(&dlg)) return FALSE;
				pGrp = dlg.m_pGroup;
			}
...
BOOL CMainFrame::ImportListOfDownloads_FromDLInfoListFile(LPCSTR pszFile)
{
	...
	pugi::xml_node root = doc.child("FDM_Downloads_Info_List");
	...
	{
		if (strcmp(child.name(), "download") == 0) ImportDownload(child, v);
	}
...
BOOL CMainFrame::ImportDownload(pugi::xml_node node, DLDS_LIST_REF v)
{
	vmsDWCD_AdditionalParameters params;
	params.dwMask = 0;
	CString strUrl;
	...
	for (pugi::xml_node child : node.children())
	{
		...
		{
			strUrl = child.child_value();
		}
		else if (strcmp(name, "comment") == 0)
		{
			strComment = child.child_value();
		}
		else if (strcmp(name, "group") == 0)
		{
			CString str = child.child_value();
			params.pGroup = _DldsGrps.FindGroupByName(str);
			if (params.pGroup == NULL) params.pGroup = _DldsGrps.FindGroup(GRP_OTHER_ID);
	...
	if (FALSE == _pwndDownloads->CreateDownloadWithDefSettings(dld, strUrl)) return FALSE;
...
void CMainFrame::OnPausealldlds()
{
	...
	if (_TumMgr.getRestrainAll() && _App.View_DontShowPauseAlldldsEnabled() == FALSE)
	{
		...
		if (nRet & 0x00010000) _App.View_DontShowPauseAlldldsEnabled(TRUE);
	}
}
...
void CMainFrame::ShowFdmBalloon(CFdmBalloonContent* pContent, FdmBalloonContentType enCT)
{
	...
	if (m_pFdmBalloonContent) delete m_pFdmBalloonContent;
...
LRESULT CMainFrame::OnShowBalloon(WPARAM wp, LPARAM lp)
{
	if (wp == 0)
	{
		...
		if (info->bClear) m_strTimeoutBallon_Msgs = "";
	...
	else if (wp == 1)
	{
		    ...
		    ::IsWindow(m_pFdmBalloon->m_hWnd))
		{
			...
			for (size_t i = n0; i < pOnDDnow->m_vItems.size(); i++) pOnDD->m_vItems.push_back(pOnDDnow->m_vItems[i]);
		}
...
{
	if (m_bExitHandlerPerformed) return true;

	vmsAUTOLOCKSECTION(m_csExit);

	if (_PluginMgr.IsWasExit()) return true;

	if (bQueryExit && !_PluginMgr.QueryExit()) return false;
