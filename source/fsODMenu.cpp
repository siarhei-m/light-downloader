...
fsODMenuItemData* fsODMenu::AttachMenuItem(CMenu* pMenu, UINT iPos, BOOL bByPos)
{
	...
	if (uState & MF_POPUP) uState = MF_POPUP;
...
void fsODMenu::OnMeasureItem(LPMEASUREITEMSTRUCT pmis)
{
	...
	if (pData->bMenuBar == FALSE)
	{
		...
		if (pmis->itemHeight < mincy) pmis->itemHeight = mincy;

		if (strchr(pData->strMenuText, '\t') != NULL) pmis->itemWidth += 15;
	}
	else
	{
		pmis->itemWidth -= 3;
		...
		if (pmis->itemHeight < (UINT)sz.cy) pmis->itemHeight = sz.cy;
	}
...
void fsODMenu::OnDrawItem(LPDRAWITEMSTRUCT pdis)
{
	...
	if (uState & 0x40)
		if (pData->bMenuBar) fsDrawFrame(dc, &rcSel, 1);
	...
	if (pData->bMenuBar)
	{
		if (uState & ODS_GRAYED || uState & ODS_DISABLED) dc->SetTextColor(GetSysColor(COLOR_GRAYTEXT));
	...
	else
	{
		...
		if (pszTab) dc->DrawText(pszTab + 1, -1, &rcItem, DT_VCENTER | DT_SINGLELINE | DT_RIGHT);
	...
	else if (pData->iImage != -1)
	{
		...
		{
			if (uState & ODS_SELECTED) fsDrawFrame(dc, &rcIconFrame, 1);
...
{
	if (pMenuWhere == NULL) pMenuWhere = CMenu::FromHandle(m_hMenu);
...
{
	if (pMenu == NULL) pMenu = CMenu::FromHandle(m_hMenu);
...
{
	for (UINT i = 0; i < nSize; i++) SetMenuItemText(pMenu, pTexts[i].pszText, pTexts[i].nIDItem, bByPos);
}
