...
BOOL CDownloads_InfoFrame::Create(CWnd* pParent)
{
	...
	if (FALSE == CWnd::Create(AfxRegisterWndClass(0, LoadCursor(NULL, IDC_ARROW), (HBRUSH)(COLOR_3DFACE + 1), NULL),
	                          NULL, WS_CHILD | WS_VISIBLE, rc, pParent, 0x121f))
		return FALSE;
...
{
	if (m_pChild) ResizeChild(cx, cy);
}

void CDownloads_InfoFrame::ResizeChild(int cx, int cy)
{
	if (cx > 5 && cy > 4) m_pChild->MoveWindow(3, 3, cx - 5, cy - 4);
}
