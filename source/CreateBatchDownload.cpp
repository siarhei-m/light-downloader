...
CCreateBatchDownload::CCreateBatchDownload(CWnd* pParent) : CDialog(CCreateBatchDownload::IDD, pParent)
{
...
BOOL CCreateBatchDownload::OnInitDialog()
{
	...
	{
		fsURL url;
		if (url.Crack(pszUrl) == IR_SUCCESS) m_strUrl = pszUrl;
	}

	if (m_strUrl == "") m_strUrl = "http://";
	...
	int i = 0;
	for (i = 0; i < _LastBatchUrls.GetRecordCount(); i++) pUrls->AddString(_LastBatchUrls.GetRecord(i));
	...
	SetDlgItemText(IDC_URL, m_strUrl == "http://url/" ? CString("http://") : m_strUrl);
	...
	if ((az1 < 0 || az2 >= 'Z' - 'A') || (az2 < 0 || az2 > 'Z' - 'A' - 1))
	{
		az1 = 0;
		az2 = 'Z' - 'A' - 1;
	}
	...
	if (m_wndGroups.SelectGroupById(_App.NewDL_GroupId())) m_bGroupChanged = TRUE;
	...
	if (site)
	{
		if (!m_bAuthChanged && strNewUser.IsEmpty())
		{
			if (!site->strUser.IsEmpty())
			{
				...
				if (!site->strPassword.IsEmpty()) strNewPassword = site->strPassword;
			}
		}
...
{
	if (m_wndGroups.CreateNewGroup(m_wndGroups.GetSelectedGroup())) OnChangeGroups();
}
...
{
	if (FALSE == BuildBatchList()) return;
...
{
	if (m_bBLParametersChanged == false) return TRUE;

	GetDlgItemText(IDC_URL, m_strUrl);
	if (FALSE == CheckUrl()) return FALSE;

	m_iAlphaFrom = m_wndA.GetCurSel();
	m_iAlphaTo = m_iAlphaFrom + m_wndZ.GetCurSel() + 1;
	CString strNumbers;
	GetDlgItemText(IDC_NUMBERS, strNumbers);
	if (false == m_bl.Create(m_strUrl, strNumbers, (char)('A' + m_iAlphaFrom), (char)('A' + m_iAlphaTo)))
	{
...
BOOL CCreateBatchDownload::CheckUrl()
{
	...
	if (m_strUrl.Find("(*)") == -1 && m_strUrl.Find("(*A)") == -1 && m_strUrl.Find("(*a)") == -1)
	{
...
void CCreateBatchDownload::OnOK()
{
	...
	if (strSaveAs != "")
	{
		...
		m_blSaveAs.Create(strSaveAs, strNumbers, (char)('A' + m_iAlphaFrom), (char)('A' + m_iAlphaTo));
	...
	if (FALSE == CCreateDownloadDlg::_CheckFolderName(this, IDC_OUTFOLDER)) return;
...
DWORD WINAPI CCreateBatchDownload::_threadCalculateSize(LPVOID lp)
{
	...
	for (size_t i = 0; i < pThis->m_pvDownloads->size() && info->bNeedStop == FALSE; i++)
	{
		...
		if (IR_SUCCESS != dld->pMgr->GetDownloadMgr()->QuerySize()) iFailed++;
...
{
	CString str;
	if (m_wndZ.GetCurSel() != LB_ERR) m_wndZ.GetLBText(m_wndZ.GetCurSel(), str);

	m_wndZ.ResetContent();
	for (int i = m_wndA.GetCurSel() + 1; i <= 'Z' - 'A'; i++)
	{
		CString _str = (char)('A' + i);
		m_wndZ.AddString(_str);
	}

	if (str != "") m_wndZ.SelectString(-1, str);
	if (m_wndZ.GetCurSel() == LB_ERR) m_wndZ.SetCurSel(m_wndZ.GetCount() - 1);
}
...
void CCreateBatchDownload::OnSetnumbers()
{
	...
	if (IDOK == _DlgMgr.DoModal(&dlg)) SetDlgItemText(IDC_NUMBERS, dlg.m_strNumbers);
}
