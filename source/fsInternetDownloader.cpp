...
fsInternetDownloader::fsInternetDownloader() : m_pManagerPersistObject(0)
{
...
fsInternetDownloader::~fsInternetDownloader()
{
	...
	while (m_cThreads) Sleep(10);

	// std::string auto-manages memory, no need for fsDNP_GetByUrl_Free
}

fsDownload_NetworkProperties* fsInternetDownloader::DNP(int nSection)
{
	if (nSection == -1) return &m_dnp;

	return m_vSections[nSection].nMirrorURL == UINT_MAX ? &m_dnp : &m_vMirrs[m_vSections[nSection].nMirrorURL].dnp;
}

fsInternetResult fsInternetDownloader::AddSection(BOOL bQueryCreation)
{
	if (m_bErrDownloading) return IR_ERROR;
...
fsInternetResult fsInternetDownloader::CreateMainSection(UINT uStartFrom, BOOL bQueryCreation)
{
	...
	if (bQueryCreation && Event(DE_QUERYNEWSECTION, UINT_MAX) == FALSE) return IR_S_FALSE;
...
{
	if (m_bNoNewSections || m_bAddingSection) return IR_S_FALSE;

	if (m_vSections.size() - GetDoneSectionCount() >= MAX_NOT_COMPLETED_SECTION_COUNT) return IR_S_FALSE;

	if (GetSSFileSize() == _UI64_MAX) return IR_S_FALSE;
	...
	{
		const fsSectionEx& _sect = m_vSections[i];

		if (_sect.state == SS_DONE) continue;

		UINT uSpeed = _sect.state == SS_DOWNLOADING ? _sect.uSpeed : 0;

		UINT64 uCurrentPosCorrection = _sect.dwCacheLen + uSpeed * 20;

		vmsAUTOLOCKSECTION(m_csSections);
		if (_sect.uCurrent + uCurrentPosCorrection >= _sect.uEnd)
		{
			vmsAUTOLOCKSECTION_UNLOCK(m_csSections);
			continue;
		}

		UINT64 uSectionNotDownloadedSize = _sect.uEnd - (_sect.uCurrent + uCurrentPosCorrection);
		ASSERT(_sect.uCurrent + uCurrentPosCorrection < _sect.uEnd);
		vmsAUTOLOCKSECTION_UNLOCK(m_csSections);
	...
	sect.uStart = sect.uCurrent = m_vSections[iSection].uCurrent + uBiggestSectCurrentPosCorrection + uMaxSection / 2;
...
BOOL fsInternetDownloader::CreateSection(fsSection& sect, BOOL bCreateThread, BOOL bReduceSCN)
{
	...
	{
		if (FALSE == Event(DE_NEEDFILE)) return FALSE;
	}
	...
	if (sect.iSection == 0 && bCreateThread)
	{
		if (FALSE == Event(DE_NEEDFILE_FINALINITIALIZATION)) return FALSE;
	}
...
BOOL fsInternetDownloader::__threadDownload_flushdata(fsSectionEx* sect, LPBYTE pBuffer, DWORD dwRead, LPBYTE& pbCache,
                                                      DWORD& dwCacheSize, DWORD& dwCacheLen)
{
	if (pBuffer == NULL && pbCache == NULL) return TRUE;
	...
	if (pBuffer && pbCache)
	{
		...
		{
			if (FALSE == __threadDownload_flushdata(sect, NULL, 0, pbCache, dwCacheSize, dwCacheLen)) return FALSE;
	...
	if (pBuffer == NULL || dwCacheSize - dwCacheLen < dwRead)
	{
		...
		if (!fsSetFilePointer(sect->pThis->m_hOutFile, sect->uDCurrent, FILE_BEGIN)) return FALSE;
		...
		if (pbCache == NULL && dwNewCacheSize)
		{
			try
			{
				...
				if (pbCache) dwCacheSize = dwNewCacheSize;
			}
...
#define CURRENT_DW_POS (sect->uDCurrent + sect->dwCacheLen)
	...
	while (sect->uEnd > CURRENT_DL_POS)
	{
		...
		if (dwRead >= sect->uEnd - CURRENT_DL_POS)
		{
			...
			EnterCriticalSection(&sect->pThis->m_csSections);
			if (dnp->enProtocol == NP_FTP && sect->file->FtpGetTransferType() == FTT_ASCII &&
			    sect->uEnd == sect->file->GetFileSize())
			{
				sect->uEnd += 100;
				sect->pThis->setDirty();
			}
		...
		if (dwRead == 0 && sect->lastErr == IR_SUCCESS)
		{
			...
			if (cZeroReads++ > 100)
			{
				if (sect->uEnd == _UI64_MAX ||
				    (sect->uEnd >= sect->file->GetFileSize() && sect->file->FtpGetTransferType() == FTT_ASCII &&
				     dnp->enProtocol == NP_FTP))
				{
					EnterCriticalSection(&sect->pThis->m_csSections);
		...
		if (sect->lastErr != IR_SUCCESS)
		{
			...
			CHECK_NEED_STOP;
			sect->state = SS_ERROR;
			if (sect->file) sect->pThis->Event(DE_ERROROCCURED, sect->iSection);
			...
			do
			{
				...
				sect->lastErr =
				    sect->pThis->OpenUrl(CURRENT_DL_POS, &sect->file, sect->iSection, sect->nMirrorURL, TRUE);
				sect->pThis->setDirty();
		...
		BOOL bWriteOk = __threadDownload_flushdata(sect, pBuffer, dwRead, pbCache, dwCacheSize, sect->dwCacheLen);
	...
	if (sect->uDCurrent > sect->uDEnd) sect->uDCurrent = sect->uDEnd;
	LeaveCriticalSection(&sect->pThis->m_csSections);
	TSECT(sect);
	sect->state = SS_DONE;
	sect->pThis->setDirty();
...
_exit:
	...
	if (pbCache && sect->dwCacheLen) __threadDownload_flushdata(sect, NULL, 0, pbCache, dwCacheSize, sect->dwCacheLen);
	if (sect->dwCacheLen)
	{
		EnterCriticalSection(&sect->pThis->m_csSections);
	...
	{
	case SS_DONE:
		if (sect->nMirrorURL != UINT_MAX) sect->pThis->m_vMirrs[sect->nMirrorURL].bIsGood = TRUE;
		EnterCriticalSection(&sect->pThis->m_csSections);
...
fsInternetResult fsInternetDownloader::OpenUrl(UINT64 uStartPos, fsInternetURLFile** ppFile, int iSectIndex,
                                               UINT& nMirror, BOOL bCheckFileSize, LPCSTR pszContentTypeReq)
{
	return OpenUrl_imp(uStartPos, ppFile, iSectIndex, nMirror, bCheckFileSize, 0, pszContentTypeReq, NULL);
}

fsInternetResult fsInternetDownloader::OpenUrl_imp(UINT64 uStartPos, fsInternetURLFile** ppFile, int iSectIndex,
                                                   UINT& nMirror, BOOL bCheckFileSize, int iAttempt, LPCSTR,
                                                   fsDownload_NetworkProperties* pDNPRedirectedUrl, bool bIsRedirected)
{
	fsInternetResult ir;
	...
	*ppFile = NULL;
	...
	if (iAttempt > 15) return IR_ERROR;

	vmsAUTOLOCKSECTION(m_csOpenUrl);
	if (m_bNeedStop) return IR_S_FALSE;
	...
	// TEST
	if (dnp->enAccType == IATE_SOCKS5PROXY)
	{
		CString strSocksProxy = _App.SocksProxy_Name();
		strcpy_s(szProxy, sizeof(szProxy), "socks=");
		strcat_s(szProxy, sizeof(szProxy), strSocksProxy); // socks=localhost:5150
	}
	else
		vmsMakeWinInetProxy(dnp->strProxyName.c_str(), dnp->enProtocol, dnp->enProtocol, szProxy);

	ir = pSession->Create(dnp->strAgent.c_str(), dnp->enAccType, szProxy, dnp->enProtocol);
	if (ir != IR_SUCCESS)
	{
		delete pSession;
	...
	{
		int posPath = dnp->strPathName.length() - 1;
		int posExt = 0;
		LPSTR pszExt = new char[MY_MAX_PATH];

		while (posPath && dnp->strPathName[posPath] != '.') pszExt[posExt++] = dnp->strPathName[posPath--];
		...
		if (posPath)
		{
			...
			int i = 0;
			for (i = 0; i < posExt; i++) pszExtension[i] = pszExt[posExt - i - 1];

			pszExtension[i] = 0;

			if (IsExtInExtsStr(dnp->strASCIIExts.c_str(), pszExtension))
	...
	{
		if (m_bNeedStop || IsDone()) TerminateThread(hOpen, IR_TIMEOUT);
	}
	...
	{
		if (m_pOpeningFile->GetFileSize() != m_uSSFileSize) ir = IR_BADFILESIZE;
	}
	...
	if (ir != IR_SUCCESS)
	{
		if (ir == IR_NEEDREDIRECT)
		{
			...
			if (0 == (m_dnp.dwFlags & DNPF_DONT_UPDATE_ORIGINAL_URL_AFTER_REDIRECT))
			{
				...
				{
					LPCSTR pszServerName = DNP(i)->strServerName.c_str();
					if (fsIsServersEqual(pszServerName, dnp->strServerName.c_str(), FALSE))
					{
			...
			if (url.Crack(pszUrlTo) != IR_SUCCESS)
			{
				...
				if (*pszUrlTo == 0)
					strcpy_s(pszUrlPath, 10000, "/");
				else if (*pszUrlTo != '/' && *pszUrlTo != '\\')
				{
					fsPathFromUrlPath(dnp->strPathName.c_str(), dnp->enProtocol == NP_FTP, FALSE, pszUrlPath, 10000);

					if (pszUrlPath[lstrlen(pszUrlPath) - 1] != '\\' && pszUrlPath[lstrlen(pszUrlPath) - 1] != '/')
						lstrcat(pszUrlPath, "\\");

					strcat_s(pszUrlPath, 10000, pszUrlTo);
				}
				else
				{
					strcpy_s(pszUrlPath, 10000, pszUrlTo);
				}

				url.Create(fsNPToScheme(dnp->enProtocol), dnp->strServerName.c_str(), dnp->uServerPort, dnp->strUserName.c_str(),
				           dnp->strPassword.c_str(), pszUrlPath, pszUrl, &dwLen);

				delete[] pszUrlPath;
			}
			else
			{
				strcpy_s(pszUrl, 10000, pszUrlTo);
			}
			...
			if (!dnp->strUserName.empty())
				strcpy_s(pszUser, 10000, dnp->strUserName.c_str());
			else
				*pszUser = 0;

			if (!dnp->strPassword.empty())
				strcpy_s(pszPassword, 10000, dnp->strPassword.c_str());
			else
				*pszPassword = 0;
			...
			if (dnpRedirectedUrl) delete dnpRedirectedUrl;
	...
	*ppFile = m_pOpeningFile;
	...
	if (m_strContentType.Length() == 0)
	{
		...
		if ((*ppFile)->GetContentType(szContType)) m_strContentType = szContType;
	}
...
{
	if (iSection >= m_vSections.size()) return FALSE;
...
void fsInternetDownloader::StopDownloading()
{
	...
	try
	{
		for (int i = 0; i < m_vSections.size(); i++)
		{
			...
			try
			{
				if (sect->file) sect->file->CloseHandle();
			}
			catch (const std::exception& ex)
			{
				ASSERT(FALSE);
...
{
	if (dwVer < 5) return RestoreSectionsState_vlt5(pBuffer, dwSize, dwVer);

	if (dwVer == 5) return RestoreSectionsState_v5(pBuffer, dwSize);

	if (pBuffer == NULL || m_vSections.size()) return FALSE;
	...
	for (int i = 0; i < cSects; i++)
	{
		...
		if (pSects->iSection != i) return FALSE;
		...
		sect->file = NULL;
		if (sect->state != SS_DONE) sect->state = SS_STOPPED;
	...
	if (iStrLen > MY_MAX_PATH || iStrLen < 0) return FALSE;
	...
	if (m_vSections.size() == 1 && m_vSections[0].uDEnd == _UI64_MAX && m_uLDFileSize != _UI64_MAX)
...
{
	if (dwVer < 4) return RestoreSectionsState_vlt4(pBuffer, dwSize);
	...
	m_uSSFileSize = 0;
	while (cSects--) m_uSSFileSize = max(m_uSSFileSize, m_vSections[cSects].uEnd);
...
{
	if (pBuffer == NULL || dwSize % sizeof(fsSection_v1) || m_vSections.size()) return FALSE;
...
fsInternetResult fsInternetDownloader::StartDownloading(UINT uStartFrom)
{
	...
	if (size == 0) return CreateMainSection(uStartFrom);

	if (IsRunning() || IsDone()) return IR_S_FALSE;
	...
	if (nSC) m_dwForceCacheSizePerSection = _App.FileWriteCacheSize() / nSC;

	for (int i = 0; i < size && m_bNeedStop == FALSE; i++)
	{
		fsSection& sect = m_vSections[i];

		if (sect.state == SS_DONE) continue;

		if (bQS)
		{
			if (Event(DE_QUERYNEWSECTION, sect.nMirrorURL) == FALSE) continue;
		}
		else
			bQS = TRUE;
		...
		if (sect.lastErr == IR_SUCCESS)
		{
			...
			if (Event(DE_NEEDFILE) == FALSE || FALSE == Event(DE_NEEDFILE_FINALINITIALIZATION)) return IR_S_FALSE;
		}
			...
			if (m_uSSFileSize != uNewSize)
			{
				...
				case SCR_ADJUSTFORNEWSIZE:
					...
					if (m_uSSFileSize > uNewSize)
					{
						...
						{
							fsSection& _sect = m_vSections[j];

							if (_sect.uEnd > uNewSize)
							{
								EnterCriticalSection(&m_csSections);
								_sect.uEnd = _sect.uDEnd = uNewSize;
								if (_sect.uCurrent > _sect.uEnd)
								{
									_sect.uCurrent = _sect.uDCurrent = _sect.uEnd;
									_sect.state = SS_DONE;
								}
								setDirty();
								LeaveCriticalSection(&m_csSections);
						...
						for (int j = 0; j < size; j++)
						{
							if (m_vSections[j].uEnd == m_uSSFileSize)
							{
								m_vSections[j].uEnd = uNewSize;
								setDirty();
								break;
							}
						}
		...
		if (sect.file == NULL && bAtLeast1Started == FALSE) return sect.lastErr;
...
BOOL fsInternetDownloader::IsDownloading()
{
	for (int i = 0; i < m_vSections.size(); i++)
	{
		if (m_vSections[i].state == SS_DOWNLOADING) return TRUE;
	}
	return FALSE;
}
...
BOOL fsInternetDownloader::IsDone()
{
	...
	for (int i = 0; i < size; i++)
	{
		if (m_vSections[i].state != SS_DONE) return FALSE;
	}
...
BOOL fsInternetDownloader::IsAllSectionsOk()
{
	...
	for (int i = 0; i < cSections; i++)
	{
		...
		if (state != SS_DOWNLOADING && state != SS_DONE) return FALSE;
	}
...
int fsInternetDownloader::GetDownloadingSectionCount()
{
	...
	for (int i = 0; i < cSections; i++)
		if (m_vSections[i].state == SS_DOWNLOADING) cDlding++;
...
{
	if (dnp->strReferer.empty() && (dnp->enProtocol == NP_HTTP || dnp->enProtocol == NP_HTTPS))
	{
		char szUrl[10000];
		char szRefUrl[10000];
		fsURL url;
		DWORD dw = sizeof(szUrl);
		if (!dnp->strPathName.empty() && strcmp(dnp->strPathName.c_str(), "/") && strcmp(dnp->strPathName.c_str(), "\\") &&
		    IR_SUCCESS == url.Create(fsNPToScheme(dnp->enProtocol), dnp->strServerName.c_str(), dnp->uServerPort, NULL, NULL,
		                             dnp->strPathName.c_str(), szUrl, &dw))
		{
			if (fsFilePathFromUrlPath(szUrl, dnp->enProtocol == NP_FTP, FALSE, szRefUrl, sizeof(szRefUrl)))
			{
				LPSTR psz1 = strrchr(szRefUrl, '\\');
				LPSTR psz2 = strrchr(szRefUrl, '/');
				if (psz2 > psz1) psz1 = psz2;
		...
		pFile->SetReferer(dnp->strReferer.c_str());

	pFile->UseHttp11(dnp->bUseHttp11);
	pFile->UseCookie(dnp->bUseCookie);
	pFile->SetCookies(dnp->strCookies.c_str());
	pFile->SetPostData(dnp->strPostData.c_str());
...
int fsInternetDownloader::GetDoneSectionCount()
{
	...
	for (int i = 0; i < cSections; i++)
		if (m_vSections[i].state == SS_DONE) cDlding++;
...
UINT64 fsInternetDownloader::GetBytesLeft()
{
	try
	{
		...
		for (int i = cSections - 1; i >= 0; i--)
		{
			...
			if (sect->uDEnd == _UI64_MAX) return _UI64_MAX;

			if (sect->uDEnd > sect->uDCurrent) uLeft += sect->uDEnd - sect->uDCurrent;
		}
...
float fsInternetDownloader::GetPercentDone()
{
	...
	if (uSize != _UI64_MAX) uDone = uSize - uBytesLeft;
...
{
	if (m_cBaseSectCreatingNow < 0) m_cBaseSectCreatingNow = 0;
...
fsInternetResult fsInternetDownloader::QuerySize()
{
	...
	if (m_pOpeningFile) return IR_S_FALSE;
...
fsInternetResult fsInternetDownloader::QuerySize(fsInternetURLFile* file)
{
	...
	ir = file->QuerySize(fsNPToScheme(DNP()->enProtocol), DNP()->strServerName.c_str(), DNP()->strUserName.c_str(), DNP()->strPassword.c_str(),
	                     DNP()->uServerPort, DNP()->strPathName.c_str(), DNP()->dwFlags & DNPF_IMMEDIATELY_SEND_AUTH_AS_BASIC);

	if (ir != IR_SUCCESS)
	{
		if (ir == IR_NEEDREDIRECT)
		{
			if (m_vSections.size()) return IR_SERVERUNKERROR;
			...
			if (url.Crack(pszUrl) != IR_SUCCESS)
			{
				...
				if (*pszUrl == 0)
					strcpy_s(szUrlPath, sizeof(szUrlPath), "/");
				else if (*pszUrl != '/' && *pszUrl != '\\')
				{
					fsPathFromUrlPath(DNP()->strPathName.c_str(), DNP()->enProtocol == NP_FTP, FALSE, szUrlPath, 10000);

					if (szUrlPath[lstrlen(szUrlPath) - 1] != '\\' && szUrlPath[lstrlen(szUrlPath) - 1] != '/')
						lstrcat(szUrlPath, "\\");

					strcat_s(szUrlPath, sizeof(szUrlPath), pszUrl);
				}
				else
					strcpy_s(szUrlPath, sizeof(szUrlPath), pszUrl);

				url.Create(fsNPToScheme(DNP()->enProtocol), DNP()->strServerName.c_str(), DNP()->uServerPort,
				           DNP()->strUserName.c_str(), DNP()->strPassword.c_str(), szUrlPath, szUrl, &dwLen);
			}
			else
				strcpy_s(szUrl, sizeof(szUrl), pszUrl);
...
int fsInternetDownloader::GetStoppedSectionCount()
{
	...
	for (int i = 0; i < m_vSections.size(); i++)
		if (m_vSections[i].state == SS_STOPPED) cStopped++;
...
fsInternetResult fsInternetDownloader::AddMirrorURL(LPCSTR pszUrl, LPCSTR pszUser, LPCSTR pszPassword,
                                                    BOOL bDontMeasureSpeed)
{
	fsInternetResult ir;
	...
	if (pszUser)
	{
		...
		{
			size_t len = pszPassword ? strlen(pszPassword) + 1 : 1;
			dnp.strPassword = pszPassword ? pszPassword : "";
		}
	}
...
{
	if (m_vMirrs.size() == 0) return UINT_MAX;
	...
	int i = 0;
	for (i = 0; i < cMirrs; i++) pfWeight[i] = 1;

	for (i = 0; i < m_vSections.size(); i++)
	{
		if (m_vSections[i].nMirrorURL != UINT_MAX) pfWeight[m_vSections[i].nMirrorURL]++;
	}

	for (i = 0; i < cMirrs; i++)
	{
		DWORD dwPingTime = m_vMirrs[i].dwPingTime;
		if (dwPingTime == UINT_MAX) dwPingTime = 50000;
		pfWeight[i] = 1.0 / (pfWeight[i] * pow(double(dwPingTime) / 1000.0f, 2));
	}
...
BOOL fsInternetDownloader::MeasureMirrorsSpeed()
{
	...
	m_dwBaseServerPingTime = pinger.Ping(DNP()->strServerName.c_str());
	...
	for (int i = 0; i < m_vMirrs.size(); i++)
	{
		...
		m_vMirrs[i].dwPingTime = pinger.Ping(m_vMirrs[i].dnp.strServerName.c_str());
		setDirty();
	}
...
int fsInternetDownloader::GetMirrorRefs(int iMirrorIndex)
{
	int cRefs = 0;
	for (int i = 0; i < m_vSections.size(); i++)
		if (m_vSections[i].nMirrorURL == UINT(iMirrorIndex)) cRefs++;
...
{
	if (GetCreatingNowSectionCount(UINT_MAX)) return TRUE;
	for (int i = 0; i < m_vMirrs.size(); i++)
	{
		if (GetCreatingNowSectionCount(i)) return TRUE;
	}
	return FALSE;
}
...
void fsInternetDownloader::AddMirror(fsDownload_NetworkProperties* dnp, BOOL bIsGood, BOOL bDontMeasureSpeed)
{
	...
	{
		fsSitePingMgr pinger;
		if (pinger.TestState()) mirr.dwPingTime = pinger.Ping(dnp->strServerName.c_str());
	}

	mirr.dnp = *dnp;
	mirr.bIsGood = bIsGood;

	if (dnp->enProtocol == NP_FILE) m_dwState |= IDS_MIRRORS_HAS_LOCAL_SOURCE;
...
UINT64 fsInternetDownloader::GetDownloadedBytesCount()
{
	...
	if (size != _UI64_MAX) return size - GetBytesLeft();
...
{
	pSession->SetProxyAuth(dnp->strProxyUserName.c_str(), dnp->strProxyPassword.c_str());
...
BOOL fsInternetDownloader::RemoveMirror(int iIndex)
{
	...
	for (int i = 0; i < m_vSections.size(); i++)
	{
		...
		else if (m_vSections[i].nMirrorURL != UINT_MAX && m_vSections[i].nMirrorURL > (UINT)iIndex)
		{
...
fsString fsInternetDownloader::get_URL(BOOL bIncludeAuthInfo)
{
	...
	if (IR_SUCCESS != url.Create(fsNPToScheme(DNP()->enProtocol), DNP()->strServerName.c_str(), DNP()->uServerPort,
	                             bIncludeAuthInfo ? DNP()->strUserName.c_str() : "", bIncludeAuthInfo ? DNP()->strPassword.c_str() : "",
	                             DNP()->strPathName.c_str(), szUrl, &dw))
	{
		return "";
	}
...
DWORD WINAPI fsInternetDownloader::_threadOpenUrl(LPVOID lp)
{
	...
	try
	{
		...
		dw = p->pFile->Open(fsNPToScheme(p->dnp->enProtocol), p->dnp->strServerName.c_str(), p->dnp->strUserName.c_str(),
		                    p->dnp->strPassword.c_str(), p->dnp->uServerPort, p->dnp->strPathName.c_str(), p->uStartPos,
		                    p->dnp->dwFlags & DNPF_IMMEDIATELY_SEND_AUTH_AS_BASIC);
	}
	catch (const std::exception& ex)
	{
		ASSERT(FALSE);
...
{
	if (m_strSuggFileName.Length() < 13) return;

	if (strnicmp(m_strSuggFileName, "=?UTF-8?", 8)) return;

	LPCSTR psz = &m_strSuggFileName[8];
	if (*psz != 'B' && *psz != 'b') return;
	if (psz[1] != '?') return;

	psz += 2;

	fsString str;
	while (psz[1] && (*psz != '?' || psz[1] != '=')) str += *psz++;
