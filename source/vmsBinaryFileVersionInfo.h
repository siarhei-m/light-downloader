...
class vmsBinaryFileVersionInfo
{
  public:
	BOOL Retrieve(LPCTSTR pszFile)
	{
		...
		if (FALSE == GetFileVersionInfo(pszFile, 0, dwSize, pvVer)) return FALSE;
		return ExtractVersionInfo(pvVer, dwSize);
	}

	vmsBinaryFileVersionInfo(LPCTSTR ptszFile = NULL)
	{
		if (ptszFile) Retrieve(ptszFile);
	}
  ...
  protected:
	BOOL ExtractVersionInfo(LPVOID pvVer, DWORD dwSize)
	{
		...
		LPCTSTR atszValueName[] = {
		    _T ("ProductVersion"), _T ("ProductName"),  _T ("FileDescription"), _T ("CompanyName"),
		    _T ("FileVersion"),    _T ("InternalName"), _T ("LegalCopyright"),  _T ("OriginalFilename"),
		};
		...
		tstring* atstrValues[] = {&m_tstrProductVersion, &m_tstrProductName,     &m_tstrFileDescription,
		                          &m_tstrCompanyName,    &m_tstrFileVersion,     &m_tstrInternalName,
		                          &m_tstrLegalCopyright, &m_tstrOriginalFileName};
		...
		else
		{
			...
			{
				if (m_tstrProductVersion[i] == ',') m_tstrProductVersion[i] = '.';
