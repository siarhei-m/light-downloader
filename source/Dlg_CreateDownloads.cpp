...
int CALLBACK _SortFunc(LPARAM item1, LPARAM item2, LPARAM)
{
...
CCreateDownloadsDlg::CCreateDownloadsDlg(CWnd* pParent) : CDialog(CCreateDownloadsDlg::IDD, pParent)
{
...
void CCreateDownloadsDlg::AddToList(fs::tree<fsFileInfo*>* pTree, HTREEITEM hWhere)
{
	...
	while (pCur)
	{
		...
		HTREEITEM hItem =
		    m_wndFiles.InsertItem(TVIF_TEXT | TVIF_PARAM, file->strName, 0, 0, 0, 0, (LPARAM)file, hWhere, TVI_LAST);
		m_wndFiles.SetCheck(hItem);

		if (pCur->Right()) AddToList(pCur->Right(), hItem);
...
{
	if (m_wndGroups.CreateNewGroup(m_wndGroups.GetSelectedGroup())) OnSelchangeGroups();
}
...
void CCreateDownloadsDlg::BuildDownloads(fs::tree<fsFileInfo*>* pTree, LPCSTR pszRootUrl)
{
	...
	while (pCur)
	{
		...
		if (pCur->Right())
		{
			if (strUrl[strUrl.GetLength() - 1] != '/') strUrl += '/';
...
void CCreateDownloadsDlg::OnOK()
{
	...
	if (strFolder[strFolder.GetLength() - 1] != '\\' && strFolder[strFolder.GetLength() - 1] != '/') strFolder += '\\';
...
void CCreateDownloadsDlg::CreateDownloads(HTREEITEM hTree, LPCSTR pszRootDir, BOOL bCreate)
{
	...
	while (hCur)
	{
		...
		if (m_wndFiles.ItemHasChildren(hCur))
		{
			...
			if (strDir[strDir.GetLength() - 1] != '\\' || strDir[strDir.GetLength() - 1] != '/') strDir += '\\';
