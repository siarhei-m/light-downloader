...
CDlg_Download::CDlg_Download(CWnd* pParent) : CDialog(CDlg_Download::IDD, pParent)
{
...
void CDlg_Download::ApplyLanguage()
{
	fsDlgLngInfo lnginfo[] = {
	    ...
	    fsDlgLngInfo(IDC__RESUMESUPPORT_, L_RESUMESUPPORT, TRUE),
...
void CDlg_Download::OnTimer(UINT_PTR nIDEvent)
{
	if (nIDEvent == 2)
	{
		KillTimer(2);
...
void CDlg_Download::Update()
{
	...
	SetDlgItemText2(IDC_URL, m_dld->pMgr->get_URL());
...
void CDlg_Download::SetDlgItemText2(UINT nID, LPCSTR pszText)
{
	...
	if (str != pszText) SetDlgItemText(nID, pszText);
}
...
void CDlg_Download::OnHide()
{
	...
	if (_App.DownloadDialog_DontAskOnHide() == FALSE && m_dld->pMgr->IsDone() == FALSE && m_dld->pMgr->IsRunning())
	{
		CDlg_Download_OnHide dlg(this);

		if (_DlgMgr.DoModal(&dlg) == IDCANCEL) return;

		if (!_pwndDownloads || !IsWindow(hwndThis)) return;

		switch (dlg.m_enResult)
		{
		case DDOR_SHOW:
			if (dlg.m_bDontAskAgain) _App.DownloadDialog_DontAskOnHide(TRUE);
			break;
