...
CString BytesToString(UINT64 uSize, bool bForceXbytes)
{
	...
	if (bForceXbytes || (_pwndDownloads != NULL && FALSE == _pwndDownloads->IsSizesInBytes()))
	{
		float val;
		char szDim[10];
...
{
	if (pszDate) GetDateFormat(LOCALE_USER_DEFAULT, DATE_SHORTDATE, time, NULL, pszDate, 100);
...
BOOL fsErrorToStr(LPSTR pszErr, DWORD dwMaxSize, DWORD* pdwLastError)
{
	...
	*pszErr = 0;
	...
	if (strlen(pszDesc) >= dwMaxSize) return FALSE;
...
void fsGetPath(LPCSTR pszFile, LPSTR pszPath)
{
	...
	while (len >= 0 && pszPath[len] != '\\' && pszPath[len] != '/') len--;
...
BOOL fsBuildPathToFileW(LPCWSTR pwszFileName)
{
	...
	for (;;)
	{
		pwsz = wcschr(pwsz + 1, '\\');
		if (pwsz == NULL) break;
		if (pwsz - pwszFileName == 2) continue;
...
BOOL DPEntry_IsAllEqual(DLDS_LIST* pv, int offset, int size, BOOL bString)
{
	...
	for (int i = 1; i < cDlds; i++)
	{
		...
		{
			if (memcmp(dp0, dpn, size)) return FALSE;
		}
	}
...
BOOL DPEntry_IsAllEqual_BitMask(DLDS_LIST* pv, int offset, DWORD dwBitMask)
{
	...
	for (int i = 1; i < cDlds; i++)
	{
		...
		if ((*dp0 & dwBitMask) != (*dpn & dwBitMask)) return FALSE;
	}
...
BOOL DNPEntry_IsAllEqual_BitMask(DLDS_LIST* pv, int offset, DWORD dwBitMask)
{
	...
	for (int i = 1; i < cDlds; i++)
	{
		...
		if ((*dnp0 & dwBitMask) != (*dnpn & dwBitMask)) return FALSE;
	}
...
{
	if (*pszWhere == 0) return *pszWhat == 0 ? pszWhere : NULL;

	if (*pszWhat == 0) return NULL;

	if (*pszWhat == '*')
	{

		if (pszWhat[1] == 0) return pszWhere;
	...
	if (*pszWhat != '?')
	{
		...
		if (*pszWhere != *pszWhat) return NULL;
	}
...
LPCSTR strcmpi_m(LPCSTR pszWhere, LPCSTR pszWhat)
{
	...
	if (psz) pszRet = pszWhere + (psz - psz1);
...
BOOL IsExtInExtsStr(LPCSTR pszExts, LPCSTR pszExt)
{
	...
	do
	{
		...
		while (i < len && pszExts[i] != ' ') szExt[j++] = pszExts[i++];

		szExt[j] = 0;
		i++;

		if (IsExtStrEq(szExt, pszExt)) return TRUE;
...
BOOL fsSaveStrToFile(LPCSTR pszStr, HANDLE hFile)
{
	...
	if (!WriteFile(hFile, &len, sizeof(len), &dw, NULL)) return FALSE;

	if (len != -1) return WriteFile(hFile, pszStr, len, &dw, NULL);
...
BOOL fsReadStrFromFile(LPSTR* ppszStr, HANDLE hFile)
{
	...
	if (!ReadFile(hFile, &len, sizeof(len), &dw, NULL) || dw != sizeof(len)) return FALSE;

	if (len != -1)
	{
		if (UINT(len) > 100000) return FALSE;

		fsnew(*ppszStr, char, len + 1);
		if (!ReadFile(hFile, *ppszStr, len, &dw, NULL)) return FALSE;

		if (len != int(dw)) return FALSE;
...
CString fsTimeInSecondsToStr(DWORD dwAmount)
{
	...
	strResult.Format(_T ("%02d:%02d:%02d"), (int)(dwAmount / 3600), (int)((dwAmount % 3600) / 60),
	                 (int)(dwAmount % 60));
...
void fsPathToGoodPath(LPSTR pszPath)
{
	...
	while (pszBad = strchr(pszBad, '/')) *pszBad = '\\';
}
...
// CString
fsString fsGetGrpOTHEROutFolder()
{
...
void vmsUtf8ToAscii(LPSTR psz)
{
	...
	wchar_t* pwsz = new wchar_t[nRL + 1];
	*pwsz = 0;
