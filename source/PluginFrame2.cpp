...
BOOL CPluginFrame2::Create(CWnd* pParent)
{
	...
	if (FALSE == CWnd::Create(AfxRegisterWndClass(0, NULL, (HBRUSH)(COLOR_BTNFACE + 1), NULL), NULL,
	                          WS_CHILD | WS_VISIBLE, rc, pParent, 0x111))
		return FALSE;

	if (FALSE == m_wndTabs.Create(this)) return FALSE;

	if (FALSE == Initialize()) return FALSE;
...
{
	if (m_iActivePlugin == iPlugin) return;

	// int cnt = _PluginMgr.GetPluginCount();
	// if (iPlugin == cnt)
	//{
	...
	m_pMainFrame->AddPluginMenuView(plug.hMenuView, plug.pMenuViewImages, plug.cMenuViewImages, plug.pMenuViewItems,
	                                plug.cMenuViewItems);
...
void CPluginFrame2::OnNewLng()
{
	...
	for (i = 0; i < _PluginMgr.GetPluginCount(); i++)
	{
		...
		{
			m_pMainFrame->AddPluginMenu(plug->hMenuMain, plug->pMenuMainImages, plug->cMenuMainImages);
			m_pMainFrame->AddPluginMenuView(plug->hMenuView, plug->pMenuViewImages, plug->cMenuViewImages,
			                                plug->pMenuViewItems, plug->cMenuViewItems);
		}
	}
...
BOOL CPluginFrame2::Initialize()
{
	...
	if (FALSE == _PluginMgr.LoadPlugins(this)) return FALSE;
	...
	if (iPlug >= _PluginMgr.GetPluginCount() || iPlug < 0) iPlug = 0;
