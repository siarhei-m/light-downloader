...
BOOL CSchedule_WTSPage::OnInitDialog()
{
	...
	for (int i = 0; i < 9; i++) ShowGroup(i, FALSE);
...
void CSchedule_WTSPage::ShowGroup(int iGrp, BOOL bShow, BOOL bEnabled)
{
	...
	for (UINT i = 0; i < ppGrpID[iGrp][0]; i++)
	{
		...
		if (bEnabled == FALSE) pWnd->EnableWindow(FALSE);
	}
}
...
void CSchedule_WTSPage::OnSelchangeWts()
{
	...
	case WTS_RESTRAINALLDLDS:
		...
		{
			CheckRadioButton(IDC_RAD_ENABLE, IDC_RAD_INVERT, IDC_RAD_ENABLE + m_task->wts.enRAD);
		}
		break;
	}
...
void CSchedule_WTSPage::OnChoosename()
{
	...
	CFileDialog dlg(TRUE, NULL, NULL, OFN_HIDEREADONLY | OFN_FILEMUSTEXIST | OFN_NOCHANGEDIR, strFilter, this);
...
BOOL CSchedule_WTSPage::OnApply()
{
	...
	{
	case WTS_PROGRAM:
		if (FALSE == ApplyProg()) return FALSE;
		break;

	case WTS_STARTDOWNLOAD:
	case WTS_STOPDOWNLOAD:
		if (FALSE == ApplyStartStopDownloads()) return FALSE;
	...
	if (pschScheduleParam->m_ppoTaskWrapper) pschScheduleParam->m_ppoTaskWrapper->setDirty();
...
BOOL CSchedule_WTSPage::ApplyStartStopDownloads()
{
	...
	for (int i = 0; i < cItems; i++) m_task->wts.dlds.pvIDs->add(m_wndSelDlds.GetItemData(i));
...
void CSchedule_WTSPage::ApplyLanguage()
{
	fsDlgLngInfo lnginfo[] = {
	    fsDlgLngInfo(IDC__PROGNAME, L_PRGNAME),  fsDlgLngInfo(IDC__ARGUMENTS, L_ARGS),
	    fsDlgLngInfo(IDC__TYPE, L_SHUTDOWNTYPE), fsDlgLngInfo(IDC_FORCEEXIT, L_FORCEAPPS),
	    fsDlgLngInfo(IDC_RAD_ENABLE, L_ENABLE),  fsDlgLngInfo(IDC_RAD_DISABLE, L_DISABLE),
	    fsDlgLngInfo(IDC_RAD_INVERT, L_SWITCH),  fsDlgLngInfo(IDC_RESTARTDLDIFDONE, L_RESTARTCOMPLDLDS),
	};

	CString str = LS(L_CHANGETUMTO);
	str += ':';
