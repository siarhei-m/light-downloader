...
bool vmsDownloadsGroup::loadObjectItselfFromStateBuffer(LPBYTE pbtBuffer, LPDWORD pdwSize, DWORD dwVer)
{
	...
	if (glehEventHandler) glehEventHandler(pvData);
...
void vmsDownloadsGroupsMgr::CreateDefaultGroups()
{
	...
	CString strRoot((LPCSTR)grp->strOutFolder);
	grp->strExts = "";
...
BOOL vmsDownloadsGroupsMgr::LoadFromDisk()
{
	...
	fsString strFile = (LPCSTR)theApp.m_strAppPath;
	strFile += "groups.sav";
	...
	if (hFile != INVALID_HANDLE_VALUE)
	{
		...
		std::unique_ptr<BYTE[]> pbtBufferGuard(new BYTE[dwRequiredSize]);
		...
		if (!loadFromStateBuffer(pbtBuffer, &dwRequiredSize, DLDSGRPSFILE_CURRENT_VERSION)) // hdr.wVer
		{
			CloseHandle(hFile);
			return FALSE;
		}
	...
	if (m_tGroups->GetLeafCount() == 0) CreateDefaultGroups();
...
BOOL vmsDownloadsGroupsMgr::SaveToDisk()
{
	...
	HANDLE hFile = CreateFile(strFile, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_HIDDEN, NULL);
	...
	if (dwRequiredSize == 0) return FALSE;

	std::unique_ptr<BYTE[]> apbtBufferGuard(new BYTE[dwRequiredSize]);
	LPBYTE pbtBuffer = apbtBufferGuard.get();
	if (pbtBuffer == 0) return FALSE;
	memset(pbtBuffer, 0, dwRequiredSize);
...
bool vmsDownloadsGroupsMgr::loadObjectItselfFromStateBuffer(LPBYTE pbtBuffer, LPDWORD pdwSize, DWORD dwVer)
{
	...
	if (!getVarFromBuffer(m_nGrpNextId, pbtCurrentPos, pbtBuffer, *pdwSize)) return false;
...
PDLDS_GROUPS_TREE vmsDownloadsGroupsMgr::Add(vmsDownloadsGroupSmartPtr grp, vmsDownloadsGroupSmartPtr pParentGroup,
                                             BOOL bKeepIdAsIs, bool bDontQueryStoringGroupsInformation)
{
	PDLDS_GROUPS_TREE pParent = pParentGroup == NULL ? (PDLDS_GROUPS_TREE)m_tGroups : FindGroupInTree(pParentGroup);
...
vmsDownloadsGroupSmartPtr vmsDownloadsGroupsMgr::FindGroup(UINT nId)
{
	...
	{
		if (m_vGroups[i]->GetData()->nId == nId) return m_vGroups[i]->GetData();
	}
...
vmsDownloadsGroupSmartPtr vmsDownloadsGroupsMgr::FindGroupByExt(LPCSTR pszExt)
{
	for (size_t i = 0; i < m_vGroups.size(); i++)
	{
		if (IsExtInExtsStr(m_vGroups[i]->GetData()->strExts, pszExt)) return m_vGroups[i]->GetData();
	}
...
PDLDS_GROUPS_TREE vmsDownloadsGroupsMgr::FindGroupInTree(vmsDownloadsGroupSmartPtr pGroup)
{
	for (size_t i = 0; i < m_vGroups.size(); i++)
	{
		if (pGroup->nId == m_vGroups[i]->GetData()->nId) return m_vGroups[i];
	}
...
void vmsDownloadsGroupsMgr::SetGroupsRootOutFolder(PDLDS_GROUPS_TREE pRoot, LPCSTR pszFolder)
{
	for (int i = 0; i < pRoot->GetLeafCount(); i++)
	{
		...
		if (str[str.GetLength() - 1] != '\\' && str[str.GetLength() - 1] != '/') str += '\\';
...
BOOL vmsDownloadsGroupsMgr::LoadGroupsTreeFromFile(HANDLE hFile, PDLDS_GROUPS_TREE pRoot)
{
	int cGroups = 0;
	DWORD dw;
	if (FALSE == ReadFile(hFile, &cGroups, sizeof(int), &dw, NULL)) return FALSE;

	while (cGroups-- > 0)
	{
		vmsDownloadsGroupSmartPtr pGroup;
		pGroup.CreateInstance();
		if (FALSE == LoadGroupFromFile(hFile, pGroup)) return FALSE;
		PDLDS_GROUPS_TREE pGroupRoot = Add(pGroup, pRoot, TRUE, true);
		if (FALSE == LoadGroupsTreeFromFile(hFile, pGroupRoot)) return FALSE;
	}

	return TRUE;
}

bool vmsDownloadsGroupsMgr::LoadGroupsTreeFromBuffer(LPBYTE& pbtCurrentPos, LPBYTE pbtBuffer, DWORD dwBufferSize,
                                                     PDLDS_GROUPS_TREE pRoot)
{
	int cGroups = 0;

	if (!getVarFromBuffer(cGroups, pbtCurrentPos, pbtBuffer, dwBufferSize)) return false;

	while (cGroups-- > 0)
	{
		vmsDownloadsGroupSmartPtr pGroup;
		pGroup.CreateInstance();
		if (!LoadGroupFromBuffer(pbtCurrentPos, pbtBuffer, dwBufferSize, pGroup)) return false;
		PDLDS_GROUPS_TREE pGroupRoot = Add(pGroup, pRoot, TRUE, true);
		if (!LoadGroupsTreeFromBuffer(pbtCurrentPos, pbtBuffer, dwBufferSize, pGroupRoot)) return false;
	}
...
BOOL vmsDownloadsGroupsMgr::LoadGroupFromFile(HANDLE hFile, vmsDownloadsGroupSmartPtr pGroup)
{
	...
	if (FALSE == ReadFile(hFile, &pGroup->nId, sizeof(pGroup->nId), &dw, NULL)) return FALSE;

	if (FALSE == fsReadStringFromFile(hFile, pGroup->strName)) return FALSE;

	if (FALSE == fsReadStringFromFile(hFile, pGroup->strOutFolder)) return FALSE;

	if (FALSE == fsReadStringFromFile(hFile, pGroup->strExts)) return FALSE;

	return TRUE;
}

bool vmsDownloadsGroupsMgr::LoadGroupFromBuffer(LPBYTE& pbtCurrentPos, LPBYTE pbtBuffer, DWORD dwBufferSize,
                                                vmsDownloadsGroupSmartPtr pGroup)
{
	if (!getVarFromBuffer(pGroup->nId, pbtCurrentPos, pbtBuffer, dwBufferSize)) return false;

	if (!getStrFromBuffer(&pGroup->strName.pszString, pbtCurrentPos, pbtBuffer, dwBufferSize)) return false;

	if (!getStrFromBuffer(&pGroup->strOutFolder.pszString, pbtCurrentPos, pbtBuffer, dwBufferSize)) return false;

	if (!getStrFromBuffer(&pGroup->strExts.pszString, pbtCurrentPos, pbtBuffer, dwBufferSize)) return false;

	return true;
}

PDLDS_GROUPS_TREE vmsDownloadsGroupsMgr::Add(vmsDownloadsGroupSmartPtr grp, PDLDS_GROUPS_TREE pParentGroup,
                                             BOOL bKeepIdAsIs, bool bDontQueryStoringGroupsInformation)
{
	if (pParentGroup == NULL) pParentGroup = GetGroupsTree();
...
{
	fsString strName;
	while (*pszName && *pszName != '\\' && *pszName != '/') strName += *pszName++;
	if (*pszName) pszName++;
	...
	for (int i = 0; i < pRoot->GetLeafCount(); i++)
	{
		...
		{
			if (*pszName) return FindGroupByName(pszName, pRoot->GetLeaf(i));
...
BOOL vmsDownloadsGroupsMgr::SaveGroupsTreeToFile(HANDLE hFile, PDLDS_GROUPS_TREE pRoot)
{
	...
	DWORD dw;
	if (FALSE == WriteFile(hFile, &cGroups, sizeof(int), &dw, NULL)) return FALSE;

	for (int i = 0; i < cGroups; i++)
	{
		PDLDS_GROUPS_TREE pGroupTree = pRoot->GetLeaf(i);
		if (FALSE == SaveGroupToFile(hFile, pGroupTree->GetData())) return FALSE;
		if (FALSE == SaveGroupsTreeToFile(hFile, pGroupTree)) return FALSE;
	}
...
BOOL vmsDownloadsGroupsMgr::SaveGroupToFile(HANDLE hFile, vmsDownloadsGroupSmartPtr pGroup)
{
	...
	if (FALSE == WriteFile(hFile, &pGroup->nId, sizeof(pGroup->nId), &dw, NULL)) return FALSE;

	if (FALSE == fsSaveStrToFile(pGroup->strName, hFile)) return FALSE;

	if (FALSE == fsSaveStrToFile(pGroup->strOutFolder, hFile)) return FALSE;

	if (FALSE == fsSaveStrToFile(pGroup->strExts, hFile)) return FALSE;

	return TRUE;
}

void vmsDownloadsGroupsMgr::SaveGroupsTreeToBuffer(LPBYTE& pbtCurrentPos, LPBYTE pbtBuffer, DWORD dwBufferSize,
                                                   DWORD* pdwSizeRequired, PDLDS_GROUPS_TREE pRoot)
{
	int cGroups = pRoot->GetLeafCount();
...
void vmsDownloadsGroupsMgr::SaveGroupToBuffer(LPBYTE& pbtCurrentPos, LPBYTE pbtBuffer, DWORD dwBufferSize,
                                              DWORD* pdwSizeRequired, vmsDownloadsGroupSmartPtr pGroup)
{
...
void vmsDownloadsGroupsMgr::UpdateIdForNextGroup(UINT nId)
{
...
{
	std::list<TGroupLoadEventData> l; // = std::list<TGroupLoadEventData>();
	m_lstGroupLoadEventData.swap(l);
...
bool FdmGroupLoadEventHandler(void* pvData)
{
	...
	if (pgtParent == 0) return false;

	if (pdgmGroupsMgr == 0) return false;

	if (bIsGroupLoaded)
	{
		if ((vmsDownloadsGroup*)gledData->pGroupPtr == 0) return false;
	...
	if ((vmsDownloadsGroup*)pgtParent->GetData() == 0) return false;
