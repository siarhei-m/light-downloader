...
{
	if (fsSysGetOsMajorVersion() < 5) return FALSE;
	...
	if (_pfnSetLWA == NULL)
	{
		...
		if (hDll) _pfnSetLWA = (fntSetLayeredWndAttr)GetProcAddress(hDll, _T ("SetLayeredWindowAttributes"));

		if (_pfnSetLWA == NULL) return FALSE;
	}
...
void fsInitRAS()
{
	...
	if (hDll)
	{
		...
		if (_pfnRasHangUp == NULL) _pfnRasHangUp = (fntRasHangUp)GetProcAddress(hDll, _T ("RasHangUp"));
		...
		if (_pfnRasDial == NULL) _pfnRasDial = (fntRasDial)GetProcAddress(hDll, "RasDial");
		...
		if (_pfnRasEnumEntries == NULL) _pfnRasEnumEntries = (fntRasEnumEntries)GetProcAddress(hDll, "RasEnumEntries");
		...
		if (_pfnRasGetStat == NULL && _pfnRasHangUp == NULL && _pfnRasGetErrorString == NULL && _pfnRasDial == NULL &&
		    _pfnRasGetEntryDialParams == NULL && _pfnRasEnumEntries == NULL && _pfnRasEnumConnections == NULL)
		{
			FreeLibrary(hDll);
		}
	}
...
{
	if (fsSysGetOsMajorVersion() < 5) return;

	if (_pfnRasGetStat == NULL)
	{
		fsInitRAS();
		if (_pfnRasGetStat == NULL) return;
	}
...
DWORD fsRasHangUp(HRASCONN hRasConn)
{
	...
	{
		fsInitRAS();
		if (_pfnRasHangUp == NULL) return ERROR_INVALID_HANDLE;
	}
...
DWORD fsRasEnumEntries(LPCTSTR p1, LPCTSTR p2, LPRASENTRYNAME p3, LPDWORD p4, LPDWORD p5)
{
	...
	{
		fsInitRAS();
		if (_pfnRasEnumEntries == NULL) return ERROR_INVALID_HANDLE;
	}
...
DWORD fsRasEnumConnections(LPRASCONN p1, LPDWORD p2, LPDWORD p3)
{
	...
	{
		fsInitRAS();
		if (_pfnRasEnumConnections == NULL) return ERROR_INVALID_HANDLE;
	}
...
DWORD fsRasGetErrorString(UINT p1, LPTSTR p2, DWORD p3)
{
	...
	{
		fsInitRAS();
		if (_pfnRasGetErrorString == NULL) return ERROR_INVALID_HANDLE;
	}
...
DWORD fsRasDial(LPRASDIALEXTENSIONS p1, LPCTSTR p2, LPRASDIALPARAMS p3, DWORD p4, LPVOID p5, LPHRASCONN p6)
{
	...
	{
		fsInitRAS();
		if (_pfnRasDial == NULL) return ERROR_INVALID_HANDLE;
	}
...
DWORD fsRasGetEntryDialParams(LPCTSTR p1, LPRASDIALPARAMS p2, LPBOOL p3)
{
	...
	{
		fsInitRAS();
		if (_pfnRasGetEntryDialParams == NULL) return ERROR_INVALID_HANDLE;
	}
...
void fsOpenUrlInBrowser(LPCSTR pszUrl)
{
	...
	if (strnicmp(pszUrl, "https", 5) == 0)
		strcpy_s(szReg, sizeof(szReg), "https");
	else if (strnicmp(pszUrl, "http", 4) == 0)
		strcpy_s(szReg, sizeof(szReg), "http");
	else
		strcpy_s(szReg, sizeof(szReg), "ftp");

	strcat_s(szReg, sizeof(szReg), "\\shell\\open\\command");

	HKEY hReg;

	if (ERROR_SUCCESS != RegOpenKeyEx(HKEY_CLASSES_ROOT, szReg, 0, KEY_QUERY_VALUE, &hReg)) goto _lErr;
...
void fsOnMemoryError()
{
	if (IDCANCEL ==
	    MessageBox(
	        NULL,
	        "Out of memory! Please close some applications and press Retry or Cancel to exit without saving any data",
	        "Out of memory", MB_ICONEXCLAMATION | MB_RETRYCANCEL))
	{
		_TrayMgr.Remove();
		ExitProcess(0);
	}
...
BOOL fsIsSystemInFullScreenMode()
{
	...
	if (pwnd != NULL && pwnd->m_hWnd != GetDesktopWindow())
	{
		...
		if (rc.Width() == GetSystemMetrics(SM_CXSCREEN) && rc.Height() == GetSystemMetrics(SM_CYSCREEN))
		{
			CString str;
			...
			str = sz;
			if (str.CompareNoCase("progman")) return TRUE;
		}
	}
...
BOOL vmsSetSuspendState(BOOL bHibernate, BOOL bForce, BOOL bDisableWakeEvent)
{
	...
	if (_pfnSSS == NULL)
	{
		...
		if (hDll) _pfnSSS = (fntSetSuspendState)GetProcAddress(hDll, "SetSuspendState");
	}

	if (_pfnSSS == NULL) return FALSE;
...
LONG fsCopyKey(HKEY hSrcParent, HKEY hTargParent, LPCTSTR szSrcKey, LPCTSTR szTargKey)
{
	...
	__try
	{
		...
		RegQueryInfoKey(hSrcKey, NULL, NULL, NULL, NULL, &nMaxKeyNameLen, NULL, NULL, &nMaxValueNameLen, &nMaxValueLen,
		                NULL, NULL);
		...
		{
			LONG _nRes = fsCopyKey(hSrcKey, hTargKey, szSubKeyName, szSubKeyName);
			if (_nRes != ERROR_SUCCESS) __leave;
	...
	__finally
	{
		if (szSubKeyName != NULL) delete[] szSubKeyName;

		if (szValueName != NULL) delete[] szValueName;

		if (pData != NULL) delete[] pData;

		if (hSrcKey != NULL) RegCloseKey(hSrcKey);

		if (hTargKey != NULL) RegCloseKey(hTargKey);
...
DWORD vmsSHCopyKey(HKEY hkeySrc, LPCSTR pszSubKey, HKEY hkeyDst)
{
	...
	if (_pfnCK == NULL)
	{
		...
		if (hDll) _pfnCK = (fntSHCopyKey)GetProcAddress(hDll, "SHCopyKeyA");
	}

	if (_pfnCK == NULL) return FALSE;
...
{
	if ((GetVersion() & 0x80000000) == 0) return MoveFileEx(pszSrc, pszDst, MOVEFILE_DELAY_UNTIL_REBOOT);

	char szWinInit[MAX_PATH] = "";
	GetWindowsDirectory(szWinInit, MAX_PATH);
	if (szWinInit[3] != 0) lstrcat(szWinInit, "\\");
	lstrcat(szWinInit, "wininit.ini");

	char sz[32000] = "";
	DWORD dwLen = GetPrivateProfileSection("rename", sz, sizeof(sz), szWinInit);
	LPSTR psz = sz + dwLen;
	size_t remaining = sizeof(sz) - dwLen;
	strcpy_s(psz, remaining, pszDst ? pszDst : "NUL");
	strcat_s(psz, remaining, "=");
	strcat_s(psz, remaining, pszSrc);
	psz[lstrlen(psz) + 1] = 0;
