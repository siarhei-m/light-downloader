...
CDDRDlg::CDDRDlg(CWnd* pParent) : CDialog(CDDRDlg::IDD, pParent)
{
...
BOOL CDDRDlg::OnInitDialog()
{
	...
	CString str;
	str.Format("%s\r\n \"%s\"\r\n%s", LS(L_DOWNLOAD2), m_strUrl.GetString(), LS(L_WBDELETED));
	SetDlgItemText(IDC_MESSAGE, str);

	if (m_bNoCancel) GetDlgItem(IDCANCEL)->EnableWindow(FALSE);
...
void CDDRDlg::ApplyLanguage()
{
	fsDlgLngInfo lnginfo[] = {
	    fsDlgLngInfo(IDC_DONTASKAGAIN, L_DONTASKAGAIN), fsDlgLngInfo(IDC_YES, L_YES),
	    fsDlgLngInfo(IDC_YESTOALL, L_YESTOALL),         fsDlgLngInfo(IDC_NO2, L_NO),
	    fsDlgLngInfo(IDC_NOTOALL, L_NOTOALL),           fsDlgLngInfo(IDCANCEL, L_CANCEL),
	};
...
{
	if (m_bNoCancel == FALSE) CDialog::OnClose();
}
