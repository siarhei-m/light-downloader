...
void CComboBoxTreeCtrl::OnKillFocus(CWnd* pNewWnd)
{
	...
	if (pNewWnd == NULL || pNewWnd->m_hWnd != m_pCombo->m_hWnd) m_pCombo->ShowDropDown(FALSE);
}
...
void CComboBoxTreeCtrl::OnKeydown(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	if (pTVKeyDown->wVKey == VK_ESCAPE && IsWindowVisible()) m_pCombo->ShowDropDown(FALSE);

	*pResult = 0;
}

BOOL CComboBoxTreeCtrl::CreateEx(DWORD dwExStyle, LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle,
