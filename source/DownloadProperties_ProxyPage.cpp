...
#define DNP_SET(member, bString, NewVal)                                                                               \
	DNPEntry_SetValue(m_pvDlds, DNP_OFFSET(member), DNP_SIZEOF(member), bString, NewVal)
...
BOOL CDownloadProperties_ProxyPage::OnInitDialog()
{
	...
	if (DNP_EQ(enAccType, FALSE))
	{
		...
		case IAT_SOCKS5PROXY:
			nChecked = IDC_USESOCKS;
			break;
		...
		CheckRadioButton(IDC_DONTUSEPROXY, IDC_USESOCKS, nChecked);
	}
	...
	{
		CheckRadioButton(IDC_DONTUSEPROXY, IDC_USESOCKS, IDC_DONTUSEPROXY);
	...
	// TEST
	CString strSocksProxyName = _App.SocksProxy_Name();
	if (!strSocksProxyName.IsEmpty())
	{
		CString strSocksProxy;
		USHORT uSocksPort;

		if (CrackProxyName(strSocksProxyName, strSocksProxy, uSocksPort))
			SetDlgItemInt(IDC_SOCKSPROXYPORT, uSocksPort, FALSE);
		else
			SetDlgItemText(IDC_SOCKSPROXYPORT, "");

		SetDlgItemText(IDC_SOCKSPROXYNAME, strSocksProxy);
	}
...
void CDownloadProperties_ProxyPage::UpdateEnabled()
{
	...
	// manual (HTTP/FTP-Proxy)
	b = IsDlgButtonChecked(IDC_MANUALLY) == BST_CHECKED;
	GetDlgItem(IDC__PROTOCOL)->EnableWindow(b);
	GetDlgItem(IDC_PROTOCOLS)->EnableWindow(b);
	GetDlgItem(IDC__ADDRESS)->EnableWindow(b);
	GetDlgItem(IDC__PORT)->EnableWindow(b);
	GetDlgItem(IDC__PROTOCOL)->EnableWindow(b);
	GetDlgItem(IDC_PROXYNAME)->EnableWindow(b);
	GetDlgItem(IDC_PORT)->EnableWindow(b);
	GetDlgItem(IDC_AUTHORIZATION)->EnableWindow(b);

	b &= IsDlgButtonChecked(IDC_AUTHORIZATION) == BST_CHECKED;
	...
	// socks
	b = IsDlgButtonChecked(IDC_USESOCKS) == BST_CHECKED;
	GetDlgItem(IDC__ADDRESS2)->EnableWindow(b);
	GetDlgItem(IDC__PORT2)->EnableWindow(b);
	GetDlgItem(IDC_SOCKSPROXYNAME)->EnableWindow(b);
	GetDlgItem(IDC_SOCKSPROXYPORT)->EnableWindow(b);

	// rollback
	b = IsDlgButtonChecked(IDC_USEROLLBACK) == BST_CHECKED;
...
void CDownloadProperties_ProxyPage::OnChangeProxyname()
{
	...
	GetDlgItemText(IDC_PROXYNAME, str); // actually: host!

	if (str == "http:" || str == "ftp:" || str == "https:") return;

	if (CrackProxyName(str, strName, uPort)) SetDlgItemInt(IDC_PORT, uPort, FALSE);
...
{
	if (m_bIChange == FALSE) SetModified();
}

void CDownloadProperties_ProxyPage::OnAuthorization()
{
	if (IsDlgButtonChecked(IDC_AUTHORIZATION) == BST_INDETERMINATE) CheckDlgButton(IDC_AUTHORIZATION, BST_UNCHECKED);
...
BOOL CDownloadProperties_ProxyPage::CrackProxyName(LPCSTR pszName, CString& strName, USHORT& uPort)
{
	...
	if (strstr(pszName, ":/") == pszPort || strstr(pszName, ":\\") == pszPort) pszPort = NULL;
	...
	if (pszPort - pszName + 1 == (int)strlen(pszName)) return FALSE;
...
BOOL CDownloadProperties_ProxyPage::OnApply()
{
	...
	if (IsDlgButtonChecked(IDC_DONTUSEPROXY) == BST_CHECKED)
	{
	...
	else if (IsDlgButtonChecked(IDC_MANUALLY) == BST_CHECKED)
	{
		...
		if (m_vHttp.size()) ApplyPS(&m_vHttp, &m_vProxies[i++]);

		if (m_vHttps.size()) ApplyPS(&m_vHttps, &m_vProxies[i++]);

		if (m_vFtp.size()) ApplyPS(&m_vFtp, &m_vProxies[i++]);
	}

	else if (IsDlgButtonChecked(IDC_USESOCKS) == BST_CHECKED)
	{
		// check
		m_bIChange = TRUE;

		CString strSocksProxy;
		GetDlgItemText(IDC_SOCKSPROXYNAME, strSocksProxy);
		if (strSocksProxy.GetLength() == 0)
		{
			MessageBox(LS(L_ENTERPROXY), LS(L_INPERR), MB_ICONEXCLAMATION);
			nErrID = IDC_SOCKSPROXYNAME;
			goto _lErr;
		}

		BOOL bOk;
		USHORT uSocksProxyPort = (USHORT)GetDlgItemInt(IDC_SOCKSPROXYPORT, &bOk, FALSE);
		if (bOk == FALSE)
		{
			MessageBox(LS(L_ENTERPROXYPORT), LS(L_INPERR), MB_ICONEXCLAMATION);
			nErrID = IDC_SOCKSPROXYPORT;
			goto _lErr;
		}

		CString str;
		str.Format(":%d", uSocksProxyPort);
		strSocksProxy += str;
		_App.SocksProxy_Name(strSocksProxy);

		fsInternetAccessTypeEx type = IATE_SOCKS5PROXY;
		DNP_SET(enAccType, FALSE, &type); //???
	}
...
BOOL CDownloadProperties_ProxyPage::ReadPS()
{
	...
	if (pi->uAuthorization == BST_CHECKED)
	{
		...
		if (pi->bPasswordModified) GetDlgItemText(IDC_PASSWORD, pi->strPassword);
	}
...
{
	if (m_wndProtocols.GetCurSel() != m_iCurProtocol) ReadPS();
}
...
void CDownloadProperties_ProxyPage::ApplyLanguage()
{
	fsDlgLngInfo lnginfo[] = {
	    ...
	    // fsDlgLngInfo (IDC_USESOCKS, L_USESOCKS), //TODO
...
void CDownloadProperties_ProxyPage::PrepareCHMgr(CPoint point)
{
	fsCtrlContextHelp aCH1[] = {
	    ...
	    // fsCtrlContextHelp (IDC_USESOCKS, LS (L_FROMUSESOCKS)), //TODO
...
{
	if (IsDlgButtonChecked(IDC_USEROLLBACK) == BST_INDETERMINATE) CheckDlgButton(IDC_USEROLLBACK, BST_UNCHECKED);
