...
{
	// if (m_bUseRegistry)
	return m_app->GetProfileInt(strlen(pszSection) == 0 ? "Settings" : pszSection, pszEntry, nDefault);
...
{
	// if (m_bUseRegistry)
	return m_app->WriteProfileInt(strlen(pszSection) == 0 ? "Settings" : pszSection, pszEntry, nValue);
	...
	// m_file.set_Value (pszSection, pszEntry, nValue); //LEAK
...
UINT64 vmsAppSettingsStore::GetProfileUint64(LPCSTR pszSection, LPCSTR pszEntry, UINT64 nDefault)
{
	LPBYTE pData;
	UINT uSize;
	if (!GetProfileBinary(pszSection, pszEntry, &pData, &uSize)) return nDefault;
...
{
	// if (m_bUseRegistry)
	return m_app->GetProfileString(strlen(pszSection) == 0 ? "Settings" : pszSection, pszEntry, pszDefault);
...
{
	// if (m_bUseRegistry)
	return m_app->WriteProfileString(strlen(pszSection) == 0 ? "Settings" : pszSection, pszEntry, pszValue);
...
{
	// if (m_bUseRegistry)
	return m_app->GetProfileBinary(strlen(pszSection) == 0 ? "Settings" : pszSection, pszEntry, ppData, pBytes);
...
{
	// if (m_bUseRegistry)
	return m_app->WriteProfileBinary(strlen(pszSection) == 0 ? "Settings" : pszSection,
	                                 strlen(pszEntry) == 0 ? NULL : pszEntry, pbData, nBytes);
