...
#include "system.h"
#include "Dlg_DeletedProperties.h"
...
{
	// TEST
	for (size_t i = 0; i < m_vStateFilters.size(); i++) delete m_vStateFilters[i];

	for (size_t i = 0; i < m_vGroups.size(); i++) delete m_vGroups[i].pGroupFilter;
}
...
BOOL CDownloads_Groups::Create(CWnd* pParent)
{
	...
	if (FALSE ==
	    CTreeCtrl::Create(TVS_HASLINES | TVS_HASBUTTONS | TVS_LINESATROOT | TVS_SHOWSELALWAYS, rc, pParent, 0x9876))
		return FALSE;
...
void CDownloads_Groups::OnGrpdelete()
{
	...
	if (hItem == NULL || hItem == m_hOther) return;

	if (IsGroupItem(hItem) == FALSE) return;
...
void CDownloads_Groups::OnGrpproperties()
{
	...
	if (filter == NULL || filter->GetType() != DFT_GROUP) return;
...
void CDownloads_Groups::OnOpengroupfolder()
{
	...
	CString str((LPCSTR)filter->GetGroup()->strOutFolder);
	...
	if (sz[2] != 0) ShellExecute(::GetDesktopWindow(), "explore", sz, NULL, NULL, SW_SHOW);
}
...
fsDldFilter* CDownloads_Groups::GetCurrentFilter()
{
	...
	if (hItem == m_hTasks) return &m_filterTasks;

	if (hItem == m_hAllGroups || hItem == m_hFilters) return &m_filterAll;

	if (GetParentItem(hItem) == m_hFilters || IsGroupItem(hItem)) return (fsDldFilter*)GetItemData(hItem);
...
{
	m_hDeleted = InsertItem(TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_STATE | TVIF_TEXT, LS(L_DELETED), 6, 6, 0, 0, 0,
	                        TVI_ROOT, TVI_LAST);
...
void CDownloads_Groups::ShowDeletedContextMenu()
{
	...
	if (_DldsMgr.Get_DeletedDownloadCount() == 0) pPopup->EnableMenuItem(ID_DELETED_CLEAR, MF_BYCOMMAND | MF_GRAYED);
...
{
	if (0 == _DldsMgr.Get_DeletedDownloadCount()) return;

	if (IDYES == MessageBox(LS(L_AREYOUSURE), LS(L_CONFIRMATION), MB_YESNO | MB_ICONQUESTION))
	{
		if (_pwndDownloads->Get_DWWN() == DWWN_DELETED) _pwndDownloads->m_wndDeleted.ShowWindow(SW_HIDE);
		try
		{
		...
		if (_pwndDownloads->Get_DWWN() == DWWN_DELETED) _pwndDownloads->m_wndDeleted.ShowWindow(SW_SHOW);
	}
...
{
	if (GetSelectedItem() != m_hAllGroups) SelectItem(m_hAllGroups);
}

void CDownloads_Groups::SetFilter_DeletedDownloads()
{
	if (GetSelectedItem() != m_hDeleted) SelectItem(m_hDeleted);
}
...
void CDownloads_Groups::UpdateDeletedIcon()
{
	...
	if (img != imgold) SetItemImage(m_hDeleted, img, img);
}
...
void CDownloads_Groups::SetGroupFilter(vmsDownloadsGroupSmartPtr pGroup)
{
	...
	if (hGrp != NULL && hGrp != GetSelectedItem()) SelectItem(hGrp);
}

void CDownloads_Groups::OnSetGroupsRootFolder()
{
	CFolderBrowser* fb =
	    CFolderBrowser::Create(LS(L_SELECTGRPSROOTFLDR), _DldsGrps.GetGroupsRootOutFolder(), NULL, this);
	if (fb == NULL) return;
...
void CDownloads_Groups::InsertGroups_tree(PDLDS_GROUPS_TREE pRoot, HTREEITEM hRoot)
{
	for (int i = 0; i < pRoot->GetLeafCount(); i++)
	{
		...
		if (pGroup->GetLeafCount()) InsertGroups_tree(pGroup, hGroup);
	}
}
...
void CDownloads_Groups::OnNewGroupCreated(vmsDownloadsGroupSmartPtr pGroup)
{
	...
	if (FindGroupIndex(pGroup) != -1) return;
...
void CDownloads_Groups::OnGroupNameChanged(vmsDownloadsGroupSmartPtr pGroup)
{
	...
	CString str;
	if (pGroup->cDownloads)
		str.Format("%s (%d)", pGroup->strName.pszString, pGroup->cDownloads);
	else
		str = (LPCSTR)pGroup->strName;
...
int CDownloads_Groups::FindGroupIndex(vmsDownloadsGroupSmartPtr pGroup)
{
	for (size_t i = 0; i < m_vGroups.size(); i++)
	{
		if (m_vGroups[i].pGroupFilter->GetGroup()->nId == pGroup->nId) return i;
	}
...
HTREEITEM CDownloads_Groups::InsertGroup(vmsDownloadsGroupSmartPtr pGroup, HTREEITEM hRoot)
{
	...
	if (pGroup->nId == GRP_OTHER_ID)
	{
		...
		grpItem.hGroup = m_hOther =
		    InsertItem(TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_STATE | TVIF_TEXT, str, 1, 1, 0, 0, 0, hRoot, TVI_LAST);
	}
	else
	{
		CString str;
...
{
	if (hItem == m_hTasks) return FALSE;

	if (hItem == m_hAllGroups || hItem == m_hFilters) return FALSE;

	if (hItem == m_hHistory || hItem == m_hDeleted || GetParentItem(hItem) == m_hDeleted) return FALSE;
...
void CDownloads_Groups::OnTimer(UINT_PTR nIDEvent)
{
		...
		for (size_t i = 0; i < m_vGroups.size(); i++)
		{
			...
			if (m_vGroups[i].cDownloads != m_vGroups[i].pGroupFilter->GetGroup()->cDownloads)
			{
				LPCSTR pszName = m_vGroups[i].pGroupFilter->GetGroup()->nId == GRP_OTHER_ID
				                     ? LS(L_OTHER)
				                     : (LPCSTR)m_vGroups[i].pGroupFilter->GetGroup()->strName;
