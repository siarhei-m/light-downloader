...
LRESULT CCustomTreeChildCtrl::OwnerDraw(CDC* pDC)
{
	...
	if (m_bkImage.hbm && IsWindowEnabled())
	{
		...
		CBitmap* pOldBitmap = dcTemp.SelectObject(CBitmap::FromHandle(m_bkImage.hbm));
		...
		pDC->BitBlt(+(int)x, (int)y, bm.bmWidth, bm.bmHeight, &dcTemp, 0, 0, SRCCOPY);
	...
	if (pimgList)
	{
		...
		if (pimgList->GetImageInfo(0, &ii))
			iconSize = CSize(ii.rcImage.right - ii.rcImage.left, ii.rcImage.bottom - ii.rcImage.top);
	}
	...
	if (pstateList)
	{
		...
		if (pstateList->GetImageInfo(0, &ii))
			stateImgSize = CSize(ii.rcImage.right - ii.rcImage.left, ii.rcImage.bottom - ii.rcImage.top);
	}
	...
	while (hItem)
	{
		...
		if (!(dwFlags & CDRF_SKIPDEFAULT))
		{
			...
			if (pimgList)
			{
				...
				x -= iconSize.cx + 1;
				pimgList->Draw(pDC, nImageIndex, CPoint(x, yCenterItem - iconSize.cy / 2), ILD_TRANSPARENT);
			}
			...
			if (GetStyle() & TVS_CHECKBOXES && pstateList != NULL)
			{
				...
				pstateList->Draw(pDC, dwStateImg, CPoint(x, yCenterItem - stateImgSize.cy / 2), ILD_TRANSPARENT);
			}
			...
			if (dwStyle & TVS_HASLINES)
			{
				...
				{
					_DotHLine(pDC->m_hDC, x - iconSize.cx / 2 - 2, yCenterItem, iconSize.cx / 2 + 2,
					          RGB(128, 128, 128));
				}

				if (hItemParent != NULL || dwStyle & TVS_LINESATROOT && GetPrevSiblingItem(hItem) != NULL)
				{
					_DotVLine(pDC->m_hDC, x - iconSize.cx / 2 - 2, rcItem.top, yCenterItem - rcItem.top,
					          RGB(128, 128, 128));
				}

				if (GetNextSiblingItem(hItem) != NULL)
				{
					_DotVLine(pDC->m_hDC, x - iconSize.cx / 2 - 2, yCenterItem, rcItem.bottom - yCenterItem,
					          RGB(128, 128, 128));
				}
			...
			if (dwStyle & TVS_HASBUTTONS && ItemHasChildren(hItem))
			{
				...
				m_imgBtns.Draw(pDC, nImg, CPoint(x - iconSize.cx / 2 - 6, yCenterItem - 4), ILD_TRANSPARENT);
			}
		}
...
BOOL CCustomTreeChildCtrl::CheckHit(CPoint point)
{
	...
	if (fFlags & TVHT_ONITEMICON || fFlags & TVHT_ONITEMBUTTON || fFlags & TVHT_ONITEMSTATEICON) return TRUE;

	if (GetStyle() & TVS_FULLROWSELECT)
	{
		rcItem.right = rcClient.right;
		if (rcItem.PtInRect(point)) return TRUE;
	...
	if (fFlags & TVHT_ONITEMLABEL)
	{
		...
		if (!rcItem.PtInRect(point)) return FALSE;
...
{
	if (!m_bCreatingWindowByOurself) Initialize();
}
...
void CColumnTreeCtrl::Initialize()
{
	...
	if (hComCtlDll)
	{
		...
		if (pfnDllGetVersion)
		{
			...
			if (SUCCEEDED(hRes) && dvi.dwMajorVersion >= 6) bIsComCtl6 = TRUE;
		}
...
void CColumnTreeCtrl::OnTreeCustomDraw(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	case CDDS_ITEMPOSTPAINT:
	{
		...
		if (rcLabel.Width() < 0) crTextBk = crWnd;
		if (crTextBk != crWnd)
		{
			...
			if (pNMCustomDraw->uItemState & CDIS_FOCUS) dc.DrawFocusRect(&rcSelect);
		}
		...
		if (IsWindowEnabled() && !bFullRowSelect) dc.SetTextColor(::GetSysColor(COLOR_MENUTEXT));

		if (pOldFont && !(m_Tree.GetStyle() & TVS_FULLROWSELECT)) dc.SelectObject(pOldFont);
		...
		for (i = 1; i < nColsCnt; i++)
		{
			if (AfxExtractSubString(strSub, strText, i, '\t'))
			{
				...
				DWORD dwFormat =
				    m_arrColFormats[i] & HDF_RIGHT ? DT_RIGHT : (m_arrColFormats[i] & HDF_CENTER ? DT_CENTER : DT_LEFT);

				dc.DrawText(strSub, &rcText, DT_SINGLELINE | DT_VCENTER | DT_NOPREFIX | DT_END_ELLIPSIS | dwFormat);
			}
			xOffset += m_arrColWidths[i];
		}
...
void CColumnTreeCtrl::UpdateColumns()
{
	...
	for (int i = 0; i < nCnt; i++)
	{
		if (m_Header.GetItem(i, &hditem))
		{
			...
			if (i == 0) m_Tree.m_nFirstColumnWidth = hditem.cxy;
		}
	}
...
void CColumnTreeCtrl::RepositionControls()
{
	...
	if (m_Tree.m_hWnd)
	{
		...
		m_horScroll.MoveWindow(0, rcClient.Height() - cyHScroll, rcClient.Width() - (bVScrollVisible ? cxVScroll : 0),
		                       cyHScroll);
...
int CColumnTreeCtrl::InsertColumn(int nCol, LPCTSTR lpszColumnHeading, int nFormat, int nWidth, int nSubItem)
{
	...
	if (m_Header.GetItemCount() > 0)
	{
		...
		hditem.pszText = const_cast<LPTSTR>(_T(""));
...
HTREEITEM CColumnTreeCtrl::HitTest(CTVHITTESTINFO* pHitTestInfo) const
{
	...
	if (!(uFlags & TVHT_ONITEMLABEL || uFlags & TVHT_ONITEMRIGHT)) return hItem;
...
// was: int
INT_PTR CCustomTreeChildCtrl::OnToolHitTest(CPoint point, TOOLINFO* pTI) const
{
...
LRESULT CColumnTreeCtrl::OnCtccGetToolTipText(WPARAM wp, LPARAM lp)
{
	...
	if (pt.x > m_arrColWidths[0]) return 0;
	...
	if (cxText + rc.left + 7 < m_arrColWidths[0]) return 0;
