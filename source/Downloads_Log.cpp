...
BOOL CDownloads_Log::Create(CWnd* pParent)
{
	...
	if (FALSE == CListCtrlEx::Create(LVS_REPORT | LVS_OWNERDRAWFIXED | LVS_NOSORTHEADER, rc, pParent, 0x56789))
		return FALSE;
...
void CDownloads_Log::AddRecord(fsDownloadEvents* event)
{
	...
	{
		iItem = AddItem(bFirst ? strTime : "", event->clrBg, event->clrText, bFirst ? event->iImage : -1);
		if (bFirst) SetItemText(iItem, 1, strDate);
		SetItemText(iItem, 2, pszEv2);
	}

	if (m_bAutoScroll) EnsureVisible(iItem, FALSE);
}
...
void CDownloads_Log::SetActiveDownload(vmsDownloadSmartPtr dld)
{
	...
	try
	{
		...
		for (int i = 0; i < cEvs; i++) AddRecord(&dld->vEvents[i]);
	}
...
void CDownloads_Log::OnRClick()
{
	...
	if (m_bAutoScroll) pPopup->CheckMenuItem(ID_AUTOSCROLL, MF_CHECKED | MF_BYCOMMAND);
