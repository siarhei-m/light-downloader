...
BOOL CToolBarEx::Create(CWnd* pParent)
{
	if (!CreateEx(pParent, TBSTYLE_FLAT | TBSTYLE_TRANSPARENT,
	              WS_CHILD | WS_VISIBLE | CBRS_TOOLTIPS | CBRS_ALIGN_TOP)) // crash in x64
		return FALSE;
	...
	SetSizes(CSize(_TB_SIZE_X + 7, _TB_SIZE_Y + 7), CSize(_TB_SIZE_X, _TB_SIZE_Y));

	if (!m_images.Create(_TB_SIZE_X, _TB_SIZE_Y, ILC_COLOR32 | ILC_MASK, 30, 5)) return FALSE;
...
{
	if (iGrp >= m_vGroups.size()) return FALSE;
...
{
	if (nm->hwndFrom != m_hWnd) return FALSE;
...
CToolBarEx::fsTBInfo* CToolBarEx::FindButton(UINT nID)
{
	...
	for (int i = 0; i < cButtons; i++)
	{
		...
		if (bt.idCommand != (int)nID) continue;

		fs::list<fsTBInfo>* pvGroup = &m_vGroups[bt.dwData];

		int j = 0;
		for (j = 0; j < pvGroup->size(); j++)
			if (pvGroup->at(j).idCommand == bt.idCommand) break;
...
void CToolBarEx::UpdateGroupToolTips(int iGrp, LPCSTR* ppszTips, UINT uSize)
{
	...
	for (int i = 0; i < cTips; i++) m_vGroups[iGrp][i].strToolTip = ppszTips[i];
}
