...
CDlg_SetBatchNumbers::CDlg_SetBatchNumbers(CWnd* pParent) : CDialog(CDlg_SetBatchNumbers::IDD, pParent)
{
...
void CDlg_SetBatchNumbers::UpdateEnabled()
{
	...
	for (int i = 0; i < sizeof(aIDs) / sizeof(UINT); i++) GetDlgItem(aIDs[i])->EnableWindow(b);
...
void CDlg_SetBatchNumbers::PrepareCHMgr(CPoint point)
{
	fsCtrlContextHelp aCH1[] = {
	    fsCtrlContextHelp(IDC__FROM, LS(L_FROMHERE)),    fsCtrlContextHelp(IDC_FROM, LS(L_FROMHERE)),
	    fsCtrlContextHelp(IDC__TO, LS(L_TOHERE)),        fsCtrlContextHelp(IDC_TO, LS(L_TOHERE)),
	    fsCtrlContextHelp(IDC__STEP, LS(L_STEPHERE)),    fsCtrlContextHelp(IDC_STEP, LS(L_STEPHERE)),
	    fsCtrlContextHelp(IDC__WB, LS(L_WILDBYTESHERE)), fsCtrlContextHelp(IDC_WB, LS(L_WILDBYTESHERE)),
	};
...
void CDlg_SetBatchNumbers::OnAdd()
{
	...
	if (IsDlgButtonChecked(IDC_SETOFNUMBERS) == BST_CHECKED)
	{
		...
		if (false == CheckSetOfNumbersParameters(from, to, step, wc)) return;
