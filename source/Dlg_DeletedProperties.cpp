...
CDlgDeletedProperties::CDlgDeletedProperties(CWnd* pParent) : CDialog(CDlgDeletedProperties::IDD, pParent)
{
...
BOOL CDlgDeletedProperties::OnInitDialog()
{
	...
	CheckDlgButton(IDC_CONFIRMDELETION, _App.Deleted_ConfirmDeletion() ? BST_CHECKED : BST_UNCHECKED);
...
{
	if (IsDlgButtonChecked(IDC_DONTUSE) == BST_CHECKED) CheckDlgButton(IDC_CONFIRMDELETION, BST_CHECKED);
...
void CDlgDeletedProperties::OnOK()
{
	...
	if (bDontUse == FALSE)
	{
		...
		_App.Deleted_BypassCompleted(IsDlgButtonChecked(IDC_BYPASSCOMPLETED) == BST_CHECKED);
	}
