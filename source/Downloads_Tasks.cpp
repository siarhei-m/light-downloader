...
BOOL CDownloads_Tasks::Create(CWnd* pParent)
{
	...
	if (FALSE == CListCtrlEx::Create(HDS_DRAGDROP | LVS_REPORT | LVS_OWNERDRAWFIXED | LVS_SHOWSELALWAYS | LVS_OWNERDATA,
	                                 rc, pParent, 0x56789))
		return FALSE;

	SetExtendedStyle(LVS_EX_FULLROWSELECT | LVS_EX_HEADERDRAGDROP | LVS_EX_INFOTIP | 0x00004000);
...
{
	if (_PluginMgr.IsWasExit()) return;

	if (FindItem(dld) != -1) return;
	...
	if (m_bAddingDownloads == false) SetItemCountEx(m_vDownloads.size(), LVSICF_NOINVALIDATEALL | LVSICF_NOSCROLL);
...
void CDownloads_Tasks::UpdateDownload(vmsDownloadSmartPtr dld, BOOL bRedraw)
{
	...
	if (iItem != -1) UpdateDownload(iItem, bRedraw);
}
...
void CDownloads_Tasks::OnRClick()
{
	...
	m_odmenu.SetImageList(&((CMainFrame*)AfxGetApp()->m_pMainWnd)->m_imgsMenu,
	                      &((CMainFrame*)AfxGetApp()->m_pMainWnd)->m_dimgsMenu);
	fsSetImage* pImages;
	int cImages;
...
void CDownloads_Tasks::DeleteSelected(BOOL bDontConfirmFileDeleting)
{
	...
	{
		if (IDNO == MessageBox(bDontConfirmFileDeleting ? LS(L_DLDWBDELETEDFROMDISK) : LS(L_AREYOUSURE),
		                       LS(L_CONFIRMATION), MB_YESNO | MB_ICONQUESTION))
		{
			SetFocus();
			return;
		}
	...
	DLDS_LIST vpDlds;
	int i = 0;
	for (i = 0; i < vDlds.size(); i++) vpDlds.push_back(vDlds[i].dld);
	...
	{
		if (vDlds[i].iIndex >= m_vDownloads.size()) break;
...
void CDownloads_Tasks::OnDldproperties()
{
	...
	while (pos)
	{
		...
		if (dld->pMgr->GetDownloadMgr() != NULL) vDlds.push_back(dld);
	}
	...
	if (0 == vDldsType) m_pDownloadsWnd->OnDownloadProperties(vDlds);
...
void CDownloads_Tasks::OnDldstart()
{
	...
	while (pos)
	{
		...
		if (dld->pMgr->IsRunning() == FALSE && dld->pMgr->IsDone() == FALSE) vDlds.push_back(dld);
	}
...
void CDownloads_Tasks::OnDldstop()
{
	...
	if (vDlds.size()) _DldsMgr.StopDownloads(vDlds, TRUE);
}
...
void CDownloads_Tasks::OnDldopenfolder()
{
	...
	while (pos)
	{
		...
		{
			if (vOpened[i] == strFileName) break;
		}
		if (i != -1) continue;
	...
	try
	{
		...
		else
		{
			...
			while (pos)
			{
				...
				if (bRunning == FALSE && dld->pMgr->IsRunning()) bRunning = TRUE;

				if (bStopped == FALSE && dld->pMgr->IsRunning() == FALSE && dld->pMgr->IsDone() == FALSE)
					bStopped = TRUE;

				if (bDone == FALSE && dld->pMgr->IsDone()) bDone = TRUE;

				if (bCanQuery == FALSE && dld->pMgr->GetNumberOfSections() == 0) bCanQuery = TRUE;
			}

			if (bRunning == FALSE)
			{
				if (iAutoStart == 0) pPopup->EnableMenuItem(ID_DLDSTOP, MF_BYCOMMAND | MF_GRAYED);
				pPopup->EnableMenuItem(ID_DLDADDSECTION, MF_BYCOMMAND | MF_GRAYED);
				pPopup->EnableMenuItem(ID_DLDDELSECTION, MF_BYCOMMAND | MF_GRAYED);
				pPopup->EnableMenuItem(ID_DLDGRANTBANDWIDTHFORDLD, MF_BYCOMMAND | MF_GRAYED);
				pPopup->EnableMenuItem(ID_DLDSHOWPROGRESSDLG, MF_BYCOMMAND | MF_GRAYED);
			...
			{
				pPopup->EnableMenuItem(ID_DLDSTART, MF_BYCOMMAND | MF_GRAYED);
				if (bDone == FALSE) pPopup->EnableMenuItem(ID_DLDRESTART, MF_BYCOMMAND | MF_GRAYED);
				pPopup->EnableMenuItem(ID_DLDQSIZE, MF_BYCOMMAND | MF_GRAYED);
			}
			...
			if (bCanQuery == FALSE) pPopup->EnableMenuItem(ID_DLDQSIZE, MF_BYCOMMAND | MF_GRAYED);

			if (iAutoStart == 1) pPopup->CheckMenuItem(ID_DLDAUTOSTART, MF_CHECKED | MF_BYCOMMAND);
...
int CDownloads_Tasks::GetDownloadImage(vmsDownloadSmartPtr dld)
{
	...
	if (_pwndScheduler && _pwndScheduler->GetMgr()->IsDownloadScheduled(dld)) return 2;
		...
		if (task->wts.enType == WTS_STARTDOWNLOAD || task->wts.enType == WTS_STOPDOWNLOAD)
		{
			for (int j = task->wts.dlds.pvIDs->size() - 1; j >= 0; j--)
			{
				if (task->wts.dlds.pvIDs->at(j) == nID)
				{
					...
					if (task->wts.dlds.pvIDs->size() == 0) mgr->DeleteTask(task);
					break;
				}
			}
		}
...
int CDownloads_Tasks::OnUpdateToolBar(UINT nID)
{
	...
	else
	{
		...
		{
		case ID_DLDSTART:
			if (bStopped == FALSE) bEnabled = FALSE;
			break;

		case ID_DLDSTOP:
			if (bRunning == FALSE && iAutoStart == 0) bEnabled = FALSE;
			break;

		case ID_DLDSCHEDULE:
		case ID_DLDSCHEDULESTOP:
			if (bRunning == FALSE && bStopped == FALSE) bEnabled = FALSE;
			break;
		}

		if (nID == ID_DLDAUTOSTART && iAutoStart == 1) bChecked = TRUE;
	}
...
void CDownloads_Tasks::ApplyLanguageToMenu(CMenu* menu)
{
	...
	menu->GetSubMenu(0)->ModifyMenu(10 - 1, MF_BYPOSITION | MF_STRING, 0, LS(L_ADVANCED));

	str = LS(L_RESTARTDL);
	str += "\tShift+R";
	menu->ModifyMenu(ID_DLDRESTART, MF_BYCOMMAND | MF_STRING, ID_DLDRESTART, str);

	str = LS(L_ADDONEMORESECT);
	str += "\t+";
	...
	menu->GetSubMenu(0)->ModifyMenu(12 - 1, MF_BYPOSITION | MF_STRING, 0, LS(L_SCHEDULE));
	...
	menu->ModifyMenu(ID_DLDGRANTBANDWIDTHFORDLD, MF_BYCOMMAND | MF_STRING, ID_DLDGRANTBANDWIDTHFORDLD,
	                 LS(L_GRANTBANDWIDTHFORDLD));
...
void CDownloads_Tasks::CalcCoordsForCurSel()
{
	...
	if (iCurSel == -1)
	{
		...
		POSITION pos = GetFirstSelectedItemPosition();
		if (pos) iCurSel = GetNextSelectedItem(pos);
	}
...
void CDownloads_Tasks::OnDldaddsection()
{
	...
	{
		vmsDownloadSmartPtr dld = m_vDownloads[GetNextSelectedItem(pos)];
		if (dld->pMgr->IsRunning()) dld->pMgr->CreateOneMoreSection();
	}
}
...
void CDownloads_Tasks::OnDlddelsection()
{
	...
	{
		vmsDownloadSmartPtr dld = m_vDownloads[GetNextSelectedItem(pos)];
		if (dld->pMgr->IsRunning()) dld->pMgr->StopSection();
	}
}
...
void CDownloads_Tasks::OnDldrestart()
{
	...
	if (pos)
	{
		if (MessageBox(LS(L_AREYOUSURE_WILLSTARTFROM0), LS(L_CONFIRMATION), MB_ICONQUESTION | MB_YESNO) == IDNO) return;
	}

	DLDS_LIST vpDlds;

	while (pos) vpDlds.push_back(m_vDownloads[GetNextSelectedItem(pos)]);

	for (size_t i = 0; i < vpDlds.size(); i++) _pwndDownloads->RestartDownload(vpDlds[i], i == vpDlds.size() - 1);
}
...
void CDownloads_Tasks::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	case 'A':
		if (bCtrl)
		{
			...
			for (int i = cItems - 1; i >= 0; i--) SetItemState(i, LVIS_SELECTED, LVIS_SELECTED);
			return;
		}
		break;
	...
	while (pos)
	{
		...
		if (dld->pMgr->GetDownloadMgr() != NULL)
		{
			if (dld->pMgr->IsRunning() == FALSE && dld->pMgr->IsDone() == FALSE &&
			    dld->pMgr->GetDownloadMgr()->IsQueringSize() == FALSE && dld->pMgr->GetNumberOfSections() == 0)
...
std::string CDownloads_Tasks::GetFileName(vmsDownloadSmartPtr dld)
{
	...
	if (dld->pMgr->GetDownloadMgr())
	{
		...
		CString strFile((LPCSTR)dld->pMgr->get_OutputFilePathName());

		int fl = strFile.GetLength();

		ASSERT(fl > 0);

		if (!fl) return "index.html";
		...
		else
		{
			...
			if (pMgr->GetDP()->pszAdditionalExt && *pMgr->GetDP()->pszAdditionalExt)
			{
				...
				if (pszAddExt) *(pszAddExt - 1) = 0;
			}
			strResult = sz;
		}

		if (strResult.empty()) strResult = "index.html";
	}
...
{
	if (CListCtrlEx::OnCreate(lpCreateStruct) == -1) return -1;
...
void CDownloads_Tasks::SelectDownload(vmsDownloadSmartPtr dld)
{
	...
	while (pos) SetItemState(GetNextSelectedItem(pos), 0, LVIS_SELECTED | LVIS_FOCUSED);
...
void CDownloads_Tasks::OnGetdispinfo(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	try
	{
		...
		if (pItem->mask & LVIF_TEXT)
		{
			...
			if (nSubItem != -1) lstrcpy(pItem->pszText, GetDownloadText(dld, nSubItem));
		}
	}
	catch (const std::exception& ex)
	{
...
CString CDownloads_Tasks::GetDownloadText(vmsDownloadSmartPtr dld, int nSubItem)
{
	...
	if (nSubItem == 1)
	{
		if (dld->pMgr->GetNumberOfSections() == 0) return "";
	...
	if (nSubItem == 2)
	{
		...
		if (_pwndDownloads->IsSizesInBytes() == FALSE)
		{
			...
			if (uSize != _UI64_MAX && uSize != 0)
				str.Format("%d%% [%.*g %s]", (int)((double)(INT64)uDone / (INT64)uSize * 100), val > 999 ? 4 : 3, val,
				           szDim);
			else
		...
		else
		{
			...
			if (uSize != _UI64_MAX)
				str.Format("%d%% [%s]", (int)((double)(INT64)uDone / (INT64)uSize * 100), s.GetString());
			else
				str = s;
		}
	...
	{
		CString str;
		str.Format("%d/%d", dld->pMgr->GetDownloadingSectionCount(), dld->pMgr->GetNumberOfSections());
		return str;
	}
	...
	if (nSubItem == 5)
	{
		...
		CString str;
		str.Format("%s/%s", BytesToString(dld->pMgr->GetSpeed()).GetString(), LS(L_S));
		return str;
	}
...
void CDownloads_Tasks::SortDownloads()
{
	...
	int i = 0;
	for (i = 0; i < N; i++) pDlds[i] = m_vDownloads[i];
	...
	for (i = 0; i < N; i++) m_vDownloads.push_back(pDlds[i]);
...
{
	if (pLV->uNewState != pLV->uOldState) UpdateActiveDownload(0);
}
...
{
	if (m_sortMode != LCSM_NONE) return;
	...
	int i;
	for (i = vIndxs.size() - 1; i >= 0; i--) m_vDownloads.erase(m_vDownloads.begin() + vIndxs[i]);
	if (enMT == MT_DOWN || enMT == MT_BOTTOM) nIndxDldPos -= vDlds.size() - 1;

	for (i = 0; i < (int)vDlds.size(); i++) m_vDownloads.insert(m_vDownloads.begin() + nIndxDldPos + i, vDlds[i]);
...
void CDownloads_Tasks::OnDldcheckintegrity()
{
	...
	if (dld->pMgr->IsDone() == FALSE) return;
	...
	if (IDCANCEL == _DlgMgr.DoModal(&dlg2)) return;

	if (dlg2.is_CheckingSucceeded() == false || _App.View_DontShowHashCheckResultIfOK() == FALSE)
	{
		...
		if (IDC_RESTARTDOWNLOAD == _DlgMgr.DoModal(&dlg3)) _pwndDownloads->RestartDownload(dld, TRUE);
	}
}
...
void CDownloads_Tasks::OnDldmovetofolder()
{
	...
	CFolderBrowser* fb = CFolderBrowser::Create(LS(L_CHOOSEOUTFOLDER), _App.View_LastDldMoveToFolder(), NULL, this);
	if (fb == NULL) return;

	_App.View_LastDldMoveToFolder(fb->GetPath());

	for (size_t i = 0; i < vDlds.size(); i++) vDlds[i]->pMgr->MoveToFolder(fb->GetPath());
}
...
void CDownloads_Tasks::ScheduleSelectedDlds(BOOL bStart)
{
	...
	while (pos)
	{
		int iItem = GetNextSelectedItem(pos);
		vmsDownloadSmartPtr dld = m_vDownloads[iItem];
		if (dld->pMgr->IsDone() == FALSE) vDlds.push_back(dld);
	}
	...
	if (uIDRet == IDOK)
	{
		if (bCreate)
		{
			...
			for (size_t i = 0; i < vDlds.size(); i++) pTask->wts.dlds.pvIDs->add(vDlds[i]->nID);

			_pwndScheduler->AddTask(pTask);
		}
		else
		{
			if (sheet.IsNeedRecalculateStartTime()) mgr->CalculateStartTime(pTask);
...
{
	COLORREF clr;
	if (m_vDownloads[nIndex]->pMgr->GetDownloadMgr() &&
	...
	if (bRedraw) RedrawItems(nIndex, nIndex);
}
...
void CDownloads_Tasks::OnDldgrantbandwidthfordld()
{
	...
	vmsDownloadSmartPtr dld = m_vDownloads[GetNextSelectedItem(pos)];
	if (dld == _DldsMgr.get_HighestPriorityDownload()) dld = NULL;
...
int CDownloads_Tasks::_compareDownloadsByLDFileSize(LPCVOID p1, LPCVOID p2)
{
	...
	if (size1 == _UI64_MAX) return size2 == _UI64_MAX ? 0 : 1;

	if (size2 == _UI64_MAX) return size1 == _UI64_MAX ? 0 : -1;
...
int CDownloads_Tasks::_compareDownloadsByTimeRamaining(LPCVOID p1, LPCVOID p2)
{
	...
	if (speed1 == 0 || size1 == _UI64_MAX) return speed2 == 0 || size2 == _UI64_MAX ? 0 : 1;

	if (speed2 == 0 || size2 == _UI64_MAX) return speed1 == 0 || size1 == _UI64_MAX ? 0 : -1;
