...
CDlgSounds::CDlgSounds(CWnd* pParent) : CDialog(CDlgSounds::IDD, pParent)
{
...
void CDlgSounds::PrepareCHMgr(CPoint point)
{
	...
	fsCtrlContextHelp aCH1[] = {
	    fsCtrlContextHelp(IDC_SOUNDS, pszSoundsHere),         fsCtrlContextHelp(IDC_SETSOUND, LS(L_SETSOUNDHERE)),
	    fsCtrlContextHelp(IDC_REMOVE, LS(L_REMOVESOUNDHERE)), fsCtrlContextHelp(IDC_TEST, LS(L_TESTSOUNDHERE)),
	    fsCtrlContextHelp(IDCANCEL, LS(L_DONTACCEPTCHANGES)), fsCtrlContextHelp(IDOK, LS(L_ACCEPTCHANGES)),
	};
...
{
	if (strSndFile == "") strSndFile = LS(L_NOSOUND);
	return strSndFile;
}
...
void CDlgSounds::UpdateEnabled()
{
	...
	if (cSel)
	{
		...
		while (pos)
		{
			...
			if (str != LS(L_NOSOUND)) bNoSound = FALSE;
		}
	}
...
{
	if (m_wndSnds.GetSelectedCount() == 0) return;
