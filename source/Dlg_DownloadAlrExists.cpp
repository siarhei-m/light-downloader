...
CDownloadAlrExistsDlg::CDownloadAlrExistsDlg(CWnd* pParent) : CDialog(CDownloadAlrExistsDlg::IDD, pParent)
{
...
{
	if (IsDlgButtonChecked(IDC_DONTCHECKAGAIN) == BST_CHECKED) _App.CheckIfDownloadWithSameUrlExists(FALSE);
...
BOOL CDownloadAlrExistsDlg::OnInitDialog()
{
	...
	if (m_dld->pMgr->GetPercentDone() == -1) GetDlgItem(IDC_RESUME)->EnableWindow(FALSE);

	if (m_dld->pMgr->IsRunning()) GetDlgItem(IDC_RESTART)->EnableWindow(FALSE);
...
void CDownloadAlrExistsDlg::ApplyLanguage()
{
	fsDlgLngInfo lnginfo[] = {
	    fsDlgLngInfo(IDC__MSG, L_DLDALREXISTS), fsDlgLngInfo(IDC_RESUME, L_RESUME),
	    fsDlgLngInfo(IDC_RESTART, L_RESTART),   fsDlgLngInfo(IDC_LAUNCH, L_LAUNCH),
	    fsDlgLngInfo(IDCANCEL, L_CANCEL),       fsDlgLngInfo(IDC_DONTCHECKAGAIN, L_DONTCHECKAGAIN),
	};
