...
bool fsCmdHistoryMgr::fs1DayRecords::loadObjectItselfFromStateBuffer(LPBYTE pbtBuffer, LPDWORD pdwSize, DWORD dwVer)
{
	...
	if (!getVarFromBuffer(day, pbtCurrentPos, pbtBuffer, *pdwSize)) return false;
	...
	while (cRecs--)
	{
		...
		if (!getStrFromBuffer(&pszRec, pbtCurrentPos, pbtBuffer, *pdwSize)) return false;
...
bool fsCmdHistoryMgr::loadObjectItselfFromStateBuffer(LPBYTE pbtBuffer, LPDWORD pdwSize, DWORD dwVer)
{
	LPBYTE pbtCurrentPos = pbtBuffer;
	int cDays = 0;
	if (!getVarFromBuffer(cDays, pbtCurrentPos, pbtBuffer, *pdwSize)) return false;
...
{
	int cRecords = 0;
	for (int i = 0; i < m_vRecs.size(); i++) cRecords += m_vRecs[i]->vRecs.size();
	return cRecords;
}

void fsCmdHistoryMgr::AddRecord(LPCSTR pszRecord)
{
	if (m_bNoHistory) return;
	...
	if (m_vRecs.size() == 0)
	{
		...
		if ((fs1DayRecords*)pRecPtr == 0) return;
