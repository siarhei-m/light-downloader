...
CDlg_Options_Downloads::CDlg_Options_Downloads(CWnd* pParent) : CDlg_Options_Page(CDlg_Options_Downloads::IDD, pParent)
{
...
BOOL CDlg_Options_Downloads::OnInitDialog()
{
	...
	CheckDlgButton(IDC_SIZEINBYTES,
	               _pwndDownloads->m_wndDownloads.m_tasks.m_bSizesInBytes ? BST_CHECKED : BST_UNCHECKED);
...
BOOL CDlg_Options_Downloads::Apply()
{
	...
	if (!bShow != !_App.DownloadDialog_Use())
	{
		...
		if (bShow) _App.DownloadDialog_DontAskOnHide(FALSE);
	}

	DWORD dw = 0;
	if (IsDlgButtonChecked(IDC_USEWC) == BST_CHECKED) dw = GetDlgItemInt(IDC_WC_SIZE, NULL, FALSE) * 1024 * 1024;
...
{
	fsDlgLngInfo lnginfo[] = {fsDlgLngInfo(IDC__LISTOFDLDS, L_LISTOFDLDS),
...
void CDlg_Options_Downloads::UpdateEnabled()
{
	...
	for (UINT i = 0; i < sizeof(aIDs2) / sizeof(UINT); i++) GetDlgItem(aIDs2[i])->EnableWindow(b);
}
