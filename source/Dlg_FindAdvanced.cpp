...
CDlgFindAdvanced::CDlgFindAdvanced(CWnd* pParent) : CDialog(CDlgFindAdvanced::IDD, pParent)
{
	// m_bTimeSet = FALSE;
...
BOOL CDlgFindAdvanced::OnInitDialog()
{
	...
	if (dwFlags & SEARCHWH_LISTOFDLDS) CheckDlgButton(IDC_LISTOFDLDS, BST_CHECKED);

	if (dwFlags & SEARCHWH_DELETED) CheckDlgButton(IDC_DELETED, BST_CHECKED);

	m_wndGroups.Fill();
	m_wndGroups.SelectGroup(NULL);

	if (dwFlags & SEARCHU_DLDGROUP) m_wndGroups.SelectGroupById(_App.Find_GroupId());

	if (dwFlags & SEARCHU_TIME) CheckDlgButton(IDC_USETIME, BST_CHECKED);

	if (dwFlags & SEARCHU_FILESIZE) CheckDlgButton(IDC_USESIZE, BST_CHECKED);

	m_wndDim1.AddString(LS(L_B));
	m_wndDim2.AddString(LS(L_B));
	m_wndDim1.AddString(LS(L_KB));
	m_wndDim2.AddString(LS(L_KB));
	m_wndDim1.AddString(LS(L_MB));
	m_wndDim2.AddString(LS(L_MB));
...
void CDlgFindAdvanced::ApplyLanguage()
{
	fsDlgLngInfo lnginfo[] = {
	    fsDlgLngInfo(IDC__FINDWHERE, L_FINDWHERE),       fsDlgLngInfo(IDC_LISTOFDLDS, L_FINDINLISTOFDLDS),
	    fsDlgLngInfo(IDC__SEARCHINGROUP, L_FINDINGROUP), fsDlgLngInfo(IDC_DELETED, L_FINDINDELETED),
	    fsDlgLngInfo(IDC_USESIZE, L_FINDUSESIZE),        fsDlgLngInfo(IDC__MINIMUM, L_MAXIMAL),
	    fsDlgLngInfo(IDC__MAXIMUM, L_MINIMAL),           fsDlgLngInfo(IDCANCEL, L_CANCEL),
	};
...
void CDlgFindAdvanced::OnOK()
{
	...
	if (bListOfDlds) dwFlags |= SEARCHWH_LISTOFDLDS;

	if (bDeleted) dwFlags |= SEARCHWH_DELETED;
