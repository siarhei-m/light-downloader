...
BOOL fsLangMgr::Initialize()
{
	...
	//		CStdioFile file (m_strLngFolder + info.strFileName, CFile::modeRead | CFile::typeText |
	//CFile::shareDenyNone);
	...
	return FALSE; // m_vLngFiles.size() != 0;
}
...
//	try
//	{
//		CStdioFile file (m_strLngFolder + m_vLngFiles [iIndex].strFileName, CFile::modeRead | CFile::typeText |
//CFile::shareDenyNone); 		CString str;
//
//		do
//		{
...
LPCSTR fsLangMgr::GetString(int iIndex)
{
	...
	try
	{
		...
		// else
		if (iIndex < m_vBuiltInStrings.size())
...
void fsLangMgr::LoadBuiltInLngStrings()
{
	...
	while (psz < pszE)
	{
		...
		if (str == "" || str[0] == LNG_COMMENT_CHAR) continue;
