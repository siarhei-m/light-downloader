...
class fsInternetDownloader
{
  ...
  public:
	...
	fsInternetResult AddMirrorURL(LPCSTR pszUrl, LPCSTR pszUser = NULL, LPCSTR pszPassword = NULL,
  ...
  protected:
	...
	fsInternetResult OpenUrl(UINT64 uStartPos, fsInternetURLFile** ppFile, int iSectIndex, UINT& nMirror,
	                         BOOL bCheckFileSize = FALSE, LPCSTR pszContentTypeReq = NULL);
	fsInternetResult OpenUrl_imp(UINT64 uStartPos, fsInternetURLFile** ppFile, int iSectIndex, UINT& nMirror,
	                             BOOL bCheckFileSize, int iAttempt, LPCSTR pszContentTypeReq = NULL,
	                             fsDownload_NetworkProperties* pDNPRedirectedUrl = NULL, bool bIsRedirected = false);
  ...
  private:
	static BOOL __threadDownload_flushdata(fsSectionEx* sect, LPBYTE pBuffer, DWORD dwRead, LPBYTE& pbCache,
