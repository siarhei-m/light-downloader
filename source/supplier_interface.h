...
{
	TImportResult() : nTotal(-1), nSuccess(-1), nFailure(-1), nUnprocessed(-1) {}
...
{
  public:
	CImportResultGuard(TImportResult& tImportResult) : m_tImportResult(tImportResult), m_bIsSuccess(false) {}
...
typedef void (*InitManagerResult)(int nManagerIndex, bool bChecked, const CString& sSupplierName,
                                  const TImportResult& tImportResult, void* pData);
