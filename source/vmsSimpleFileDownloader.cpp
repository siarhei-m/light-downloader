...
void vmsSimpleFileDownloader::_DownloadMgrDescEvents(fsDownloadMgr*, fsDownloadMgr_EventDescType, LPCSTR pszDesc,
                                                     LPVOID lp)
{
}
...
DWORD vmsSimpleFileDownloader::_DownloadMgrEvents(fsDownloadMgr* pMgr, fsDownloaderEvent ev, UINT uInfo, LPVOID lp)
{
	...
	case DE_EXTERROR:
		...
		{
			if (pthis->m_dldr->IsDone() == FALSE) pthis->m_dldr->DeleteFile();
		}
		break;
	}
...
{
	if (m_dldr) m_dldr->StopDownloading();
}
