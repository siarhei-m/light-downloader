...
void CDownloads_Progress::DrawProgress(CDC* dc, vmsDownloadSmartPtr dld)
{
	...
	if (uFileSize == 0 || uFileSize == _UI64_MAX) return;
	...
	if (m_iLastProgress != (int)(dld->pMgr->GetPercentDone() * 10) || m_bmpProgress.m_hObject == NULL ||
	    m_bRepaintBitmap)
	{
		if (m_bmpProgress.m_hObject) m_bmpProgress.DeleteObject();
		m_bRepaintBitmap = false;

		bool bDontUseAlreadyDraw = FALSE; // dld->pMgr->IsBittorrent() != FALSE;
		...
		for (size_t i = 0; i < v.size(); i++) DrawSectionProgress(&dcDraw, &v[i], i, uFileSize, bDontUseAlreadyDraw);
...
void CDownloads_Progress::DrawSectionProgress(CDC* dc, vmsSectionInfo* sect, int iSect, UINT64 uFileSize,
                                              bool bDontUseAlreadyDraw)
{
	...
	for (int i = 0; i < nsqe; i++)
	{
		if (i >= nsqs)
		{
			if (i >= nsqc) dc->SelectObject(&m_brProgress);
