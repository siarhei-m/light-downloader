...
#include "stdafx.h"
#include "Dlg_TumSettingsPerDirection.h"
...
BOOL CDlgTumSettingsPerDirection::OnInitDialog()
{
	...
	UINT aSpinIds[] = {IDC_RESTTRAFFSSPIN, IDC_MNC_VALUESPIN, IDC_MNCPS_VALUESPIN, IDC_MNT_VALUESPIN};
...
{
	for (int i = 0; i < 3; i++) m_settings[i] = _TumMgr.GetTumRawSettings((fsTUM)i).getSettings(bForDownload);
}

void CDlgTumSettingsPerDirection::SaveSettingsToTumMgr(bool bForDownload)
{
	for (int i = 0; i < 3; i++) _TumMgr.SetTumRawSettingsPerDirection((fsTUM)i, m_settings[i], bForDownload);
}
...
{
	CString str1 = LS(L_MAXCOUNTOFCONNS), str2 = LS(L_MAXCOUNTOFCONNSPERSERV), str3 = LS(L_MAXDLDS);
	if (str1.Right(1) == _T (":")) str1.Delete(str1.GetLength() - 1);
	if (str2.Right(1) == _T (":")) str2.Delete(str2.GetLength() - 1);
	if (str3.Right(1) == _T (":")) str3.Delete(str3.GetLength() - 1);
