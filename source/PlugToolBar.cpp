...
BOOL CPlugToolBar::Create(CWnd* pParent)
{
	...
	if (FALSE == CListBox::Create(LBS_OWNERDRAWFIXED | LBS_HASSTRINGS | LBS_NOINTEGRALHEIGHT | WS_CHILD | WS_VSCROLL |
	                                  WS_HSCROLL,
	                              rc, pParent, 0x123))
		return FALSE;
...
BOOL CPlugToolBar::Initialize()
{
	...
	if (FALSE == _PluginMgr.LoadPlugins(m_pPlugFrame)) return FALSE;
