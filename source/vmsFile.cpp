...
void vmsFile::Create(LPCTSTR pszFileName, DWORD dwAccess, DWORD dwShareMode, DWORD dwCreationDisposition, DWORD dwFlags)
{
	...
	m_hFile = CreateFile(pszFileName, dwAccess, dwShareMode, NULL, dwCreationDisposition, dwFlags, NULL);

	if (m_hFile == INVALID_HANDLE_VALUE) throw GetLastError();
}
...
void vmsFile::Close()
{
	...
	{
		if (FALSE == CloseHandle(Detach())) throw GetLastError();
	}
}

void vmsFile::Read(LPVOID pvBuffer, DWORD dwToRead, DWORD* pdwRead)
{
	if (m_hFile == INVALID_HANDLE_VALUE) throw ERROR_INVALID_HANDLE;

	DWORD dwRead = 0;

	if (FALSE == ReadFile(m_hFile, pvBuffer, dwToRead, &dwRead, NULL)) throw GetLastError();

	if (pdwRead) *pdwRead = dwRead;

	if (m_bThrowOnPartialRead && dwRead != dwToRead) throw ERROR_BAD_FORMAT;
}
...
void vmsFile::Write(LPCVOID pvBuffer, DWORD dwToWrite)
{
	...
	if (FALSE == WriteFile(m_hFile, pvBuffer, dwToWrite, &dwWritten, NULL)) throw GetLastError();

	if (dwWritten != dwToWrite) throw ERROR_BAD_LENGTH;
}
...
{
	if (FALSE == fsSaveStrToFile(psz, m_hFile)) throw GetLastError();
}

void vmsFile::ReadString(fsString& str)
{
	LPSTR psz;
	if (FALSE == fsReadStrFromFile(&psz, m_hFile)) throw GetLastError();
	str = psz;
	delete[] psz;
}
