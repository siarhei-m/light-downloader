...
class vmsCommandLineParser
{
  ...
  public:
	bool Parse(LPCTSTR ptszCmdLine = GetCommandLine())
	{
		...
		while (*ptszCmdLine)
		{
			...
			{
				while (*++ptszCmdLine && *ptszCmdLine != ' ' && *ptszCmdLine != '=') tstrParam += *ptszCmdLine;
			...
			if (!tstrParam.empty() || !tstrValue.empty()) m_vArgs.push_back(Argument(tstrParam, tstrValue));
		}
  ...
  public:
	...
	const Argument* findArgument(LPCTSTR ptszName) const
	{
		...
		{
			if (m_vArgs[i].first == ptszName) return &m_vArgs[i];
		}
		return NULL;
	}
