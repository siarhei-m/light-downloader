...
CWaitForConfirmationDlg::CWaitForConfirmationDlg(CWnd* pParent) : CDialog(CWaitForConfirmationDlg::IDD, pParent)
{
...
void CWaitForConfirmationDlg::Init(LPCSTR pszMsg, UINT uWait, BOOL bShowDontAsk, BOOL bNoIsDefault,
                                   LPCSTR pszDontAskAgainText, LPCSTR pszMsgIfDontAskAgain)
{
	m_msg = pszMsg;
	m_wait = uWait;
...
BOOL CWaitForConfirmationDlg::OnInitDialog()
{
	...
	if (m_strDontAskAgainText.GetLength() != 0) SetDlgItemText(IDC_DONTASK, m_strDontAskAgainText);
...
void CWaitForConfirmationDlg::OnTimer(UINT_PTR nIDEvent)
{
