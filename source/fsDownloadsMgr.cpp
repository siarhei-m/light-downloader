...
fsDownloadsMgr::~fsDownloadsMgr()
{
	...
	while (m_cThreadsRunning) Sleep(10);
}
...
void fsDownloadsMgr::_DownloadMgrEventDesc(fsDownloadMgr* pMgr, fsDownloadMgr_EventDescType enType, LPCSTR pszEvent,
                                           LPVOID lp)
{
	try
	{
...
vmsDownloadSmartPtr fsDownloadsMgr::GetDownloadByDownloadMgr(fsDownloadMgr* pMgr)
{
	...
	try
	{
		...
		{
			if (m_vDownloads[i]->pMgr->GetDownloadMgr() == pMgr) return m_vDownloads[i];
		}
	}
	catch (const std::exception& ex)
	{
...
DWORD fsDownloadsMgr::_DownloadMgrEvents(fsDownloadMgr* pMgr, fsDownloaderEvent enEvent, UINT uInfo, LPVOID lp)
{
	...
	try
	{
		...
		case DE_EXTERROR:
			...
			case DMEE_STOPPEDORDONE:
				...
				if (FALSE == pThis->OnDownloadStoppedOrDone(dld)) return TRUE;
				dld = NULL;
				break;
			...
			case DMEE_STARTING:
				pThis->RegisterDownloadInTum(dld);
				if (FALSE == pThis->Event(dld, DME_DLMGRTHREADSTARTING)) return FALSE;
	...
	if (dld) pThis->Event(dld, DME_DOWNLOADEREVENTRECEIVED);
...
{
	if (m_dwPDTimeLimit && isAllowedToPDCurrently() == false) return;

	DLDS_LIST vDldsToStop, vDldsToStart;
	vmsDownloadSmartPtr dldBogus;

	if (m_bAllowStart == FALSE || m_bDisablePD) return;
	...
	try
	{
					...
					else
					{
						if (dld->bAutoStart)
						{
							...
							if (pMgr && cDldConns > 1)
							{
								...
								{
									cDldConns = min(cDldConns, UINT(pMgr->GetDownloader()->GetBytesLeft() /
									                                pMgr->GetDP()->uSectionMinSize));
									if (!cDldConns) cDldConns = 1;
								}
							}
							if (cDldConns) bAccept = TRUE;
						}
					}
		...
		if (cConns == _TumMgr.getSettings().download.uMaxConns && dldBogus != NULL)
		{
			ASSERT(vDldsToStart.empty());
			if (vDldsToStart.empty()) vDldsToStart.push_back(dldBogus);
		}
...
void fsDownloadsMgr::StartDownloads(DLDS_LIST& vpDlds, BOOL)
{
	try
	{
		...
		for (size_t i = 0; i < vpDlds.size(); i++)
		{
			...
			if (cRunning < _TumMgr.getSettings().download.uMaxConns &&
			    cRunning < _TumMgr.getSettings().download.uMaxTasks && cRunning < 20)
			{
...
void fsDownloadsMgr::StopDownloads(DLDS_LIST& vDlds, BOOL bByUser)
{
	try
	{
		...
		for (size_t i = 0; i < vDlds.size(); i++) vDlds[i]->pMgr->StopDownloading();
	}
	catch (const std::exception& ex)
	{
		ASSERT(FALSE);
	...
	if (bByUser) Event(NULL, NULL, DME_DLDSAUTOSTARTMDFD);
}
...
DWORD fsDownloadsMgr::Event(fsDownload* dld, fsDLHistoryRecord* phst, fsDownloadsMgrEvent ev)
{
	...
	if (dld && dld->pfnDownloadEventsFunc) dwRet = dld->pfnDownloadEventsFunc(dld, phst, ev, dld->lpEventsParam);

	if (m_pfnEvents) dwRet = m_pfnEvents(dld, phst, ev, m_lpEventsParam);
...
vmsDownloadSmartPtr fsDownloadsMgr::GetDownload(size_t iIndex)
{
	...
	try
	{
		if (iIndex >= m_vDownloads.size()) return NULL;
...
BOOL fsDownloadsMgr::LoadDownloads()
{
	...
	if (!m_saver.Load(m_vDownloads, "downloads", FALSE)) return FALSE;
	...
	for (int j = 0; j < 2; j++)
	{
		...
		if (res == DLLR_TOOLARGESIZE)
		{
			...
			if (dlg.m_bDontAsk) _App.CheckRecycleBinSize(FALSE);
			if (nRet == IDOK) break;
		}
		else
			break;
...
BOOL fsDownloadsMgr::Save()
{
	...
	{
		vmsAUTOLOCKRW_READ(m_rwlDownloads);
		if (FALSE == m_saver.Save(m_vDownloads, "downloads")) b = FALSE;
		vmsAUTOLOCKRW_READ_UNLOCK(m_rwlDownloads);
	}
	catch (const std::exception& ex)
	{
...
int fsDownloadsMgr::GetDownloadIndex(vmsDownloadSmartPtr dld, int nFindStartPos)
{
	...
	{
		if (m_vDownloads[i] == dld) return i;
	}
...
int fsDownloadsMgr::DeleteDownloads(DLDS_LIST& vDlds, BOOL bByUser, BOOL bDontConfirmFileDeleting,
                                    BOOL bDontDeleteFiles)
{
	size_t cMaxDlds = 40;

	try
	{
		if (vDlds[0]->pfnDownloadEventsFunc) cMaxDlds = 20;
	}
	catch (const std::exception& ex)
	{
		ASSERT(FALSE);
	...
	if (vDlds.size() < cMaxDlds) return DeleteDownloads2(&vDlds, bByUser, bDontConfirmFileDeleting, bDontDeleteFiles);

	int iDeleted = 0;

	CWaitDlg dlg;
	dlg.StartWaiting(LS(L_DELETINGDLDS), _threadWaitDelDlds, TRUE, &vDlds, &bByUser, &bDontConfirmFileDeleting,
	                 &bDontDeleteFiles, &iDeleted, this);
...
void fsDownloadsMgr::StopGroup(vmsDownloadsGroupSmartPtr pGroup)
{
	...
	for (size_t i = 0; i < m_vDownloads.size(); i++)
	{
		...
		if (dld->pGroup->nId == pGroup->nId) vDlds.push_back(dld);
	}
...
void fsDownloadsMgr::StartGroup(vmsDownloadsGroupSmartPtr pGroup)
{
	...
	for (size_t i = 0; i < m_vDownloads.size(); i++)
	{
		...
		if (dld->pGroup->nId == pGroup->nId && dld->pMgr->IsRunning() == FALSE) vDlds.push_back(dld);
	}
...
DWORD WINAPI fsDownloadsMgr::_threadDownloadsMgr(LPVOID lp)
{
	...
	while (pThis->m_bShuttingDown == FALSE)
	{
		if (ticksNow - lastSpeedMeasure >= 5000)
		{
			...
			if (pThis->m_vSummSpeed.size() == MAX_SUMMS) pThis->m_vSummSpeed.clear();
		...
		if (pThis->m_dwPDTimeLimit) pThis->ManagePD();
		...
		while (ticksNow < ticks1)
		{
			...
			if (pThis->m_ticksNeedStartProcessDownloads.m_dwTicks &&
			    pThis->m_ticksNeedStartProcessDownloads <= ticksNow)
			{
				bNeedPD = true;
				pThis->m_ticksNeedStartProcessDownloads.m_dwTicks = 0;
				lastDldsProc.Now();
			}
			if (pThis->m_ticksNeedStartApplyTrafficLimit.m_dwTicks &&
			    pThis->m_ticksNeedStartApplyTrafficLimit <= ticksNow)
			{
				bNeedAL = true;
				pThis->m_ticksNeedStartApplyTrafficLimit.m_dwTicks = 0;
			}
			if (pThis->m_ticksNeedCheckNoActiveDownloads.m_dwTicks &&
			    pThis->m_ticksNeedCheckNoActiveDownloads <= ticksNow)
			{
				bNeedCNAD = true;
			...
			if (bNeedPD) pThis->ProcessDownloads();
			if (bNeedAL) pThis->ApplyTrafficLimit();
			if (bNeedCNAD) pThis->CheckNoActiveDownloads();
...
void fsDownloadsMgr::RebuildServerList(BOOL bUpdateSiteList)
{
	...
	try
	{
		...
		for (i = m_vDownloads.size() - 1; i >= 0; i--)
		{
			...
			if (dld->pMgr->GetDownloadMgr() == NULL) continue;
			...
			for (int j = -1; j < cSects || (cSects == 0 && j == 0); j++)
			{
				...
				if (bSect) pMgr->GetDownloader()->GetSectionInfo(j, &sect);
				...
				{
					if (vMirrsCalculated[k] == sect.nMirrorURL) break;
				}
	...
	try
	{
		if (bUpdateSiteList)
		{
			for (int i = 0; i < _SitesMgr.GetSiteCount(); i++) _SitesMgr.SiteUpdated(_SitesMgr.GetSite(i));
		}
	}
	catch (const std::exception& ex)
	{
...
BOOL fsDownloadsMgr::OnQueryNewSection(vmsDownloadSmartPtr dld, UINT nUsingMirror)
{
	...
	if (dld->pMgr->GetDownloadMgr()->GetDP()->bIgnoreRestrictions) return TRUE;
...
UINT fsDownloadsMgr::GetAmountConnections()
{
	...
	try
	{
		for (int i = _SitesMgr.GetSiteCount() - 1; i >= 0; i--) nConns += _SitesMgr.GetSite(i)->cConnsNow;
	}
	catch (const std::exception& ex)
	{
		ASSERT(FALSE);
...
void fsDownloadsMgr::StartAllDownloads(BOOL bByUser)
{
	...
	try
	{
		for (size_t i = 0; i < m_vDownloads.size(); i++)
		{
			if (m_vDownloads[i]->pMgr->IsDone() == FALSE) vDlds.push_back(m_vDownloads[i]);
		}
	}
	catch (const std::exception& ex)
	{
...
void fsDownloadsMgr::StopAllDownloads(BOOL bByUser)
{
	...
	try
	{
		for (size_t i = 0; i < m_vDownloads.size(); i++) vDlds.push_back(m_vDownloads[i]);
	}
	catch (const std::exception& ex)
	{
		ASSERT(FALSE);
...
BOOL fsDownloadsMgr::IsSummarySpeedWasLessThan(UINT uSpeed, UINT uTimeInterval)
{
	...
	if (uSize < uNeed) return FALSE;

	for (UINT i = uSize - 1; i > uSize - uNeed; i--)
	{
		if (m_vSummSpeed[i] >= uSpeed) return FALSE;
	}
...
BOOL fsDownloadsMgr::IsRunning()
{
	...
	try
	{
		for (int i = m_vDownloads.size() - 1; i >= 0; i--)
			if (m_vDownloads[i]->pMgr->IsRunning()) return TRUE;
	}
	catch (const std::exception& ex)
	{
		ASSERT(FALSE);
...
{
	if (IsRunning() == FALSE) Event((fsDownload*)NULL, DME_NOACTIVEDOWNLOADS);
}
...
{
	if (GetAmountConnections() >= _TumMgr.getSettings().download.uMaxConns) return TRUE;
...
void fsDownloadsMgr::UseDetailedLog(BOOL b)
{
	vmsAUTOLOCKRW_READ(m_rwlDownloads);
	m_bDetLog = b;
	for (int i = m_vDownloads.size() - 1; i >= 0; i--) m_vDownloads[i]->pMgr->UseDetailedLog(b);
}

int fsDownloadsMgr::DeleteDownloads2(DLDS_LIST* vDlds, BOOL bByUser, BOOL bDontConfirmFileDeleting,
                                     BOOL bDontDeleteFiles, BOOL* pbNeedStop, int* piProgress)
{
	UINT i = 0;
	...
	else if (bBypassBin)
	{
		...
		if (vDldsDDRs.size() != vDlds->size()) return 0;
	}
	else
	{
	...
	try
	{
		...
		{
			if (pbNeedStop && *pbNeedStop) break;
			...
			if (dld->pdlg) Event(dld, DME_CLOSEDLDDIALOG);

			if (piProgress) *piProgress = (int)((double)i / vDlds->size() * 100);
			...
			if (FALSE == bBypassBin && (m_bBypassBinForCompletedDownloads == FALSE || bDone == FALSE))
			{
	...
	DLDS_LIST v;
	v.reserve(m_vDownloads.size());
	for (i = 0; i < m_vDownloads.size(); i++)
	{
		if (m_vDownloads[i] != dldFake) v.push_back(m_vDownloads[i]);
	}
	m_vDownloads = v;
	...
	for (size_t i = 0; i < vDldsWhere.size(); i++)
	{
		...
		for (size_t j = nStart; j < vDldsWhat.size(); j++)
		{
			if (dld == vDldsWhat[j])
			{
				vResult[j] = (int)i;
				size_t k = j;
				while (k > nStart && vResult[k - 1] != -1) k--;
				if (k == nStart) nStart = j + 1;
				break;
			}
		}
	}
...
void fsDownloadsMgr::findDownloadsIndexes(DLDS_LIST_REF vDldsWhat, vmsDownloadList& vDldsWhere,
...
DWORD WINAPI fsDownloadsMgr::_threadWaitDelDlds(LPVOID lp)
{
	...
	*pcDlds = pThis->DeleteDownloads2(vDlds, bByUser, bDontConfirmFileDeleting, bDontDeleteFiles, &info->bNeedStop,
	                                  &info->iProgress);
...
DWORD fsDownloadsMgr::OnBeforeDownload(vmsDownloadSmartPtr dld)
{
	...
	if (bOK && (dld->dwFlags & DLD_CTREQ_HTML))
	{
		...
		if (strstr(pszType, "html") == NULL) bOK = FALSE;
	}
...
{
	vmsAUTOLOCKSECTION(m_csDeletedDownloads);
	return m_vDeletedDownloads.size() ? m_vDeletedDownloads.size() : m_vDeletedDownloads_tmp.size();
}
...
void fsDownloadsMgr::ReadDeletedSettings()
{
	...
	{
		for (size_t i = m_cMaxDeleted; i < m_vDeletedDownloads.size(); i++) v.push_back(m_vDeletedDownloads[i]);
	}

	vmsAUTOLOCKSECTION_UNLOCK(m_csDeletedDownloads);

	if (v.size()) DeleteDeletedDownloads(v, TRUE);
}
...
{
	if (dld->pMgr->GetDownloadMgr()) dld->pMgr->GetDownloadMgr()->CloseFile();
...
BOOL fsDownloadsMgr::AskToDeleteDownloadFile(vmsDownloadSmartPtr dld, BOOL bNoCancel,
                                             fsDeleteDownloadReaction& enDDRResult, bool& bApplyForAll)
{
	CDDRDlg dlg;

	dlg.m_bNoCancel = bNoCancel;
	if (dld->pMgr->GetDownloadMgr()) dlg.m_strUrl = dld->pMgr->get_URL();
	...
	if (dlg.m_bDontAskAgain) m_enDDR = enDDRResult;

	return TRUE;
}

void fsDownloadsMgr::GetDDRs(DLDS_LIST_REF vDlds, BOOL bDeleteCompletedFilesAlso, BOOL bNoCancel,
                             std::vector<fsDeleteDownloadReaction>& vResult)
{
	vResult.clear();

	if (vDlds.empty()) return;
	...
	for (size_t i = 0; i < vDlds.size(); i++)
	{
		...
		else if (enDDR == DDR_ASK && dld->pMgr->IsFileInitializedOnDisk())
		{
			...
			if (FALSE == AskToDeleteDownloadFile(dld, bNoCancel, enDDR, bApplyForAll)) return;
			if (bApplyForAll) enDefaultDDR = enDDR;
		}
...
void fsDownloadsMgr::ApplyDDRs(DLDS_LIST_REF vDlds, std::vector<fsDeleteDownloadReaction>& vDDRs, BOOL bNoCancel)
{
	...
	for (size_t i = 0; i < vDlds.size(); i++)
	{
		...
		if (enDDR == DDR_ASK) enDDR = enDDRforDDR_ASK;
		if (enDDR == DDR_ASK)
		{
			bool bApplyForAll = false;
			AskToDeleteDownloadFile(dld, bNoCancel, enDDR, bApplyForAll);
			if (bApplyForAll) enDDRforDDR_ASK = enDDR;
		}
...
void fsDownloadsMgr::ClearDeleted()
{
	...
	if (m_vDeletedDownloads.size() == 0) return;
...
int fsDownloadsMgr::DeleteDeletedDownloads(DLDS_LIST& vDlds, BOOL bNoCancel)
{
	...
	if (vDlds.size() < cMaxDlds) return DeleteDeletedDownloads2(&vDlds, bNoCancel, NULL, NULL);
...
int fsDownloadsMgr::DeleteDeletedDownloads2(DLDS_LIST* vDlds, BOOL bNoCancel, BOOL* pbNeedStop, int* piProgress)
{
	...
	DLDS_LIST v;
	v.reserve(m_vDeletedDownloads.size());
	for (i = 0; i < m_vDeletedDownloads.size(); i++)
	{
		if (m_vDeletedDownloads[i] != dldFake) v.push_back(m_vDeletedDownloads[i]);
	}
...
int fsDownloadsMgr::GetDeletedDownloadIndex(vmsDownloadSmartPtr dld)
{
	...
	{
		if (m_vDeletedDownloads[i] == dld) return i;
	}
...
int fsDownloadsMgr::RestoreDownloads(DLDS_LIST& vDlds)
{
	...
	if (vDlds.size() < cMaxDlds) return RestoreDownloads2(&vDlds, NULL, NULL);
...
int fsDownloadsMgr::RestoreDownloads2(DLDS_LIST* vDlds, BOOL* pbNeedStop, int* piProgress)
{
	...
	if (pbNeedStop && *pbNeedStop) return 0;
	...
	try
	{
		...
		for (i = 0; i < (UINT)vDlds->size(); i++)
		{
			...
			if (dld->pGroup->bAboutToBeDeleted) dld->pGroup = _DldsGrps.FindGroup(GRP_OTHER_ID);
	...
	if (cRestored) setNeedProcessDownloads();
...
{
	if (m_pfnEvents) m_pfnEvents(NULL, rec, ev, m_lpEventsParam);
}
...
void fsDownloadsMgr::Initialize()
{
	...
	DWORD dw;
	CloseHandle(CreateThread(NULL, 0, _threadDownloadsMgr, this, 0, &dw));
}
...
void fsDownloadsMgr::LaunchDownload(vmsDownloadSmartPtr dld, UINT nWaitForConfirmation)
{
	if (nWaitForConfirmation != 0)
	{
		...
		if (dld->pMgr->GetDownloadMgr()) fsGetFileName(dld->pMgr->get_OutputFilePathName(), szFile);
		CString strMsg = "\"";
		strMsg += szFile;
		strMsg += "\" ";
		if (dld->strComment != "") strMsg += '(';
		strMsg += dld->strComment;
		strMsg += ") ";
		strMsg += LS(L_WASSCHEDULEDTOLAUNCH);
		strMsg += "\n";
	...
	CString strFileName((LPCSTR)dld->pMgr->get_OutputFilePathName());
...
void fsDownloadsMgr::Shutdown()
{
	...
	for (size_t i = 0; i < m_vDownloads.size(); i++) UnregisterDownloadInTum(m_vDownloads[i]);
...
{
	if (dld->strComment.GetLength() == 0) return FALSE;

	CString str = dld->strComment;
	str.Replace("\r", "");

	str += "\n\n";
	str += LS(L_THISFILEWASDLDEDFROM);
	str += ":\n";
	if (dld->pMgr->GetDownloadMgr())
	{
	...
	CStdioFile file;
	CString strName((LPCSTR)dld->pMgr->get_OutputFilePathName());
...
void fsDownloadsMgr::MoveDownloads(DLDS_LIST_REF vDlds, vmsDownloadSmartPtr pDldPosition, bool bUp)
{
	...
	if (n != -1) nIndexPos = n;
	if (bUp == false) nIndexPos++;
	if (nIndexPos > m_vDownloads.size()) nIndexPos = m_vDownloads.size();

	for (i = 0; i < vDlds.size(); i++) DownloadsList_Insert(nIndexPos++, vDlds[i]);
}
...
void fsDownloadsMgr::ReorderDownloads(DLDS_LIST_REF vDlds, DLDS_LIST_REF vReqOrder, bool bDldsSorted)
{
	...
	if (vDlds.size() != vReqOrder.size()) return;
	...
	for (i = 0; i < vDlds.size(); i++)
	{
		...
		else
		{
			...
			if (nPrevIndx == m_vDownloads.size() - 1) nPrevIndx = (size_t)-1;
		}
	}
	...
	for (i = 0; i < vReqOrder.size(); i++)
	{
		...
		{
			if (dld == vDlds[j]) break;
		}
		if (j == vDlds.size()) continue;
		nOldIndx = vIndxs[j];
		if (nOldIndx == (size_t)-1) continue;
...
{
	if (dld->pMgr->GetDownloadMgr() == NULL) return FALSE;

	fsString strFile = dld->pMgr->get_OutputFilePathName();
	if (lstrlen(strFile) > 10 && lstrcmpi(strFile + lstrlen(strFile) - 9, ".metalink") != 0) return FALSE;

	vmsMetalinkFile mf;
	if (FALSE == mf.Parse(strFile)) return FALSE;

	if (mf.get_FileCount() == 0) return FALSE;
	...
	for (iFile = 0; iFile < mf.get_FileCount(); iFile++)
	{
		...
		if (file->strOS.GetLength() && strstr(file->strOS, "Windows") == NULL) continue;
		viFiles.push_back(iFile);
	}

	if (viFiles.size() == 0) viFiles.push_back(0);
	...
	for (iFile = 0; iFile < (int)viFiles.size(); iFile++)
	{
		...
		for (i = 0; i < file->vMirrors.size(); i++)
		{
			...
			{
				if (strcmpi(url->strProtocol, "bittorrent") == 0) strBtUrl = url->strUrl;
				continue;
			}

			if (bRecreated == FALSE)
			{
				if (IR_SUCCESS != dld->pMgr->GetDownloadMgr()->CreateByUrl(url->strUrl, TRUE)) continue;
				if (iFile == 0) dld->pMgr->GetDownloadMgr()->Reset();
				bRecreated = TRUE;
			}
			else
			{
		...
		if (bRecreated == FALSE)
		{
			if (strBtUrl.IsEmpty()) return FALSE;
		...
		if (iFile == 0) Event(dld, LS(L_METALINK_DETECTED), EDT_INQUIRY);

		for (i = 0; i < file->vHashes.size(); i++)
		{
			vmsMetalinkFile_File_Hash* hash = &file->vHashes[i];
			DWORD dwAlg;
			if (hash->strChecksum.GetLength() == 0) continue;

			if (lstrcmpi(hash->strAlgorithm, "md5") == 0)
				dwAlg = HA_MD5;

			else if (lstrcmpi(hash->strAlgorithm, "sha1") == 0 || lstrcmpi(hash->strAlgorithm, "sha-1") == 0)
				dwAlg = HA_SHA1;
		...
		if (dld->strComment.GetLength() != 0) dld->strComment += "\r\n";
...
void fsDownloadsMgr::DeletedDownloads_UseTmpList(BOOL bUse)
{
	...
	if (bUse)
	{
		...
		for (size_t i = 0; i < m_vDeletedDownloads.size(); i++) v.push_back(m_vDeletedDownloads[i]);
	...
	else
	{
		...
		for (iter = m_vDeletedDownloads_tmp.begin(); iter != m_vDeletedDownloads_tmp.end(); iter++) v.push_back(*iter);
		m_vDeletedDownloads = v;
...
BOOL fsDownloadsMgr::OnDownloadStoppedOrDone(vmsDownloadSmartPtr dld)
{
	...
	if (bDone)
	{
		...
		if (m_dldHighestPriority == dld) set_HighestPriorityDownload(NULL);

		Event(dld, DME_DOWNLOADSTOPPEDORDONE);
		Event(dld, DME_UPDATEDLDDIALOG);

		if (dld->pMgr->IsDescriptionFileShouldBeGenerated()) GenerateDescriptionFile(dld);
		...
		if (dld->dwFlags & DLD_DELETEFILEATRESTART) vmsDeleteFileAtWinBoot(dld->pMgr->get_OutputFilePathName());
...
void fsDownloadsMgr::OnDownloadDescEventRcvd(vmsDownloadSmartPtr dld, fsDownloadMgr_EventDescType enType,
                                             LPCSTR pszEvent)
{
	try
	{
		...
		if (enType == EDT_RESPONSE_E) dld->tstrLastErrMsg = pszEvent;

		if (IsStoringLogTurnedOn()) dld->setDirty();
...
void fsDownloadsMgr::GetRunningDownloads(DLDS_LIST& v)
{
	...
	for (size_t i = 0; i < m_vDownloads.size(); i++)
	{
		if (m_vDownloads[i]->pMgr->IsRunning()) v.push_back(m_vDownloads[i]);
	}
}

void fsDownloadsMgr::set_HighestPriorityDownload(vmsDownloadSmartPtr dld)
{
	if (m_dldHighestPriority == dld) return;

	if (m_dldHighestPriority != NULL) m_dldHighestPriority->pMgr->setPriority(0);

	m_dldHighestPriority = dld;

	if (dld) dld->pMgr->setPriority(BYTE_MAX);
}
...
int fsDownloadsMgr::IsSuchUrlExistsAlready(vmsDownloadSmartPtr dld)
{
	...
	if (dld->pMgr->GetDownloadMgr() == NULL) return 0;
	...
	for (size_t i = 0; i < nCount; i++)
	{
		...
		if (d->pMgr->GetDownloadMgr() == NULL) continue;
...
void fsDownloadsMgr::EventEx(vmsDownloadSmartPtr dld, LPCSTR pszEvent, fsDownloadMgr_EventDescType enType,
                             int nMaxCharsPerLine)
{
	...
	if (strEvent.IsEmpty() == FALSE) Event(dld, strEvent, enType);
}
...
{
	if (m_dwPDTimeLimit == 0) return true;
	...
	if (bNeedInvert) bTimeAllow = !bTimeAllow;
...
void fsDownloadsMgr::ChangeDownloadGroup(fsDownload* dld, vmsDownloadsGroupSmartPtr spGrp, bool bMoveFiles)
{
	if (dld->pGroup->nId != spGrp->nId)
	{
		...
		dld->setDirty();
		dld->pGroup->cDownloads++;
		if (bMoveFiles) dld->pMgr->GetDownloadMgr()->MoveToFolder(spGrp->strOutFolder);
	}
}
...
{
	if (dld->getState() & DLDS_REGISTERED_IN_TRAFFICUSGMGR) return;
