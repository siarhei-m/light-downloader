...
void fsTrayIconMgr::LoadIcons(UINT* pIcons, UINT cIcons)
{
	...
	for (UINT i = 0; i < cIcons; i++) m_pIcons[i] = SICOEX(pIcons[i], 16, 16);
}

void fsTrayIconMgr::Remove()
{
	if (m_pIcons == NULL) return;
...
BOOL fsTrayIconMgr::InitializeTrayIcon()
{
	...
	data.hWnd = m_hWnd;
	strcpy_s(data.szTip, sizeof(data.szTip), m_strTip);
...
{
	if (GetShell32Version() < 5) return FALSE;
	...
	strncpy_s(data.szInfo, sizeof(data.szInfo), pszInfo, _TRUNCATE);

	strncpy_s(data.szInfoTitle, sizeof(data.szInfoTitle), pszInfoTitle, _TRUNCATE);
...
void fsTrayIconMgr::setTip(LPCSTR pszTip)
{
	...
	data.uFlags = NIF_TIP;
	data.uID = m_nID;
	strcpy_s(data.szTip, sizeof(data.szTip), pszTip);
	m_strTip = pszTip;
