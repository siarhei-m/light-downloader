...
class CWaitForConfirmationDlg : public CDialog
{
  ...
  public:
	...
	void Init(LPCSTR pszMsg, UINT uWait = UINT_MAX, BOOL bShowDontAsk = FALSE, BOOL bNoIsDefault = FALSE,
	          LPCSTR pszDontAskAgainText = NULL, LPCSTR pszMsgIfDontAskAgain = NULL);
	CWaitForConfirmationDlg(CWnd* pParent = NULL);

	//{{AFX_DATA(CWaitForConfirmationDlg)
	enum
	{
		IDD = IDD_WAITFORCONFIRMATION
	};
