...
CDlg_Options_SiteMgr::CDlg_Options_SiteMgr(CWnd* pParent) : CDlg_Options_Page(CDlg_Options_SiteMgr::IDD, pParent)
{
...
{
	if (IDYES == MessageBox(LS(L_AREYOUSURE), LS(L_CONFIRMATION), MB_OK | MB_YESNO)) _SitesMgr.DeleteAllPasswords();
}
...
void CDlg_Options_SiteMgr::ApplyLanguage()
{
	fsDlgLngInfo lnginfo[] = {
	    fsDlgLngInfo(IDC__PASSWORDS, L_PASSWORDS),      fsDlgLngInfo(IDC_USE, L_USESMTOSTOREPWDS),
	    fsDlgLngInfo(IDC_ASK, L_ASKMETOSAVEPWDS),       fsDlgLngInfo(IDC_DONTASK, L_DONTASKMETOSAVEPWDS),
	    fsDlgLngInfo(IDC_AUTODEL, L_DELPWDSBEFOREEXIT), fsDlgLngInfo(IDC_DELALL, L_DELETEALLPWDS),
	};
