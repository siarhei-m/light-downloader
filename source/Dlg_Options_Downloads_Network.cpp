...
#include "Dlg_ConnectionSpeedInfo.h"
...
CDlg_Options_Downloads_Network::CDlg_Options_Downloads_Network(CWnd* pParent)
    : CDlg_Options_Page(CDlg_Options_Downloads_Network::IDD, pParent), m_uConnectionSpeedChoosenValue(UINT64_MAX)
{
...
BOOL CDlg_Options_Downloads_Network::OnInitDialog()
{
	...
	GetDlgItem(IDC_CONNECTIONSPEEDINFO)
	    ->EnableWindow(m_wndConnectionSpeed.GetCurSel() == (int)m_vConnectionSpeeds.size());
...
void CDlg_Options_Downloads_Network::FillConnectionSpeedCombo()
{
	...
	UINT64 uCurrentValue = m_uConnectionSpeedChoosenValue == UINT64_MAX
	                           ? _TumMgr.getConnectionBandwidthSpecifiedByUser(true)
	                           : m_uConnectionSpeedChoosenValue;
	int iSelection = -1;
	...
	for (size_t i = 0; i < m_vConnectionSpeeds.size(); i++)
	{
		...
		if (uVal == uCurrentValue) iSelection = (int)i;
	}
...
BOOL CDlg_Options_Downloads_Network::Apply()
{
	...
	if (!m_dlgTumSettingsPerDirection.SerializeSettings(true)) return FALSE;
...
void CDlg_Options_Downloads_Network::InsertValueIntoSortedList(std::vector<UINT64>& vValues, UINT64 uValue)
{
	...
	{
		if (vValues[i] == uValue) return;
	}
	if (vValues.size() > 20) vValues.erase(vValues.end() - 1);
	for (size_t i = 0; i < vValues.size(); i++)
	{
		if (vValues[i] > uValue)
		{
