...
class vmsFileDeleter
{
  ...
  public:
	vmsFileDeleter(LPCTSTR ptszFile, DWORD dwDeleteFileError = 0)
	    : m_dwLastError(dwDeleteFileError), m_pfnOnDeleteError(NULL)
	{
		if (ptszFile) m_tstrFile = ptszFile;
	}
	...
	bool DeleteFile()
	{
		...
		if (!b && GetLastError() == ERROR_ACCESS_DENIED)
		{
			...
			{
				if (GetLastError() == ERROR_SHARING_VIOLATION) m_dwLastError = ERROR_SHARING_VIOLATION;
			}
			else
			{
				CloseHandle(h);
	...
	bool DeleteFileEx()
	{
		...
		if (getErrorType() == FileIsInUse) return ScheduleFileDeleteAtReboot();
