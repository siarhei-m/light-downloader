...
struct fsString
{
	...
	LPCTSTR operator=(LPCTSTR pszStr)
	{
		...
		if (pszStr)
		{
			...
			if (pszString) _tcscpy(pszString, pszStr);
		}
	...
	LPCTSTR operator+=(LPCTSTR pszStr)
	{
		if (pszStr == NULL) return pszString;
	...
	BOOL operator==(const fsString& str) const { return str == (LPCTSTR)pszString; }

	BOOL operator!=(const fsString& str) const { return str != (LPCTSTR)pszString; }

	BOOL operator==(LPCTSTR pszStr) const
	{
		if (pszString == NULL || pszStr == NULL) return pszStr == pszString;
	...
	BOOL operator!=(LPCSTR pszStr) const { return !(*this == pszStr); }

	operator LPTSTR() const { return pszString; }
	...
	BOOL IsEmpty() const { return pszString == NULL || *pszString == 0; }

	char LastChar() const { return pszString ? pszString[GetLength() - 1] : (char)0; }
	...
	void Format(LPCSTR pszFormat...)
	{
		...
		vsprintf_s(psz, nLen, pszFormat, ap);
		va_end(ap);
		*this = psz;
		delete[] psz;
	}
