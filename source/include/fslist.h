...
template <class T> class list
{
  ...
  public:
	...
	BOOL save(HANDLE hFile)
	{
		...
		if (!WriteFile(hFile, &sz, sizeof(sz), &dw, NULL)) return FALSE;

		for (int i = 0; i < sz; i++)
		{
			if (!WriteFile(hFile, &at(i), sizeof(T), &dw, NULL)) return FALSE;
		}
	...
	BOOL load(HANDLE hFile)
	{
		...
		if (!ReadFile(hFile, &sz, sizeof(sz), &dw, NULL) || dw != sizeof(sz)) return FALSE;
		...
		{
			T data;
			if (!ReadFile(hFile, &data, sizeof(T), &dw, NULL) || dw != sizeof(T)) return FALSE;
	...
	{
		clear();
		for (int i = 0; i < l.size(); i++) add(l[i]);
