...
{
	CWnd::CreateEx(_theme.IsThemeActive() ? 0 : WS_EX_STATICEDGE, AfxRegisterWndClass(0, NULL, NULL, NULL), "",
	               WS_CHILD | WS_VISIBLE, CRect(0, 0, 0, 0), pwndParent, 0);
}

void CWnd_DownloadProgress::OnPaint()
{
	CPaintDC dc(this);
	if (m_dld != NULL) SetEvent(m_hevDraw);
}
...
DWORD WINAPI CWnd_DownloadProgress::_threadDraw(LPVOID lp)
{
	...
	while (WAIT_OBJECT_0 == WaitForMultipleObjects(2, phEvs, FALSE, INFINITE))
	{
		...
		if (pthis->m_iLastProgress != (int)(pthis->m_dld->pMgr->GetPercentDone() * 10) ||
		    pthis->m_bmpProgress.m_hObject == NULL)
		{
			CRect rc;
			...
			if (pthis->m_bmpProgress.m_hObject) pthis->m_bmpProgress.DeleteObject();
			...
			if (hTheme) _theme.CloseThemeData(hTheme);
