...
{
	return abs(GetRValue(clr1) - GetRValue(clr2)) < 10 && abs(GetGValue(clr1) - GetGValue(clr2)) < 10 &&
	...
	try
	{
		...
		for (int i = 0; i < cCols; i++)
		{
			...
			if (i == 0)
			{
				...
				if (item.state & LVIS_FOCUSED) bNeedBorder = TRUE;
				...
				if (item.state & LVIS_SELECTED)
				{
					...
					{
						if (GetSelectionMark() == (int)lpDraw->itemID) bNeedBorder = TRUE;
					}
				}
				...
				if (RGBIsEqual(clrBg, clrText)) clrText = (~clrText) & 0x00FFFFFF;
			...
			if (*item.pszText)
			{
				...
				if (bDrawText) dc->DrawText(szItem, &rcText, DT_SINGLELINE | DT_LEFT | DT_VCENTER | DT_NOPREFIX);
			}
...
{
	if (nm->hwndFrom == m_hWnd) OnRClick();
...
int CListCtrlEx::AddItem(LPCSTR pszItem, COLORREF clrBg, COLORREF clrText, int iImage, BOOL bAddToBeginning)
{
	...
	if (*pszItem) SetItemText(c, 0, pszItem);
...
void CListCtrlEx::SaveState(LPCSTR pszName)
{
	...
	_App.get_SettingsStore()->WriteProfileBinary(_T ("ListViews"), strIndexes, (LPBYTE)m_aIndex,
	                                             m_cTotalCols * sizeof(int));
	_App.get_SettingsStore()->WriteProfileBinary(_T ("ListViews"), strWidthes, (LPBYTE)piWidthes,
	                                             m_cTotalCols * sizeof(int));
...
void CListCtrlEx::ReadState(LPCSTR pszName)
{
	...
	if (_App.get_SettingsStore()->GetProfileBinary(_T ("ListViews"), strIndexes, &pbI, &uSizeI) &&
	    _App.get_SettingsStore()->GetProfileBinary(_T ("ListViews"), strWidthes, &pbW, &uSizeW) &&
	    uSizeI == m_cTotalCols * sizeof(int) && uSizeW == m_cTotalCols * sizeof(int))
	{
		...
		for (i = m_cTotalCols - 1; i >= 0; i--)
			if (m_aIndex[i] == -1) DeleteColumn(i);
		...
		for (i = 0; i < m_cTotalCols; i++)
		{
			if (piWidthes[i] > 0) SetColumnWidth(aOrder[m_aIndex[i]], piWidthes[i]);
		}
...
void CListCtrlEx::RebuildAIndex()
{
	...
	int i = 0;
	for (i = 0; i < m_cTotalCols; i++) m_aIndex[i] = -1;
...
{
	if (m_aIndex[iSubItem] != -1) CListCtrl::SetItemText(iItem, m_aIndex[iSubItem], pszText);
}
...
void CListCtrlEx::OnColumnclick(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	if (m_sortMode != LCSM_NONE && m_iSortCol == iSortCol)
	{
		...
		if (m_sortMode == LCSM_DESCENDING && (m_sortModeSupport & LCSM_DESCENDING_NOTSUPPORTED)) m_sortMode = LCSM_NONE;
	}
	else
	{
		m_iSortCol = iSortCol;
...
int CListCtrlEx::SubItemToSubItem(int nSubItem)
{
	for (int i = 0; i < m_cTotalCols; i++)
	{
		if (m_aIndex[i] == nSubItem) return i;
	}
