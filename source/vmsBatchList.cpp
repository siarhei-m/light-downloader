...
bool vmsBatchList::Create(LPCSTR pszTemplate, LPCSTR pszNumbers, char chStart, char chEnd)
{
	...
	if (pszNumbers == NULL || strstr(pszTemplate, "(*)") == NULL) return CreateAZBatch(pszTemplate, chStart, chEnd);
...
#define skip_digits                                                                                                    \
	while (*pszNumbers >= '0' && *pszNumbers <= '9') pszNumbers++;
#define skip_spaces                                                                                                    \
	...
	if (*pszNumbers == 0) return false;
		...
		else
		{
			...
			while (*pszNumbers && *pszNumbers != ',')
			{
				...
				case BATCHLIST_STEP_SYMBOL:
					nStep = nValue;
					break;

				case BATCHLIST_WILDCARD_SYMBOL:
					nWB = nValue;
					break;
				default:
					return false;
			...
			if (nEnd < nStart || nStep == 0) return false;
