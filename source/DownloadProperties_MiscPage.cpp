...
BOOL CDownloadProperties_MiscPage::OnInitDialog()
{
	...
	if (DP_EQ_MASK(dwFlags, DPF_DONTRESTARTIFNORESUME))
		CheckDlgButton(IDC_DONTRESTARTIFNORESUME,
		               dp0->dwFlags & DPF_DONTRESTARTIFNORESUME ? BST_CHECKED : BST_UNCHECKED);
	else
	...
	if (DP_EQ(enSCR, FALSE)) m_wndSCR.SetCurSel(dp0->enSCR);

	UINT nID[] = {IDC_FILENOTFOUND, IDC_ACCDENIED};
	int i = 0;
	for (i = 0; i < sizeof(nID) / sizeof(UINT); i++)
	{
		if (DP_EQ_EX(DP_OFFSET(aEP) + i * sizeof(fsDownloadFileErrorProcessing), sizeof(fsDownloadFileErrorProcessing),
		             FALSE))
	...
	for (i = m_pvDlds->size() - 1; i >= 0; i--)
	{
		...
		if (m_pvDlds->at(i)->pMgr->GetDownloadMgr()->IsRunning() ||
		    m_pvDlds->at(i)->pMgr->GetDownloadMgr()->GetDownloader()->GetNumberOfSections())
			m_bMayRename = FALSE;
	}
...
{
	if (IsDlgButtonChecked(IDC_RESERVESPACE) == BST_INDETERMINATE) CheckDlgButton(IDC_RESERVESPACE, BST_UNCHECKED);
...
{
	if (IsDlgButtonChecked(IDC_USEEXT) == BST_INDETERMINATE) CheckDlgButton(IDC_USEEXT, BST_UNCHECKED);
...
{
	if (IsDlgButtonChecked(IDC_FILENOTFOUND) == BST_INDETERMINATE) CheckDlgButton(IDC_FILENOTFOUND, BST_UNCHECKED);
...
{
	if (IsDlgButtonChecked(IDC_ACCDENIED) == BST_INDETERMINATE) CheckDlgButton(IDC_ACCDENIED, BST_UNCHECKED);
...
void CDownloadProperties_MiscPage::UpdateEnabled()
{
	...
	GetDlgItem(IDC_RENAMEATRESTART)->EnableWindow(m_wndAER.GetCurSel() == AER_RENAME);
}
...
BOOL CDownloadProperties_MiscPage::OnApply()
{
	...
	if (iCur != CB_ERR)
	{
		if (iCur == AER_RENAME && IsDlgButtonChecked(IDC_RENAMEATRESTART) == BST_CHECKED) iCur = AER_RENAME_2;
	...
	if (uChecked != BST_INDETERMINATE)
	{
		for (size_t i = 0; i < m_pvDlds->size(); i++)
		{
			...
			if (dw != DWORD(-1) && (dw & FILE_ATTRIBUTE_DIRECTORY) == 0 &&
			    dld->pMgr->GetDownloadMgr()->IsDone() == FALSE)
			{
...
{
	if (IsDlgButtonChecked(IDC_USECREATEEXT) == BST_INDETERMINATE) CheckDlgButton(IDC_USECREATEEXT, BST_UNCHECKED);
...
{
	if (IsDlgButtonChecked(IDC_USEHIDDENFLAG) == BST_INDETERMINATE) CheckDlgButton(IDC_USEHIDDENFLAG, BST_UNCHECKED);
...
{
	if (IsDlgButtonChecked(IDC_APPENDCOMMENT) == BST_INDETERMINATE) CheckDlgButton(IDC_APPENDCOMMENT, BST_UNCHECKED);
...
{
	if (IsDlgButtonChecked(IDC_LAUNCHWHENDONE) == BST_INDETERMINATE) CheckDlgButton(IDC_LAUNCHWHENDONE, BST_UNCHECKED);
...
{
	if (IsDlgButtonChecked(IDC_RETRFILEDATE) == BST_INDETERMINATE) CheckDlgButton(IDC_RETRFILEDATE, BST_UNCHECKED);
