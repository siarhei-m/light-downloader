...
#define WS_EX_NOACTIVATE 0x08000000L
	...
	VERIFY(CFrameWnd::CreateEx(
	    WS_EX_TOPMOST | WS_EX_TOOLWINDOW | dwLStyle,
	    AfxRegisterWndClass(CS_DBLCLKS, LoadCursor(NULL, IDC_ARROW), (HBRUSH)(COLOR_WINDOW + 1), NULL), NULL, WS_POPUP,
	    rc, NULL, 0));

	VERIFY(SetWindowPos(NULL, 0, 0, 0, 0, SWP_NOZORDER | SWP_NOMOVE | SWP_NOSIZE | SWP_SHOWWINDOW | SWP_NOACTIVATE));

	SetBlend(255);

	int n = _App.Notif_OnDoneWnds_Timeout();
	if (n) SetTimer(1, n * 1000, NULL);
...
{
	if (fsSysGetOsMajorVersion() >= 5) fsSetLayeredWindowAttributes(m_hWnd, 0, val, LWA_ALPHA);
}
...
void CWndDlDoneNotification::OnPaint()
{
	...
	SHFILEINFO fi;
	SHGetFileInfo(m_dld->pMgr->get_OutputFilePathName(), 0, &fi, sizeof(fi), SHGFI_ICON | SHGFI_SHELLICONSIZE);
...
void CWndDlDoneNotification::OnTimer(UINT_PTR nIDEvent)
{
	if (nIDEvent == 1)
	{
		KillTimer(1);
	...
	if (nIDEvent == 2)
	{
		...
		{
			if (_DldsMgr.isAutoDelCompletedDownloads()) m_bWasDeletedByAutoDel = true;
			if (!m_bWasDeletedByAutoDel) PostMessage(WM_CLOSE);
		}
	}
