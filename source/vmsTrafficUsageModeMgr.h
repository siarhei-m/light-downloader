...
class vmsTrafficUsageModeMgr
{
  public:
	...
	void SetTumRawSettingsPerDirection(fsTUM enTUM, vmsTrafficUsageModeRawSettings::PerDirection& stgs,
	                                   bool bForDownload);
  ...
  protected:
	...
	struct ManageForSpeedItemsList
	{
		...
		int findItemIndexByDownloader(vmsDownloaderWithNetworkUsageAdjustment* pDldr) const
		{
			...
			{
				if (vItems[i].pDldr == pDldr) return i;
			}
			return -1;
		}
	};
	...
	int findIndexOfItemsListExploringTotalSpeed() const
	{
		...
		{
			if (m_vpManageForSpeed[i]->totalSpeedExplorer.isExploring()) return i;
		}
		return -1;
	}
  ...
  protected:
	...
	struct NetworkStat
	{
		...
		vmsMaximumPossibleValueExplorer2<UINT64, 20, 25, 3 * 60, INT_MAX, TotalSpeedLog>
		    currentBandwidthExploreInLimitedMode;

		UINT64 tcTrafficGoesInHeavyModeFrom;
		UINT64 tcHeavyModeWorkedEnoughUntil;

		NetworkStat()
		    : uConnectionBandwidth(UINT64_MAX), uConnectionBandwidthByUser(UINT64_MAX), uCurrentBandwidth(UINT64_MAX),
		      currentBandwidthExplorer(&vTotalSpeed),
  ...
  protected:
	...
	void ApplySpeedLimitForItems(bool bForDownload, ManageForSpeedItemsList* pItems, UINT64 uLimit,
