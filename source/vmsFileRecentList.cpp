...
void vmsFileRecentList::Add(LPCSTR pszFileDispName, LPCSTR pszFilePathName, bool bDontQueryStoringStateInformation)
{
	...
	if (bNeedDel)
	{
		if (m_vList.size() > (size_t)m_nMaxEntries) m_vList.erase(m_vList.begin());
	}
...
BOOL vmsFileRecentList::Load(HANDLE hFile)
{
	...
	if (FALSE == ReadFile(hFile, &c, sizeof(c), &dw, NULL)) return FALSE;

	for (size_t i = 0; i < c; i++)
	{
		LPSTR pszDisp, pszPath;

		if (FALSE == fsReadStrFromFile(&pszDisp, hFile)) return FALSE;

		if (FALSE == fsReadStrFromFile(&pszPath, hFile)) return FALSE;
...
bool vmsFileRecentList::loadObjectItselfFromStateBuffer(LPBYTE pbtBuffer, LPDWORD pdwSize, DWORD dwVer)
{
	...
	if (!getVarFromBuffer(c, pbtCurrentPos, pbtBuffer, *pdwSize)) return false;

	for (size_t i = 0; i < c; i++)
	{
		LPSTR pszDisp, pszPath;

		if (!getStrFromBuffer(&pszDisp, pbtCurrentPos, pbtBuffer, *pdwSize)) return false;

		if (!getStrFromBuffer(&pszPath, pbtCurrentPos, pbtBuffer, *pdwSize)) return false;
