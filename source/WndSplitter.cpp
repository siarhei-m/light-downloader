...
BOOL CWndSplitter::Create(HINSTANCE hInst, HWND hWndParent, WndSplitterType enType)
{
	...
	WNDCLASS wc = {0,
	               ...
	               hInst,
	               NULL,
	               LoadCursor(NULL, m_type == WST_VERTICAL ? IDC_SIZEWE : IDC_SIZENS),
	...
	m_wndSplitter = CreateWindow(pszClassName, NULL, WS_CHILD | WS_VISIBLE, 0, 0, 5, 5, hWndParent, NULL, hInst, 0);

	SetWindowLongPtr(m_wndSplitter, GWLP_USERDATA, (LONG_PTR)this);
...
LRESULT CALLBACK CWndSplitter::_wndProc(HWND hWnd, UINT uMsg, WPARAM wp, LPARAM lp)
{
	...
	case WM_MOUSEMOVE:
		if (wp & MK_LBUTTON) pThis->OnMouseDrag(MAKEPOINTS(lp));
		return 0;
...
void CWndSplitter::ApplyRatio()
{
	...
	rcWnd2.right = rcParent.right;
	rcWnd2.bottom = rcParent.bottom;
	...
	case WST_VERTICAL:
		...
		rcWnd2.top = 0;
		rcSplit.top = 0;
		rcSplit.left = rcWnd1.right;
		rcSplit.right = rcWnd2.left;
		rcSplit.bottom = rcParent.bottom;
		break;
	...
	case WST_HORIZONTAL:
		...
		rcWnd2.left = 0;
		rcSplit.top = rcWnd1.bottom;
		rcSplit.left = 0;
		rcSplit.right = rcParent.right;
		rcSplit.bottom = rcWnd2.top;
		break;
	...
	MoveWindow(m_wnd1, rcWnd1.left, rcWnd1.top, rcWnd1.right - rcWnd1.left, rcWnd1.bottom - rcWnd1.top, TRUE);

	MoveWindow(m_wnd2, rcWnd2.left, rcWnd2.top, rcWnd2.right - rcWnd2.left, rcWnd2.bottom - rcWnd2.top, TRUE);

	MoveWindow(m_wndSplitter, rcSplit.left, rcSplit.top, rcSplit.right - rcSplit.left, rcSplit.bottom - rcSplit.top,
	           TRUE);
}
...
void CWndSplitter::OnMouseDrag(POINTS pt)
{
	...
	if (m_type == WST_VERTICAL)
	{
		if (Pt.x < rcParent.left + m_dim1min || Pt.x > rcParent.right - m_dim2min) return;
	...
	{
		if (Pt.y < rcParent.top + m_dim1min || Pt.y > rcParent.bottom - m_dim2min) return;
