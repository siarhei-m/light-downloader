...
CDownloadProperties_ConnectionPage::CDownloadProperties_ConnectionPage()
    : CPropertyPage(CDownloadProperties_ConnectionPage::IDD)
{
...
#define DP_SET(member, bString, NewVal)                                                                                \
	DPEntry_SetValue(m_pvDlds, DP_OFFSET(member), DP_SIZEOF(member), bString, NewVal)
...
BOOL CDownloadProperties_ConnectionPage::OnInitDialog()
{
	...
	UINT pID[] = {
	    IDC_PAUSEBETWEENSPIN,
	    IDC_MAXATTEMPTSSPIN,
	    IDC_SECTMAXCOUNTSPIN,
	...
	pBox->AddString(LS(L_B));
	pBox->AddString(LS(L_KB));
	pBox->AddString(LS(L_MB));

	pBox = (CComboBox*)GetDlgItem(IDC_TRAFFICLIMITDIM);
	CString str;

	str.Format("%s/%s", LS(L_B), LS(L_S));
	pBox->AddString(str);
	str.Format("%s/%s", LS(L_KB), LS(L_S));
	pBox->AddString(str);
	str.Format("%s/%s", LS(L_MB), LS(L_S));
	pBox->AddString(str);
	...
	if (DNP_EQ_MASK(dwFlags, DNPF_RESTARTSECTIONWHENSPEEDTOOLOW))
		CheckDlgButton(IDC_RESTARTIFSPEEDLOW,
		               dnp0->dwFlags & DNPF_RESTARTSECTIONWHENSPEEDTOOLOW ? BST_CHECKED : BST_UNCHECKED);
	else
...
{
	if (IsDlgButtonChecked(IDC_LIMITTRAFFIC) == BST_INDETERMINATE) CheckDlgButton(IDC_LIMITTRAFFIC, BST_UNCHECKED);
...
#define CHECK_RANGE(id, min)                                                                                           \
	if (val < min)                                                                                                     \
	{                                                                                                                  \
		ErrMustBeGreaterThan(id, min);                                                                                 \
		return FALSE;                                                                                                  \
	}

#define SET_VAL_EX(id, min, Value, Mul)                                                                                \
...
#define SET_VAL(id, min, Value) SET_VAL_EX(id, min, Value, 1)
	...
	if (nCheck == BST_CHECKED)
	{
		...
		if (w) DNP_SET(wLowSpeed_Duration, FALSE, &w);
	}
	else if (nCheck == BST_UNCHECKED)
	{
	...
	{
		if (m_pvDlds->at(i)->pMgr->IsRunning()) m_pvDlds->at(i)->pMgr->GetDownloadMgr()->ApplyProperties();
	}
...
{
	if (IsDlgButtonChecked(IDC_USEMAXATTEMPTS) == BST_INDETERMINATE) CheckDlgButton(IDC_USEMAXATTEMPTS, BST_UNCHECKED);
