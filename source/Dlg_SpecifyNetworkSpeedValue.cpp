...
CDlgSpecifyNetworkSpeedValue::CDlgSpecifyNetworkSpeedValue(CWnd* pParent)
    : CDialog(CDlgSpecifyNetworkSpeedValue::IDD, pParent), m_uValue(UINT64_MAX)
{
}
...
BOOL CDlgSpecifyNetworkSpeedValue::OnInitDialog()
{
	...
	if (bBits) CheckDlgButton(IDC_INBITS, BST_CHECKED);
...
{
	if (!vmsDialogHelper::GetDlgBytesGroup(this, IDC_VAL, IDC_DIM, &m_uValue)) return;
