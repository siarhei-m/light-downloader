...
CFdmApp theApp;
vmsAppMutex _appMutex(_T ("LightDownloader"));
...
BOOL CFdmApp::InitInstance()
{
	...
	{
		if (m_strAppPath[m_strAppPath.GetLength() - 1] != '\\' && m_strAppPath[m_strAppPath.GetLength() - 1] != '/')
			m_strAppPath += '\\';
	}
	...
	*p = 0;
	...
	if (CheckFdmStartedAlready(m_bForceSilentSpecified == FALSE)) return FALSE;
	...
	_App.View_ReadWndPlacement(pFrame, "MainFrm",
	                           bHidden ? fsAppSettings::RWPA_FORCE_SWHIDE_AND_KEEP_MINIMIZED_MAXIMIZED_STATE
	                                   : fsAppSettings::RWPA_NONE);

	if (!bHidden)
	{
		if (_App.Prg_StartMinimized())
		{
			if (IsWindowVisible(pFrame->m_hWnd)) pFrame->ShowWindow(SW_MINIMIZE);
		}
		else
		{
			pFrame->UpdateWindow();
			if (pFrame->IsWindowVisible()) pFrame->SetForegroundWindow();
		}
	}
...
BOOL CAboutDlg::OnInitDialog()
{
	...
	m_wndFirm.Init();
	m_wndFirm.SetUrl("https://github.com/siarhei-m/light-downloader");
		...
		if (bSetForIfEx)
		{
			...
			if (hWnd)
			{
				...
				else
				{
					...
					if (wc.showCmd == SW_HIDE) ShowWindow(hWnd, SW_RESTORE);
				}
...
void CFdmApp::CheckLocked()
{
	...
	do
	{
		...
		if (dwRes == ERROR_ALREADY_EXISTS) Sleep(100);
