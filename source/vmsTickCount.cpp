...
UINT64 vmsTickCount::GetTickCount64(void)
{
	...
	if (!_pfnGetTickCount64)
	{
		...
		if (hDll) _pfnGetTickCount64 = (FNGetTickCount64)GetProcAddress(hDll, "GetTickCount64");
		if (!_pfnGetTickCount64) _pfnGetTickCount64 = myGetTickCount64;
	}
...
UINT64 WINAPI vmsTickCount::myGetTickCount64(void)
{
	...
	if (dwResult < _dwPrevValue) _uCorrention += DWORD_MAX;
