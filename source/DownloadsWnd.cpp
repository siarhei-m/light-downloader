...
BOOL CDownloadsWnd::Create(CWnd* pParent)
{
	...
	if (FALSE ==
	    CWnd::Create(AfxRegisterWndClass(0, NULL, NULL, NULL), NULL, WS_CHILD | WS_VISIBLE, rc, pParent, 0x121))
		return FALSE;
...
int CDownloadsWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (!_DldsGrps.LoadFromDisk()) MessageBox(LS(L_ERRLOADGRPS), LS(L_ERR), MB_ICONERROR);

	if (FALSE == m_wndDownloads.Create(this)) return -1;

	if (FALSE == m_wndDeleted.Create(this)) return -1;

	if (FALSE == m_wndGroups.Create(this)) return -1;

	if (m_splitter.Create(AfxGetInstanceHandle(), m_hWnd) == FALSE) return -1;
...
void CDownloadsWnd::OnTimer(UINT_PTR)
{
	...
	try
	{
		for (int i = 0; i < cDownloads; i++)
		{
			...
			if (dld->pMgr->IsRunning()) bHasRunning = TRUE;
		}
	...
	if (bHasRunning && _App.PreventStandbyWhileDownloading()) SetThreadExecutionState(ES_SYSTEM_REQUIRED);
...
void CDownloadsWnd::OnDownloadProperties(DLDS_LIST& vDlds, CWnd* pwndParent)
{
	...
	{
		if (sheet.IsNeedUpdateTasks()) OnDownloadsGroupChanged();

		if (sheet.IsNeedProcessDownloads()) _DldsMgr.setNeedProcessDownloads();
	}
...
BOOL CDownloadsWnd::LoadDownloads()
{
	...
	if (retVal == FALSE)
	{
		if (IDCANCEL == MessageBox(LS(L_ERROR_LOADING_DOWNLOADS__CANCEL_TO_EXIT), NULL,
		                           MB_ICONERROR | MB_OKCANCEL | MB_SYSTEMMODAL))
...
{
	if (pGroup->nId == GRP_OTHER_ID) return FALSE;

	CString str = LS(L_AREYOUSURE);
	str += '\n';
	str += LS(L_THISDELGRPDLDS);
	if (IDNO == MessageBox(str, LS(L_CONFIRMATION), MB_ICONQUESTION | MB_YESNO)) return FALSE;

	std::vector<vmsDownloadsGroupSmartPtr> v;
	_DldsGrps.GetGroupWithSubgroups(pGroup, v);
	for (size_t i = 0; i < v.size(); i++) v[i]->bAboutToBeDeleted = true;

	if (FALSE == _DldsMgr.DeleteGroupsDownloads(pGroup, TRUE))
	{
		for (size_t i = 0; i < v.size(); i++) v[i]->bAboutToBeDeleted = false;
		return FALSE;
	}
...
DWORD CDownloadsWnd::_Events(fsDownload* dld, fsDLHistoryRecord* rec, fsDownloadsMgrEvent ev, LPVOID lp)
{
	...
	if (pthis->m_vDldsMgrEvents.size() == 1 && pthis->m_hWnd != NULL) pthis->PostMessage(WM_DW_DLDSMGR_EVENT);
...
void CDownloadsWnd::OnDownloadDefProperties()
{
	...
	vmsDownloadSmartPtr http, ftp, https; //, socks5;
...
UINT CDownloadsWnd::CreateDownload(LPCSTR pszStartUrl, BOOL bReqTopMostDialog, LPCSTR pszComment, LPCSTR pszReferer,
                                   BOOL bSilent, DWORD dwForceAutoLaunch, BOOL* pbAutoStart,
                                   vmsDWCD_AdditionalParameters* pParams, UINT* pRes)
{
	UINT res = IDOK;
	...
	if (pszStartUrl == NULL || *pszStartUrl == NULL)
	{
		...
		if (!bSilent)
		{
			...
			{
				fsURL url;
				if (url.Crack(strUrlTmp.c_str()) == IR_SUCCESS && *url.GetHostName()) pszStartUrl = strUrlTmp.c_str();
			}
		}
	}
	...
	if (FALSE == CreateDownloadWithDefSettings(dld, pszStartUrl)) return UINT_MAX;
	...
	if (bSilent == FALSE)
	{
		...
		if (pParams)
		{
			...
			if (pParams->dwMask & DWCDAP_COOKIES) dlg.m_pszCookies = pParams->strCookies;
			if (pParams->dwMask & DWCDAP_POSTDATA) dlg.m_pszPostData = pParams->strPostData;
			if (pParams->dwMask & DWCDAP_GROUP) dlg.m_pGroup = pParams->pGroup;
		}
		...
		if (res == IDOK)
		{
			...
			if (dlg.m_strUrl != pszStartUrl)
			{
				...
				{
					if (!pParams) pParams = new vmsDWCD_AdditionalParameters();
					pParams->strFileName = dlg.m_strFileName.GetString();
					if (!dlg.m_strOutFolder.IsEmpty()) pParams->strDstFolder = dlg.m_strOutFolder;
					pParams->dwFlags = 0;
					pParams->dwMask = DWCDAP_FILENAME;
				}

				// ORG
				// return CreateDownload(dlg.m_strUrl, bReqTopMostDialog, pszComment, pszReferer, bSilent,
				// dwForceAutoLaunch, pbAutoStart, pParams, pRes);
	...
	else
	{
		...
		if (pbAutoStart) dld->bAutoStart = *pbAutoStart;
		...
		BOOL bShowUI = TRUE;
		if (pParams && (pParams->dwMask & DWCDAP_FLAGS) && 0 != (pParams->dwFlags & DWDCDAP_F_NO_UI)) bShowUI = FALSE;

		if (pParams && (pParams->dwMask & DWCDAP_FLAGS) && (pParams->dwFlags & DWDCDAP_F_DONTCHECKURLALREADYEXISTS))
		{
	...
	if (pParams)
	{
		...
		if (pParams->dwMask & DWCDAP_GROUP)
		{
			...
			if (strDstFolder.IsEmpty()) strDstFolder = dld->pGroup->strOutFolder;
		}
		...
		if (pParams->dwMask & DWCDAP_FLAGS)
		{
			...
			if (pParams->dwFlags & DWDCDAP_F_PLACETOTOP) bPlaceToTop = true;

			if (pParams->dwFlags & DWDCDAP_F_NOGRPAUTOCHANGE) dld->dwFlags |= DLD_NOAUTOCHANGECURGRPFILTER;

			if (pParams->dwFlags & DWDCDAP_F_FORCEAUTOSTART) dld->bAutoStart = TRUE;
		}
...
void CDownloadsWnd::StopDownloadsInList()
{
	...
	for (i = 0; i < cDlds; i++) tasks->UpdateDownload(vDlds[i]);
}
...
void CDownloadsWnd::DeleteDownloadsInList()
{
	...
	{
		if (IDNO == MessageBox(LS(L_AREYOUSURE), LS(L_CONFIRMATION), MB_ICONQUESTION | MB_YESNO)) return;
	}
	...
	for (int i = 0; i < cDlds; i++) vDlds.push_back((fsDownload*)tasks->GetItemData(i));

	tasks->ShowWindow(SW_HIDE);
	if (m_bShowGroups) m_wndGroups.ShowWindow(SW_HIDE);

	DeleteDownloads(vDlds, TRUE, FALSE);

	tasks->ShowWindow(SW_SHOW);
	if (m_bShowGroups) m_wndGroups.ShowWindow(SW_SHOW);
}
...
void CDownloadsWnd::DeleteCompletedDownloadsInList()
{
	...
	{
		vmsDownloadSmartPtr dld = (fsDownload*)tasks->GetItemData(i);
		if (dld->pMgr->IsDone()) vDlds.push_back(dld);
	}

	if (vDlds.size() == 0) return;
...
void CDownloadsWnd::CreateDownloads(DLDS_LIST& vDlds, fsSchedule* task, BOOL bDontUseSounds, bool bPlaceToTop)
{
	...
	for (int i = 0; i < sz; i++)
	{
		...
		if (task) task->wts.dlds.pvIDs->add(uID);
	}

	_DldsMgr.UnlockList(false);

	if (task) _pwndScheduler->AddTask(task);
...
void CDownloadsWnd::UpdateTrayIconPlusOthers()
{
	...
	try
	{
		...
		for (size_t i = 0; i < nCount; i++)
		{
			...
			{
				bRun = TRUE;
				if (pMgr->GetDownloadingSectionCount() || pMgr->GetSpeed()) bRunOk = TRUE;
				if (pMgr->IsCantStart() == FALSE) bWaitRunOk = TRUE;
			}
			if (bRunOk) break;
		}
...
void CDownloadsWnd::ShowAllGroups(BOOL bShow)
{
	...
	if (bShow == FALSE) m_wndGroups.SelectItem(m_wndGroups.m_hAllGroups);
}
...
{
	if (GetCurrentFilter() && GetCurrentFilter()->GetType() == DFT_GROUP) ApplyCurrentFilter();
}
...
HWND CDownloadsWnd::Plugin_CreateMainWindow(HWND hParent)
{
	...
	if (!_pwndDownloads->Create(CWnd::FromHandle(hParent))) return NULL;
...
void CDownloadsWnd::Plugin_GetMenuImages(fsSetImage** ppImages, int* pcImages)
{
	static fsSetImage images[] = {
	    fsSetImage(ID_DLDCREATE, 0), fsSetImage(ID_DLDSTART, 1),    fsSetImage(ID_DLDSTOP, 2),
	    fsSetImage(ID_DLDMOVEUP, 4), fsSetImage(ID_DLDMOVEDOWN, 5), fsSetImage(ID_DLDPROPERTIES, 6),
...
void CDownloadsWnd::ApplyLanguageToMenuView(CMenu* menu)
{
	...
	UINT aCmds[] = {ID_DLLIST_1, ID_DLLIST_2, ID_DLLIST_3, ID_DLLIST_4, ID_DLLIST_5, ID_DLLIST_6,
	                ID_DLLIST_7, ID_DLLIST_8, ID_DLOG_1,   ID_DLOG_2,   ID_DLOG_3};
	LPCSTR apszCmds[] = {LS(L_FILENAME), LS(L_SIZE),  LS(L_DOWNLOADED), LS(L_TIMELEFT), LS(L_SECTIONS),   LS(L_SPEED),
	                     LS(L_COMMENT),  LS(L_ADDED), LS(L_TIME),       LS(L_DATE),     LS(L_INFORMATION)};

	for (UINT i = 0; i < sizeof(aCmds) / sizeof(UINT); i++)
...
void CDownloadsWnd::UpdateDLListColMenu(CMenu* menu)
{
	...
	for (int i = 0; i < 8; i++)
	{
		if (m_wndDownloads.m_tasks.IsColumnShown(i))
		{
			...
			if (bEn == FALSE) menu->EnableMenuItem(ID_DLLIST_1 + i, MF_GRAYED);
		}
	}
}
...
void CDownloadsWnd::UpdateDLLogColMenu(CMenu* menu)
{
	...
	for (int i = 0; i < 3; i++)
	{
		if (m_wndDownloads.m_info.Get_LogWindow()->IsColumnShown(i))
		{
			...
			if (bEn == FALSE) menu->EnableMenuItem(ID_DLOG_1 + i, MF_GRAYED);
		}
	}
}
	...
	try
	{
		...
		for (size_t i = 0; i < nCount; i++)
		{
			...
			if (filter->IsSatisfies(dld))
			{
				...
				if (dldSel == NULL) dldSel = dld;
			}
			if (pProgress) *pProgress = (int)((double)i / (double)_DldsMgr.GetCount() * 100);
		}

		_DldsMgr.UnlockList(true);

		if (dldSel != NULL) pList->SelectDownload(dldSel);
	}
	catch (const std::exception& ex)
	{
		ASSERT(FALSE);
...
{
	if (dld->pMgr == NULL) dld->setDownloadMgr(new vmsDownloadMgrEx);
	...
	if (IR_SUCCESS != dld->pMgr->GetDownloadMgr()->CreateByUrl(pszUrl, TRUE)) return FALSE;
	...
	if (pGroup == NULL) pGroup = _DldsGrps.FindGroup(GRP_OTHER_ID);
...
void CDownloadsWnd::ApplyDWWN(BOOL bRecalcSize)
{
	...
	if (bRecalcSize) m_splitter.ApplyRatio();
...
void CDownloadsWnd::SelectDownload(vmsDownloadSmartPtr dld)
{
	...
	if (filter == NULL || filter->IsSatisfies(dld) == FALSE) m_wndGroups.SetGroupFilter(dld->pGroup);
...
void CDownloadsWnd::FindDownloads_Start()
{
	...
	if (FALSE == m_finder.FindNext()) MessageBox(LS(L_THEREISNOITEMS), LS(L_FINDDONE), MB_ICONEXCLAMATION);
}

void CDownloadsWnd::FindDownloads_FindNext()
{
	if (FALSE == m_finder.FindNext()) MessageBox(LS(L_THEREISNOMOREITEMS), LS(L_FINDDONE), MB_ICONEXCLAMATION);
}
...
void CDownloadsWnd::RestartDownload(vmsDownloadSmartPtr dld, BOOL bSelThisDld)
{
	if (dld->pMgr->IsRunning() == FALSE)
	{
		...
		if (bSelThisDld) SelectDownload(dld);
	}
}
...
LRESULT CDownloadsWnd::OnDWCreateDldDialog(WPARAM wp, LPARAM lp)
{
	...
	if (_App.DownloadDialog_Use() == FALSE) return 2;
	...
	if (wp == 0)
		if (dld->dwFlags & (DLD_DONTSHOWDIALOG | DLD_BATCH)) return 1;
...
void CDownloadsWnd::DeleteDeadDownloadsInList()
{
	...
	for (int i = 0; i < cDlds; i++)
	{
		...
		{
			fsString strFile;
			if (dld->pMgr->GetDownloadMgr()) strFile = dld->pMgr->get_OutputFilePathName();
			if (GetFileAttributes(strFile) == DWORD(-1)) vDlds.push_back(dld);
		}
	}
...
void CDownloadsWnd::EndCreateDownloads()
{
	...
	if (m_vDownloadsHasBeenAdded.empty()) return;
...
void CDownloadsWnd::onDownloadsHasBeenAdded_imp(DLDS_LIST& vDlds, bool bPlaceToTop, BOOL bDontUseSounds)
{
	...
	BOOL bNeedShow = (vDlds[0]->dwFlags & (DLD_NOAUTOCHANGECURGRPFILTER)) == 0; // DLD_USEDBYHTMLSPIDER |
	...
	else
	{
		...
		if (bNeedShow) m_wndDownloads.m_tasks.SelectDownload(bPlaceToTop ? vDlds[0] : vDlds[vDlds.size() - 1]);
	}

	m_wndDownloads.m_tasks.UnlockList();

	if (bDontUseSounds == FALSE) _Snds.Event(SME_DOWNLOADADDED);
...
{
	return CreateDownload(pDld->strUrl, bReqTopMostDialog, pDld->strComment.IsEmpty() ? NULL : (LPCSTR)pDld->strComment,
	                      pDld->strReferer.IsEmpty() ? NULL : (LPCSTR)pDld->strReferer, pDld->bAddSilent,
	                      pDld->dwForceAutoLaunch, pDld->dwWhatIsValid & NDIV_AUTOSTART ? &pDld->bAutoStart : NULL,
...
void CDownloadsWnd::onEvents(fsDownload* dld, fsDLHistoryRecord* rec, fsDownloadsMgrEvent ev, int iDldLogEntry)
{
	...
	try
	{
		...
		case DME_DLMGRTHREADSTARTING:
			if (m_bDeletingManyDownloads == false) ((CMainFrame*)AfxGetApp()->m_pMainWnd)->RebuidDownloadsList();
			break;
		...
		case DME_DLDRESTORED:
			...
			if (filter)
			{
				if (filter->IsSatisfies(dld)) m_wndDownloads.m_tasks.AddDownloadToList(dld, FALSE);
			}
			bUpdateTIPO = FALSE;
			break;
		...
		if (bApplyNotGroupFilterForDownload) m_wndGroups.ApplyNotGroupFilterForDownload(dld);

		if (bUpdateTIPO) UpdateTrayIconPlusOthers();
	}
	catch (const std::exception& ex)
	{
		ASSERT(FALSE);
