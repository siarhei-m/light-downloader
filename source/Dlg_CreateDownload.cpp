    ...
    : CDialog(CCreateDownloadDlg::IDD, pParent), m_dld(dld)
{
	...
	if (!dld->strComment.IsEmpty()) m_strComment = dld->strComment;
	m_iAutostart = 0;
	m_bThread = FALSE;
	m_bNeedExit = FALSE;
...
void CCreateDownloadDlg::OnOK()
{
	...
	{
		if (FALSE == _CheckFolderName(this, IDC_OUTFOLDER)) return;
		...
		if (IsDlgButtonChecked(IDC_FILEAUTO) == BST_UNCHECKED)
		{
			...
			if (FALSE == _CheckFileName(this, IDC_SAVEAS)) return;
	...
	if (nDldType != 1) _LastFolders.AddRecord(strOutFolder);
	_LastUrlFiles.AddRecord(m_strUrl);

	if (strOutFolder[strOutFolder.GetLength() - 1] != '\\' && strOutFolder[strOutFolder.GetLength() - 1] != '/')
		strOutFolder += '\\';
...
BOOL CCreateDownloadDlg::OnInitDialog()
{
	...
	int i = 0;
	for (i = 0; i < _LastUrlFiles.GetRecordCount(); i++) pUrls->AddString(_LastUrlFiles.GetRecord(i));

	CComboBox* pDirs = (CComboBox*)GetDlgItem(IDC_OUTFOLDER);
	for (i = 0; i < _LastFolders.GetRecordCount(); i++) pDirs->AddString(_LastFolders.GetRecord(i));
	...
	if (IR_SUCCESS == url.Crack(m_strUrl) && *url.GetHostName())
	{
		...
		if (*url.GetUserName()) m_bAuthChanged = TRUE;
	}

	Update_User_Password();

	SetDlgItemText(IDC_URL, m_strUrl == "https://url/" ? CString("https://") : m_strUrl);
	...
	if (m_pGroup == NULL) m_pGroup = _DldsGrps.FindGroup(_App.NewDL_GroupId());
...
BOOL CCreateDownloadDlg::ReadDNP()
{
	...
	if (m_bUrlChanged)
	{
		...
		// new
		if (lstrcmpi(m_strUrl, "https://") == 0 || lstrcmpi(m_strUrl, "http://") == 0 ||
		    lstrcmpi(m_strUrl, "ftp://") == 0)
		{
			WrongURL();
			return FALSE;
		}
...
void CCreateDownloadDlg::UrlChanged()
{
	...
	*szFile = 0;
	...
	if (IR_SUCCESS != url.Crack(m_strUrl)) return;

	if (m_bGroupChanged == FALSE)
	{
		fsFileNameFromUrlPath(url.GetPath(), url.GetInternetScheme() == INTERNET_SCHEME_FTP, TRUE, szFile,
		                      sizeof(szFile));
		...
		if (grp == NULL) grp = _DldsGrps.FindGroup(GRP_OTHER_ID);

		if (grp != NULL)
		{
			if (m_wndGroups.SelectGroup(grp)) OnChangeGroups();
		}
	}

	//	fsSiteInfo *site = _SitesMgr.FindSite (url.GetHostName (), fsNPToSiteValidFor (fsSchemeToNP
	//(url.GetInternetScheme ()))); 	if (site)
	//	{
...
void CCreateDownloadDlg::ApplyLanguage()
{
	fsDlgLngInfo lnginfo[] = {
	    fsDlgLngInfo(IDC_USELOGIN, L_AUTHORIZATION), fsDlgLngInfo(IDC__USER, L_USER),
	    fsDlgLngInfo(IDC__PASSWORD, L_PASSWORD),     fsDlgLngInfo(IDC__SAVETO, L_SAVETO),
	    fsDlgLngInfo(IDC__SAVEAS, L_SAVEAS),         fsDlgLngInfo(IDC__START, L_START),
	    fsDlgLngInfo(IDC_STARTNOW, L_AUTOMATICALLY), fsDlgLngInfo(IDC_STARTMANUAL, L_MANUALLY),
	    fsDlgLngInfo(IDC_STARTSCHEDULE, L_SCHEDULE), fsDlgLngInfo(IDC__GROUP, L_GROUP),
	    fsDlgLngInfo(IDC_ADVANCED, L_ADVANCED),      fsDlgLngInfo(IDCANCEL, L_CANCEL),
	    fsDlgLngInfo(IDC_QSIZE, L_QUERYSIZE),        fsDlgLngInfo(IDC_FILEAUTO, L_FILEAUTO),
	    fsDlgLngInfo(IDC__DLDTYPE, L_DLDTYPE),       fsDlgLngInfo(IDC_PLACEATTOP, L_PLACEDLDATTOP),
	};

	CString str = LS(L_COMMENT);
	str += ':';
...
int CCreateDownloadDlg::_CheckDownloadAlrExists(vmsDownloadSmartPtr dld, BOOL bNeedTopMost, BOOL bShowUI,
                                                CCreateDownloadDlg* const cddlg)
{
	fsURL url1, url2;
...
{
	if (pszUser == NULL || *pszUser == 0) return NULL;
		...
		if (site == NULL || site->strUser != pszUser || site->strPassword != pszPwd)
		{
			...
			if (IDOK == _DlgMgr.DoModal(&dlg))
			{
				...
				else
				{
					...
					_site->pGroup = _DldsGrps.FindGroup(GRP_OTHER_ID);
					_site->dwFtpFlags = FTP_USEPASSIVEMODE;
					_site->cConnsNow = 0;
					_site->cMaxConns = UINT_MAX;
					_site->dwValidFor = fsNPToSiteValidFor(np) | SITE_VALIDFOR_SUBDOMAINS;
					_site->strUser = pszUser;
					_site->strPassword = pszPwd;
					_site->bTemp = FALSE;
...
BOOL CCreateDownloadDlg::_SetDownloadOutputFolderAsDefault(CWnd* pwndParent, LPCSTR pszFolder,
                                                           vmsDownloadsGroupSmartPtr pGroup)
{
	...
	dlg.m_strText.Format(LS(L_OKTOSETFOLDERASDEFFORGRP), pGroup->strName.pszString);

	dlg.m_strTitle = LS(L_CONFIRMATION);

	dlg.m_bChecked = FALSE != _App.View_SetOutputFolderAsDefForAllGrpsChecked();

	if (IDC_BTN1 != _DlgMgr.DoModal(&dlg)) return FALSE;

	_App.View_SetOutputFolderAsDefForAllGrpsChecked(dlg.m_bChecked);

	CString strFolder;
	if (pszFolder[lstrlen(pszFolder) - 1] != '\\' && pszFolder[lstrlen(pszFolder) - 1] != '/')
	{
		strFolder = pszFolder;
		strFolder += '\\';
		pszFolder = strFolder;
...
BOOL CCreateDownloadDlg::_CheckFolderName(CDialog* pdlg, UINT nIdCtrl)
{
	...
	pdlg->GetDlgItemText(nIdCtrl, str);
	LPCSTR pszInvChars = ":*?\"<>|";
	if (str.GetLength() > 2 && str[1] == ':') str.Delete(1);
	if (str.FindOneOf(pszInvChars) != -1)
	{
