...
CString fsAppSettings::Agent()
{
	...
	if (str == "") str = vmsFdmAppMgr::getAppAgentName();
	if (str == "FDM 1.x") str = vmsFdmAppMgr::getAppAgentName();
	return str;
}

void fsAppSettings::Agent(LPCSTR psz)
{
	if (lstrcmpi(psz, vmsFdmAppMgr::getAppAgentName()) == 0) psz = "";
...
{
	m_stgs.WriteProfileBinary(_T ("Network"), _T ("TUM"), (LPBYTE)aTUM, 3 * sizeof(vmsTrafficUsageModeRawSettings));
}
...
void fsAppSettings::View_ReadWndSize(CWnd* pWnd, LPCSTR pszName)
{
	...
	if (cx > GetSystemMetrics(SM_CXSCREEN)) cx = 0;
	if (cy > GetSystemMetrics(SM_CYSCREEN)) cy = 0;

	if (cx && cy) pWnd->MoveWindow(0, 0, cx, cy);
}
...
void fsAppSettings::View_SaveWndPlacement(CWnd* pWnd, LPCSTR pszName)
{
	WINDOWPLACEMENT wp;
	wp.length = sizeof(wp);
	if (FALSE == pWnd->GetWindowPlacement(&wp)) return;
	if (wp.showCmd == 0) return;
...
void fsAppSettings::View_ReadWndPlacement(CWnd* pWnd, LPCSTR pszName, ReadWndPlacementAdjustment enRWPA)
{
	...
	if (m_stgs.GetProfileBinary(_T ("Windows"), pszName, &lp, &uSize))
	{
		...
		CRect rcIntersect;
		if (!IntersectRect(&rcIntersect, &rcWA, &wp->rcNormalPosition) || rcIntersect.Width() < 50 ||
		    rcIntersect.Height() < 50)
		{
		...
		if (bHide) pWnd->ShowWindow(SW_HIDE);
	}
	else
	{
		pWnd->ShowWindow(enRWPA == RWPA_FORCE_SWHIDE || enRWPA == RWPA_FORCE_SWHIDE_AND_KEEP_MINIMIZED_MAXIMIZED_STATE
		                     ? SW_HIDE
		                     : SW_SHOW);
	}
}
...
{
	m_stgs.WriteProfileBinary(_T ("Find"), _T ("t1"), (LPBYTE)&t, sizeof(t));
}
...
{
	m_stgs.WriteProfileBinary(_T ("Find"), _T ("t2"), (LPBYTE)&t, sizeof(t));
}
...
{
	m_stgs.WriteProfileBinary(_T ("Find"), _T ("s1"), (LPBYTE)&u, sizeof(u));
}

void fsAppSettings::Find_FileSizeTo(UINT64 u)
{
	m_stgs.WriteProfileBinary(_T ("Find"), _T ("s2"), (LPBYTE)&u, sizeof(u));
}
...
{
	return m_stgs.GetProfileInt(_T ("Network"), _T ("Flags"), DPF_USEHIDDENATTRIB | DPF_DONTRESTARTIFNORESUME);
}
...
void fsAppSettings::SaveStringList(LPCSTR pszSubKey, LPCSTR pszValue, CStringList& v)
{
	int lt = 0;
	int i = 0;
	for (i = 0; i < v.GetCount(); i++) lt += v.GetAt(v.FindIndex(i)).GetLength();
	lt += v.GetCount() + 1;
...
BOOL fsAppSettings::LoadStringList(LPCSTR pszSubKey, LPCSTR pszValue, CStringList* v)
{
	...
	if (FALSE == m_stgs.GetProfileBinary(pszSubKey, pszValue, &pb, &lt)) return FALSE;
	if (pb == NULL || lt == 0) return FALSE;
...
{
	if (m_bDontSaveLogs == BOOL(-1)) m_bDontSaveLogs = m_stgs.GetProfileInt(_T ("General"), _T ("DontSaveLogs"), TRUE);
...
void fsAppSettings::Scheduler_LastTask_read(fsSchedule* task)
{
	...
	if (FALSE == m_stgs.GetProfileBinary("Scheduler", "LastTask", &pb, &uSize) || uSize != sizeof(*task))
	{
		task->wts.enType = WTS_PROGRAM;

		CString strProg = Scheduler_LastTask_ProgName(), strArgs = Scheduler_LastTask_ProgArgs();
...
{
	return (vmsNewGroupSelectWay)m_stgs.GetProfileInt(_T ("NewDownload"), _T ("GroupSW"), NGSW_CHOOSE_BY_EXT);
}

void fsAppSettings::NewGrp_SelectWay(vmsNewGroupSelectWay en)
{
	m_stgs.WriteProfileInt(_T ("NewDownload"), _T ("GroupSW"), en);
}
...
{
	if (IS_PORTABLE_MODE == FALSE) return;

	static HANDLE _hmxDisableBrMenu = NULL;
	// if (Monitor_IEMenu_Enable () == FALSE)
	//{
	if (_hmxDisableBrMenu == NULL) _hmxDisableBrMenu = CreateMutex(NULL, FALSE, "mx::fdm: brmenu_disable");
	//}
	...
	//{
	if (_hmxDisableDlLink == NULL) _hmxDisableDlLink = CreateMutex(NULL, FALSE, "mx::fdm: dllink_disable");
	//}
	...
	//{
	if (_hmxDisableDlAll == NULL) _hmxDisableDlAll = CreateMutex(NULL, FALSE, "mx::fdm: dlall_disable");
	//}
	...
	//{
	if (_hmxDisableDlSelected == NULL) _hmxDisableDlSelected = CreateMutex(NULL, FALSE, "mx::fdm: dlselected_disable");
	//}
...
UINT64 fsAppSettings::ConnectionBandwidth(bool bForDownload)
{
	return m_stgs.GetProfileUint64(_T ("Network"),
	                               bForDownload ? _T ("ConnectionDownloadBandwidth") : _T ("ConnectionUploadBandwidth"),
	                               UINT64_MAX);
}

void fsAppSettings::ConnectionBandwidth(bool bForDownload, UINT64 uVal)
{
	m_stgs.WriteProfileUint64(
	    _T ("Network"), bForDownload ? _T ("ConnectionDownloadBandwidth") : _T ("ConnectionUploadBandwidth"), uVal);
}
...
void fsAppSettings::GetConnectionBandwidthValues(bool bForDownload, std::vector<UINT64>& v)
{
	...
	if (!m_stgs.GetProfileBinary(_T ("Network"),
	                             bForDownload ? _T ("ConnectionDownloadBandwidthValues")
	                                          : _T ("ConnectionUploadBandwidthValues"),
	                             &pb, &uSize))
		return;
	UINT64* pu = (UINT64*)pb;
...
void fsAppSettings::SetConnectionBandwidthValues(bool bForDownload, const std::vector<UINT64>& v)
{
	m_stgs.WriteProfileBinary(_T ("Network"),
	                          bForDownload ? _T ("ConnectionDownloadBandwidthValues")
	                                       : _T ("ConnectionUploadBandwidthValues"),
	                          (LPBYTE)&v[0], v.size() * sizeof(UINT64));
}
