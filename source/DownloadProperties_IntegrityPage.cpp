...
CDownloadProperties_IntegrityPage::CDownloadProperties_IntegrityPage()
    : CPropertyPage(CDownloadProperties_IntegrityPage::IDD)
{
...
BOOL CDownloadProperties_IntegrityPage::OnInitDialog()
{
	...
	if (DP_EQ(enICFR, FALSE)) m_wndICFR.SetCurSel(dp0->enICFR);
	...
	{
		UINT anIds[] = {IDC__CHECKSUMOFTHISFILE, IDC_CHECKSUM, IDC__ALGORITHM, IDC_ALGORITHM};

		for (int i = 0; i < sizeof(anIds) / sizeof(UINT); i++) GetDlgItem(anIds[i])->ShowWindow(SW_HIDE);
	}
...
void CDownloadProperties_IntegrityPage::MakeChecksumValid(CString& str)
{
	...
	while (*psz)
	{
		if ((*psz >= '0' && *psz <= '9') || (*psz >= 'a' && *psz <= 'f')) str2 += *psz;
