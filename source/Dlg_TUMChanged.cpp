...
CDlg_TUMChanged::CDlg_TUMChanged(CWnd* pParent) : CDialog(CDlg_TUMChanged::IDD, pParent)
{
...
{
	if (IsDlgButtonChecked(IDC_DONTSHOW) == BST_CHECKED) _App.View_TUMChanged(FALSE);
...
HBRUSH CDlg_TUMChanged::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
	...
	if (pWnd->m_hWnd == GetDlgItem(IDC__HEAVY)->m_hWnd || pWnd->m_hWnd == GetDlgItem(IDC__MEDIUM)->m_hWnd ||
...
void CDlg_TUMChanged::ApplyLanguage()
{
	...
	CString str = LS(L_TUMCHANGED_TOP);
	str.Replace("\\n", "\n");
