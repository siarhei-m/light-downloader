...
class CDownloadsWnd : public CWnd
{
  ...
  public:
	...
	void CreateDownload(vmsDownloadSmartPtr dld, struct fsSchedule* task = NULL, BOOL bDontUseSounds = FALSE,
	...
	void CreateDownloads(DLDS_LIST& vDlds, struct fsSchedule* task, BOOL bDontUseSounds = FALSE,
	                     bool bPlaceToTop = false);
	...
	UINT CreateDownload(LPCSTR pszStartUrl, BOOL bReqTopMostDialog = FALSE, LPCSTR pszComment = NULL,
	                    LPCSTR pszReferer = NULL, BOOL bSilent = FALSE,
	                    DWORD dwForceAutoLaunch = DWCD_NOFORCEAUTOLAUNCH, BOOL* pbAutoStart = NULL,
