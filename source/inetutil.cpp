...
{
	char szURL[10000];
	strcpy_s(szURL, sizeof(szURL), "ftp://f.c");
	if (*pszPath != '/' && *pszPath != '\\') strcat_s(szURL, sizeof(szURL), "/");
	strcat_s(szURL, sizeof(szURL), pszPath);

	fsURL url;
	if (IR_SUCCESS == url.Crack(szURL))
	{
		if (*pszPath != '/' && *pszPath != '\\')
			strcpy_s(pszPath, 10000, url.GetPath() + 1);
		else
			strcpy_s(pszPath, 10000, url.GetPath());
	}
}
...
BOOL fsFilePathFromUrlPath(LPCSTR pszUrl, BOOL bUsingFTP, BOOL bDecode, LPSTR pszBuf, UINT uSize)
{
	*pszBuf = 0;
	...
	if (*pszUrl == 0) return TRUE;

	int end = bUsingFTP ? strlen(pszUrl) - 1 : strcspn(pszUrl, "?=#") - 1;

	if (end >= (int)uSize) return FALSE;

	CopyMemory(pszBuf, pszUrl, end + 1);
	pszBuf[end + 1] = 0;

	if (bUsingFTP == FALSE && bDecode) DecodeURLPath(pszBuf);
...
{
	fsURL url;
	if (url.Crack(pszFullUrl) != IR_SUCCESS) return FALSE;

	if (url.GetInternetScheme() != INTERNET_SCHEME_HTTP && url.GetInternetScheme() != INTERNET_SCHEME_FTP) return FALSE;

	LPCSTR pszPath = url.GetPath();
	LPCSTR pszA = strchr(pszPath, '#');

	if (pszA == NULL) return FALSE;
	...
	if (ppszWithoutAnchor)
	{
		...
		if (ppszAnchor) *ppszAnchor = szUrl + oldlen - oldpathlen + len;
	}
...
BOOL fsFileNameFromUrlPath(LPCSTR pszUrl, BOOL bUsingFTP, BOOL bDecode, LPSTR pszBuf, UINT uSize)
{
	*pszBuf = 0;
	...
	while (pos && pszUrl[pos] != '/' && pszUrl[pos] != '\\') pos--;

	if (pszUrl[pos] != '/' && pszUrl[pos] != '\\') return FALSE;

	if (UINT(len - pos) > uSize) return FALSE;

	strcpy_s(pszBuf, uSize, pszUrl + pos + 1);
...
BOOL fsIRToStr(fsInternetResult ir, LPSTR pszStr, UINT uMaxSize)
{
	...
	if (strlen(pszDesc) >= uMaxSize) return FALSE;

	strcpy_s(pszStr, uMaxSize, pszDesc);
...
void fsDecodeHtmlUrl(fsString& str)
{
	...
	for (int i = 0; i < len;)
	{
		...
		else if (str[i] == '&' && str[i + 1] == '#')
		{
			CString strC;
			int j = i + 2;
			while (isdigit(str[j])) strC += str[j++];
			if (str[j] == ';')
			{
