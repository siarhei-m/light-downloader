...
void CSchedule_HTSPage::UpdateEnabled()
{
	...
	for (int i = 0; i < sizeof(nIDs) / sizeof(UINT); i++) GetDlgItem(nIDs[i])->EnableWindow(b);
...
void CSchedule_HTSPage::DataToDialog()
{
	...
	case HTS_DAILY:
		int i;
		for (i = 0; i < 7; i++) CheckDlgButton(nIDs[i], m_task->hts.aWeeklyDays[i] ? BST_CHECKED : BST_UNCHECKED);
		m_wndTime.SetTime(&time);
		break;
...
void CSchedule_HTSPage::ApplyLanguage()
{
	fsDlgLngInfo lnginfo[] = {
	    fsDlgLngInfo(IDC_STARTTIME, L_TIME),      fsDlgLngInfo(IDC_ONCE, L_ONCE),
	    fsDlgLngInfo(IDC_DAILY, L_INCERTAINDAYS), fsDlgLngInfo(IDC_SUN, L_SUNDAY),
	    fsDlgLngInfo(IDC_MON, L_MONDAY),          fsDlgLngInfo(IDC_TUE, L_TUESDAY),
	    fsDlgLngInfo(IDC_WED, L_WEDNESDAY),       fsDlgLngInfo(IDC_THU, L_THURSDAY),
	    fsDlgLngInfo(IDC_FRI, L_FRIDAY),          fsDlgLngInfo(IDC_SAT, L_SATURDAY),
	    fsDlgLngInfo(IDC_ONEVERY, L_EVERY),       fsDlgLngInfo(IDC_ONEVENT, L_ONEVENT),
	    fsDlgLngInfo(IDC__DURING, L_DURING),      fsDlgLngInfo(IDC__MINUTES, L_MIN),
