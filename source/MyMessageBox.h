...
class CMyMessageBox : public CDialog
{
  ...
  public:
	...
	CString m_strText;
	CString m_strTitle;
	CString m_strBtn1Text, m_strBtn2Text, m_strBtn3Text;
	HICON m_hIcon;
...
inline UINT MyMessageBox(CWnd* pwndParent, LPCSTR pszText, LPCSTR pszTitle, LPCSTR pszCheckbox,
                         LPCSTR pszSysIcon = NULL, LPCSTR pszBtn1Text = "OK", LPCSTR pszBtn2Text = NULL,
                         LPCSTR pszBtn3Text = NULL, bool bTopMost = false)
{
	CMyMessageBox mb(pwndParent);

	if (pszSysIcon) mb.m_hIcon = LoadIcon(NULL, pszSysIcon);
	...
	if (pszBtn2Text)
	{
		...
		if (pszBtn3Text) mb.m_strBtn3Text = pszBtn3Text;
	}

	if (pszCheckbox) mb.m_strCheckBoxText = pszCheckbox;
	...
	if (mb.m_bChecked) nRet |= 0x00010000;
