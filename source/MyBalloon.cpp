...
void CMyBalloon::Create(CWnd* pwndParent, bool bTrayBalloon, BYTE byteTrancparency)
{
	...
	if (bTrayBalloon) _TrayMgr.TestIcon();
	...
	{
		CRect rc(0, 0, 0, 0);
		CWnd::CreateEx(0x80000, AfxRegisterWndClass(0, LoadCursor(NULL, IDC_ARROW), NULL, NULL), NULL, WS_POPUP, rc,
		               pwndParent, 0);
	...
	{
		CRect rc(0, 0, 0, 0);
		CWnd::Create(AfxRegisterWndClass(0, LoadCursor(NULL, IDC_ARROW), NULL, NULL), "", WS_POPUP, rc, pwndParent, 0);
	}

	CRect rc;
	m_pContent->Create(AfxRegisterWndClass(0, LoadCursor(NULL, IDC_ARROW), NULL, NULL), NULL, WS_CHILD | WS_VISIBLE, rc,
	                   this, 0);

	SetupWindowRgn();

	SetWindowPos(NULL, 0, 0, 0, 0, SWP_NOZORDER | SWP_NOMOVE | SWP_NOSIZE | SWP_SHOWWINDOW | SWP_NOACTIVATE);
}
...
void CMyBalloon::SetupWindowRgn()
{
	...
	m_bLeaderAtRight = false;
	m_bLeaderAtTop = false;
	if (m_ptAnchor.x + rcWnd.Width() > cxScreen) m_bLeaderAtRight = true;
	if (m_ptAnchor.y - rcWnd.Height() - CY_LEADER < 0) m_bLeaderAtTop = true;
	...
	ptLeader[0].x = m_bLeaderAtRight ? rcWnd.Width() - CX_ROUNDED - CX_LEADER_MARGIN : CX_ROUNDED + CX_LEADER_MARGIN;
	...
	CRgn rgn;
	rgn.CreateRectRgn(0, 0, rcWnd.Width(), rcWnd.Height() + CY_LEADER);
	rgn.CombineRgn(&rgnContent, &rgnLeader, RGN_OR);

	SetWindowPos(CWnd::FromHandle(HWND_TOPMOST),
	             (m_bLeaderAtRight ? m_ptAnchor.x - rcWnd.Width() + CX_ROUNDED + CX_LEADER_MARGIN
	                               : m_ptAnchor.x - CX_ROUNDED - CX_LEADER_MARGIN),
	             (m_bLeaderAtTop ? m_ptAnchor.y : m_ptAnchor.y - rcWnd.Height() - CY_LEADER), rcWnd.Width(),
	             rcWnd.Height() + CY_LEADER, 0);
...
void CMyBalloon::OnPaint()
{
	...
	CBrush brWnd;
	brWnd.CreateSolidBrush(m_pContent->getBkColor());
	CBrush br1;
	br1.CreateSolidBrush(m_pContent->getTextColor());
...
void CMyBalloon::OnSize(UINT nType, int cx, int cy)
{
	...
	{
		m_pContent->MoveWindow(2, m_bLeaderAtTop ? CY_LEADER + 2 : 2, cx - 4, m_pContent->getRequiredSize().cy);
	}
}
...
void CMyBalloon::OnKillFocus(CWnd* pNewWnd)
{
	...
	if (false == m_bTrayBalloon) CloseBalloon(false);
}
