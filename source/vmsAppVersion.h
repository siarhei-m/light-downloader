...
struct vmsAppVersionPart
{
	...
	vmsAppVersionPart(LPCTSTR ptsz) { fromString(ptsz); }
	...
	{
		tstring tstr;
		for (int i = 0; i < cPreZeros; i++) tstr += '0';
		TCHAR tsz[100] = _T ("");
	...
	{
		return cPreZeros != o.cPreZeros && dwVal != 0 && o.dwVal != 0 ? cPreZeros > o.cPreZeros : dwVal < o.dwVal;
	}

	BOOL operator==(const vmsAppVersionPart& o) const { return dwVal == o.dwVal && cPreZeros == o.cPreZeros; }
...
class vmsAppVersion : public std::vector<vmsAppVersionPart>
{
  public:
	...
	BOOL FromString(LPCTSTR ptszVersion)
	{
		...
		while (*ptszVersion == ' ') ptszVersion++;

		if (*ptszVersion == 'v' || *ptszVersion == 'V') ptszVersion++;

		while (*ptszVersion)
		{
			while (*ptszVersion == ' ') ptszVersion++;
			tstring tstrVer;
			while (_istdigit(*ptszVersion)) tstrVer += *ptszVersion++;
			if (tstrVer.empty()) break;
			while (*ptszVersion == ' ') ptszVersion++;
			if (*ptszVersion == '.' || *ptszVersion == ',') ptszVersion++;
			push_back(vmsAppVersionPart(tstrVer.c_str()));
		}

		if (size() && *ptszVersion)
		{
			while (ptszVersion[-1] == ' ') ptszVersion--;
