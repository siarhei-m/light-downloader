  ...
  public:
	void AcquireReaderLock()
	{
		...
		if (!cLocks)
		{
			...
			if (InterlockedIncrement(&m_cTotalThreadsHavingReadLocks) == 1) ResetEvent(m_hevNoReads);
		}
		cLocks++;
	...
	void ReleaseReaderLock()
	{
		...
		if (!cLocks)
		{
			if (!InterlockedDecrement(&m_cTotalThreadsHavingReadLocks)) SetEvent(m_hevNoReads);
		}
	}
	...
	void ReleaseWriterLock()
	{
		...
		if (--m_cWritingThreadLocks) return;
...
class vmsReaderWriterLockAutoLockRead
{
  public:
	vmsReaderWriterLockAutoLockRead(vmsReaderWriterLock* p)
	{
		assert(p != NULL);
		m_prwl = p;
		if (p) p->AcquireReaderLock();
	}
	~vmsReaderWriterLockAutoLockRead()
	{
		if (m_prwl) m_prwl->ReleaseReaderLock();
	}
	void Unlock()
	{
		if (m_prwl)
...
class vmsReaderWriterLockAutoLockWrite
{
  public:
	vmsReaderWriterLockAutoLockWrite(vmsReaderWriterLock* p)
	{
		assert(p != NULL);
		m_prwl = p;
		if (p) p->AcquireWriterLock();
	}
	~vmsReaderWriterLockAutoLockWrite()
	{
		if (m_prwl) m_prwl->ReleaseWriterLock();
	}
	void Unlock()
	{
		if (m_prwl)
