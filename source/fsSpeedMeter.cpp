...
void fsSpeedMeter::UpdateSpeed()
{
	...
	if (m_now - m_past > 1000)
	{
		...
		if (m_iCurPrevSpeed > m_iMaxPrevSpeed)
		{
			...
			{
				if (m_aPrevSpeed[i] < m_dwSpeed) bAllGreater = FALSE;

				if (m_aPrevSpeed[i] > m_dwSpeed) bAllLess = FALSE;
			}

			if (bAllGreater || bAllLess) m_cMeasures = 0;
		...
		if (m_cMeasures == 0) m_dwSpeed = m_aPrevSpeed[m_iCurPrevSpeed - 1];
