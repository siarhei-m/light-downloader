...
void vmsPersistableScheduleWrapper::putListToBuffer(fs::list<UINT>* pvIds, LPBYTE& pbtCurrentPos, LPBYTE pbtBuffer,
                                                    DWORD dwBufferSize, DWORD* pdwSizeRequired)
{
	if (pvIds == 0) return;
...
bool vmsPersistableScheduleWrapper::getListFromBuffer(fs::list<UINT>* pvIds, LPBYTE& pbtCurrentPos, LPBYTE pbtBuffer,
                                                      DWORD dwBufferSize)
{
	if (pvIds == 0) return false;
...
bool vmsPersistableScheduleWrapper::loadObjectItselfFromStateBuffer(LPBYTE pbtBuffer, LPDWORD pdwSize, DWORD dwVer)
{
	...
	if (!getVarFromBuffer(schScheduleParam.schTask, pbtCurrentPos, pbtBuffer, *pdwSize)) return false;
	...
	if (slehEventHandler) slehEventHandler(&schScheduleParam.schTask, pvData);
...
void fsScheduleMgr::ManageNotEvents()
{
	...
	for (i = m_vTasks.size() - 1; i >= 0; i--)
	{
		...
		if ((task->dwFlags & SCHEDULE_ENABLED) == 0) continue;
	...
	for (i = 0; i < vTasks.size(); i++) StartTask(vTasks[i]);
}
...
BOOL fsScheduleMgr::IsNeedToStart(fsSchedule* task)
{
	...
	{
		if (CompareFileTime(&task->hts.last, &task->hts.next)) return TRUE;
	}
...
{
	if ((task->dwFlags & SCHEDULE_ENABLED) == 0) return;
	...
	if (pschScheduleParam && pschScheduleParam->m_ppoTaskWrapper) pschScheduleParam->m_ppoTaskWrapper->setDirty();
	...
	if (task->uWaitForConfirmation)
	{
		...
		strMsg.Format(LS(L_STARTIT), strTask.GetString());
		dlg.Init(strMsg, task->uWaitForConfirmation);

		if (_DlgMgr.DoModal(&dlg) != IDOK) return;
	}
	...
	case WTS_RESTRAINALLDLDS:
	{
		...
		if (task->wts.enRAD == RAD_ENABLE)
			...
			bShould = !bEn;
		if (!bEn != !bShould) _TumMgr.setRestrainAll(bShould);
	}
	break;
...
void fsScheduleMgr::RepairNextTime(fsSchedule* task)
{
	...
	if (IsTimeBased(task) == FALSE || task->hts.enType == HTS_ONCE) return;

	if (CompareFileTime(&m_curTime, &task->hts.next) != 1) return;

	if (CompareFileTime(&task->hts.next, &task->hts.last) == 0) return;

	FileTimeToSystemTime(&m_curTime, &time);
	FileTimeToSystemTime(&task->hts.next, &time2);

	if (task->hts.enType == HTS_DAILY || (task->hts.enType == HTS_CONTINUOUSLY && task->hts.interval >= 24 * 60))
	{
...
{
	if (IsTimeBased(task) == FALSE) return;
	...
	case HTS_DAILY:
	{
		...
		for (int i = time.wDayOfWeek + 1; TRUE; i++)
		{
			...
			if (i == 7) i = 0;
			if (task->hts.aWeeklyDays[i]) break;
		}
	}
	break;
...
{
	if (!IsTimeBased(task) || task->hts.enType == HTS_ONCE) return;
...
CString fsScheduleMgr::WTSToStr(fsSchedule* task)
{
	...
	case WTS_CHANGE_TUM:
	{
		...
		str = LS(L_CHANGETUMTO);
		str += ' ';
		str += pszTum[task->wts.enTUM];
	}
	break;
...
CString fsScheduleMgr::HTSToStr(fsSchedule* task)
{
	...
	case HTS_WHENSPEEDLOW:
	{
		...
		str.Format("%s %s", str1.GetString(), LS(L_MIN_SMALL));
	}
	break;
...
CString fsScheduleMgr::NextTimeToStr(fsSchedule* task)
{
	...
	case HTS_CONTINUOUSLY:
	{
		...
		FileTimeToSystemTime(&task->hts.next, &_time);
		SystemTimeToStr(&_time, strDate, strTime, FALSE);
		str += strTime;
		str += " ";
		str += strDate;
	}
...
{
	if (IsTimeUnknown(task->hts.last)) return LS(L_NEVER);
...
{
	if (m_pfnEvents) m_pfnEvents(task, ev, m_lpEvents);
}
...
BOOL fsScheduleMgr::SaveStateToFile(HANDLE hFile)
{
	...
	if (!WriteFile(hFile, &cTasks, sizeof(cTasks), &dw, NULL)) return FALSE;

	for (int i = 0; i < cTasks; i++)
	{
		fsSchedule* task = &m_vTasks[i]->schScheduleParam.schTask;

		if (!WriteFile(hFile, task, sizeof(fsSchedule), &dw, NULL)) return FALSE;

		switch (task->wts.enType)
		{
		case WTS_PROGRAM:
			if (!fsSaveStrToFile(task->wts.prog.pszName, hFile)) return FALSE;

			if (!fsSaveStrToFile(task->wts.prog.pszArgs, hFile)) return FALSE;
			break;

		case WTS_STARTDOWNLOAD:
		case WTS_STOPDOWNLOAD:
			if (!task->wts.dlds.pvIDs->save(hFile)) return FALSE;
			break;
		}
	}
...
BOOL fsScheduleMgr::LoadStateFromFile(HANDLE hFile)
{
	...
	if (!ReadFile(hFile, &cTasks, sizeof(cTasks), &dw, NULL) || dw != sizeof(cTasks)) return FALSE;
	...
	while (cTasks--)
	{
		...
		if (!ReadFile(hFile, task, sizeof(fsSchedule), &dw, NULL) || dw != sizeof(fsSchedule)) return FALSE;

		switch (task->wts.enType)
		{
		case WTS_PROGRAM:
			if (!fsReadStrFromFile(&task->wts.prog.pszName, hFile)) return FALSE;

			if (!fsReadStrFromFile(&task->wts.prog.pszArgs, hFile)) return FALSE;
			break;

		case WTS_STARTDOWNLOAD:
		case WTS_STOPDOWNLOAD:
			fsnew1(task->wts.dlds.pvIDs, fs::list<UINT>);
			if (!task->wts.dlds.pvIDs->load(hFile)) return FALSE;
			break;
		}
...
bool fsScheduleMgr::loadObjectItselfFromStateBuffer(LPBYTE pbtBuffer, LPDWORD pdwSize, DWORD dwVer)
{
	...
	int cTasks = 0;
	if (!getVarFromBuffer(cTasks, pbtCurrentPos, pbtBuffer, *pdwSize)) return false;
...
void fsScheduleMgr::DeleteTask(fsSchedule* task)
{
	...
	if (task->wts.enType == WTS_STARTDOWNLOAD) UpdateTaskDownloads(task);
}

int fsScheduleMgr::FindTask(fsSchedule* task)
{
	for (int i = m_vTasks.size() - 1; i >= 0; i--)
		if (task == &m_vTasks[i]->schScheduleParam.schTask) return i;
...
void fsScheduleMgr::StartDownloads(fsSchedule* task)
{
	...
	for (int i = task->wts.dlds.pvIDs->size() - 1; i >= 0; i--)
	{
		...
		if (dld != NULL && dld->pMgr->IsRunning() == FALSE)
		{
			...
			{
				CString str;
				str.Format("%s \"%s\"", bDone ? LS(L_RESTART) : LS(L_START), dld->pMgr->get_URL().pszString);
				Event(str);
...
void fsScheduleMgr::StopDownloads(fsSchedule* task)
{
	...
	for (int i = task->wts.dlds.pvIDs->size() - 1; i >= 0; i--)
	{
		...
		{
			CString str;
			str.Format("%s \"%s\"", LS(L_STOP), dld->pMgr->get_URL().pszString);
			Event(str);
...
void fsScheduleMgr::Shutdown(fsSchedule* task)
{
	...
	if (task->wts.shutdown.bForce || IsWorkStationLocked()) uFlags |= EWX_FORCE;

	LPCSTR szEv[] = {LS(L_SHUTDOWN), LS(L_RESTARTCOMP), LS(L_LOGOFF), LS(L_HIBERNATE), LS(L_STANDBY)};
...
{
	if (m_pfnEventDesc) m_pfnEventDesc(pszEvent, enType, m_lpEventDesc);
}
...
void fsScheduleMgr::ExciteEvent(fsExternalEvent ev)
{
	...
	{
		DWORD dw;
		CloseHandle(CreateThread(NULL, 0, _threadScheduleMgr, this, 0, &dw));
	}
	...
	for (i = 0; i < m_vTasks.size(); i++)
	{
		...
		if (htsTask == HTS_WHENDONE) htsTask = HTS_WHENNODOWNLOADS;

		if (hts == htsTask)
		{
			if (ev == EV_STARTUP && (task->wts.enType == WTS_SHUTDOWN || task->wts.enType == WTS_EXIT))
			{
...
void fsScheduleMgr::ManageHalfEvents()
{
	...
	if (_DldsMgr.IsRunning() == FALSE) return;
	...
	for (i = m_vTasks.size() - 1; i >= 0; i--)
	{
		...
		case HTS_WHENSPEEDLOW:
			if (IsTimeUnknown(task->hts.last) || fsGetTimeDelta(&m_curTime, &task->hts.last) >= uInterval * 60)
			{
...
DWORD WINAPI fsScheduleMgr::_threadShutdown(LPVOID lp)
{
	...
	if (uFlags & EWX_FORCE) ((CMainFrame*)AfxGetApp()->m_pMainWnd)->onExit(false);
		...
		if (task->wts.enType == enWTS)
		{
			if (task->wts.dlds.pvIDs->size() == cDlds)
			{
				...
				for (j = 0; j < cDlds; j++)
				{
					int k = 0;
					for (k = 0; k < cDlds; k++)
						if (vDlds[j]->nID == task->wts.dlds.pvIDs->at(k)) break;

					if (k == cDlds) break;
				}

				if (j == cDlds) return task;
			}
		}
	}
...
int fsScheduleMgr::FindTask(fsWhatToStartType enType, int iStartPos)
{
	...
	{
		if (m_vTasks[i]->schScheduleParam.schTask.wts.enType == enType) return i;
	}
...
void fsScheduleMgr::UpdateTaskDownloads(fsSchedule* task)
{
	...
	for (int j = 0; j < task->wts.dlds.pvIDs->size(); j++)
	{
		...
		if (dld) _pwndDownloads->UpdateDownload(dld);
	}
}
...
{
	for (int i = 0; i < m_vTasks.size(); i++) RepairNextTime(&m_vTasks[i]->schScheduleParam.schTask);
}
...
BOOL fsScheduleMgr::IsWorkStationLocked()
{
	...
	bLocked = !GetUserObjectInformation(hDesk, UOI_NAME, szName, 80, &cbName) || lstrcmpi(szName, _T("default")) != 0;
...
bool FdmScheduleLoadEventHandler(fsSchedule* pschTask, void* pvData)
{
	...
	if (pschmMgr == 0) return false;

	if (pschTask == 0) return false;
