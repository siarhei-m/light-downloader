...
CDlgFind::CDlgFind(CWnd* pParent) : CDialog(CDlgFind::IDD, pParent)
{
...
BOOL CDlgFind::OnInitDialog()
{
	...
	for (int i = 0; i < _LastFind.GetRecordCount(); i++) m_wndWhat.AddString(_LastFind.GetRecord(i));
...
void CDlgFind::OnOK()
{
	...
	if (bFile) dwFlags |= SEARCHIN_FILENAME;

	if (bComment) dwFlags |= SEARCHIN_COMMENT;

	if (bURL) dwFlags |= SEARCHIN_URL;

	if (bLoc) dwFlags |= SEARCHIN_LOCFILENAME;
