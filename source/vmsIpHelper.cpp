...
vmsIpHelper::vmsIpHelper(void) : m_pAdapters(NULL) {}
...
bool vmsIpHelper::GetAdaptersInfo(void)
{
	...
	if (ERROR_BUFFER_OVERFLOW != ::GetAdaptersInfo(NULL, &dwSize)) return false;
...
PIP_ADAPTER_INFO vmsIpHelper::FindAdapterByIfIndex(DWORD dwIndex)
{
	...
	while (pAdapter && pAdapter->Index != dwIndex) pAdapter = pAdapter->Next;
	return pAdapter;
}
