...
CDlgExportDownloads::CDlgExportDownloads(CWnd* pParent) : CDialog(CDlgExportDownloads::IDD, pParent)
{
...
void CDlgExportDownloads::ExportDownloads(int iWhich, BOOL bNoDone, BOOL bAppend)
{
	...
	case 0:
	{
		...
		for (size_t i = 0; i < nCount; i++)
		{
			...
			if (bNoDone && dld->pMgr->IsDone()) continue;
	...
	case 1:
	{
		...
		for (int i = 0; i < cItems; i++)
		{
			...
			try
			{
				...
				if (bNoDone && dld->pMgr->IsDone()) dld = NULL;
			}
	...
	else
	{
		...
		if (bAppend == FALSE) flags |= OFN_OVERWRITEPROMPT;
...
BOOL CDlgExportDownloads::ExportDownloads_ToURLListFile(LPCSTR pszFile, DLDS_LIST* pvpDlds, BOOL bAppend)
{
	...
	if (bAppend == FALSE || GetFileAttributes(pszFile) == DWORD(-1)) mode |= CFile::modeCreate;
	...
	if ((mode & CFile::modeCreate) == 0) file.SeekToEnd();
...
BOOL CDlgExportDownloads::ExportDownloads_ToDLInfoListFile(LPCSTR pszFile, DLDS_LIST* pvpDlds, BOOL bAppend)
{
	...
	if (bAppend)
	{
		...
		if (result) root = doc.child("FDM_Downloads_Info_List");
	}

	if (!root) root = doc.append_child("FDM_Downloads_Info_List");

	for (size_t i = 0; i < pvpDlds->size(); i++)
	{
		ExportDownload_ToXML(root, pvpDlds->at(i));
	}

	if (!doc.save_file(pszFile, "  ")) return FALSE;

	return TRUE;
}

BOOL CDlgExportDownloads::ExportDownload_ToXML(pugi::xml_node root, vmsDownloadSmartPtr dld)
{
	...
	pugi::xml_node dlNode = root.append_child("download");

	pugi::xml_node urlNode = dlNode.append_child("url");
	urlNode.append_child(pugi::node_pcdata).set_value(dld->pMgr->get_URL());

	if (dld->strComment != "")
	{
		pugi::xml_node commentNode = dlNode.append_child("comment");
		commentNode.append_child(pugi::node_pcdata).set_value(dld->strComment);
	}

	pugi::xml_node groupNode = dlNode.append_child("group");
	groupNode.append_child(pugi::node_pcdata).set_value(_strLastGrpFullName);
