...
BOOL CDownloadProperties_GeneralPage::OnInitDialog()
{
	...
	for (i = 1; i < size; i++)
	{
		if (dld0->pGroup->nId != m_pvDlds->at(i)->pGroup->nId) break;
	}
	...
	for (i = 1; i < size; i++)
	{
		if (dld0->strComment != m_pvDlds->at(i)->strComment) break;
	}

	if (i == size) SetDlgItemText(IDC_COMMENT, dld0->strComment);
...
BOOL CDownloadProperties_GeneralPage::OnApply()
{
	...
	if (size == 1)
	{
		...
		if (FALSE == CCreateDownloadDlg::_CheckFolderName(this, IDC_SAVEAS)) return FALSE;
...
void CDownloadProperties_GeneralPage::ApplyLanguage()
{
	fsDlgLngInfo lnginfo[] = {
	    fsDlgLngInfo(IDC__GENERAL, L_GENERALINFO),   fsDlgLngInfo(IDC__USER, L_USER),
	    fsDlgLngInfo(IDC__PASSWORD, L_PASSWORD),     fsDlgLngInfo(IDC_USELOGIN, L_LOGINREQ),
	    fsDlgLngInfo(IDC__SAVEDAS, L_SAVEDAS),       fsDlgLngInfo(IDC__GROUP, L_GROUP),
...
{
	if (IsDlgButtonChecked(IDC_USELOGIN) == BST_INDETERMINATE) CheckDlgButton(IDC_USELOGIN, BST_UNCHECKED);
...
void CDownloadProperties_GeneralPage::PrepareCHMgr(CPoint point)
{
	fsCtrlContextHelp aCH1[] = {
	    fsCtrlContextHelp(IDC__URL, LS(L_ADDROFFILE)),      fsCtrlContextHelp(IDC_URL, LS(L_ADDROFFILE)),
	    fsCtrlContextHelp(IDC__USER, LS(L_USERHERE)),       fsCtrlContextHelp(IDC_USER, LS(L_USERHERE)),
	    fsCtrlContextHelp(IDC__PASSWORD, LS(L_PWDHERE)),    fsCtrlContextHelp(IDC_PASSWORD, LS(L_PWDHERE)),
	    fsCtrlContextHelp(IDC_USELOGIN, LS(L_LOGINHERE)),   fsCtrlContextHelp(IDC__SAVEDAS, LS(L_SAVEDASHERE)),
	    fsCtrlContextHelp(IDC_SAVEAS, LS(L_SAVEDASHERE)),   fsCtrlContextHelp(IDC__GROUP, LS(L_GROUPHERE)),
	    fsCtrlContextHelp(IDC_GROUPS, LS(L_GROUPHERE)),     fsCtrlContextHelp(IDC__COMMENT, LS(L_DLDSHORTDESC)),
