...
BOOL CGroupDlg::OnInitDialog()
{
	...
	for (int i = 0; i < _LastFolders.GetRecordCount(); i++) pDirs->AddString(_LastFolders.GetRecord(i));

	if (m_pGroup == NULL)
	{
		m_strBaseFolder = m_pParentGroup == NULL ? _DldsGrps.GetGroupsRootOutFolder() : m_pParentGroup->strOutFolder;
	...
	else
	{
		...
		{
			if (lstrcmp(m_pGroup->strOutFolder + m_pGroup->strOutFolder.GetLength() - m_pGroup->strName.GetLength(),
			            m_pGroup->strName) == 0)
			{
				m_strBaseFolder = m_pGroup->strOutFolder;
				m_strBaseFolder.Delete(m_pGroup->strOutFolder.GetLength() - m_pGroup->strName.GetLength(),
				                       m_pGroup->strName.GetLength());
				m_bPathChanged = FALSE;
			}
		}
...
void CGroupDlg::OnOK()
{
	...
	if (FALSE == CCreateDownloadDlg::_CheckFolderName(this, IDC_GRPFOLDER)) return;
...
void CGroupDlg::ApplyLanguage()
{
	fsDlgLngInfo lnginfo[] = {
	    fsDlgLngInfo(IDC__GROUP, L_GRPNAME),        fsDlgLngInfo(IDC__PARENTGROUP, L_PARENT_GROUP, TRUE),
	    fsDlgLngInfo(IDC__FOLDER, L_GRPFOLDER),     fsDlgLngInfo(IDC__EXTS, L_GRPEXTS),
	    fsDlgLngInfo(IDC__DEFPRIOR, L_GRPDEFPRIOR), fsDlgLngInfo(IDCANCEL, L_CANCEL),
...
void CGroupDlg::PrepareCHMgr(CPoint point)
{
	fsCtrlContextHelp aCH1[] = {
	    fsCtrlContextHelp(IDC__GROUP, LS(L_ENTERGRPNAMEHERE)), fsCtrlContextHelp(IDC_GRPNAME, LS(L_ENTERGRPNAMEHERE)),
	    fsCtrlContextHelp(IDC__FOLDER, LS(L_FOLDERINWHICH)),   fsCtrlContextHelp(IDC_GRPFOLDER, LS(L_FOLDERINWHICH)),
	    fsCtrlContextHelp(IDC_CHOOSEFOLDER, LS(L_BROWSEFOR)),  fsCtrlContextHelp(IDC__EXTS, LS(L_GRPFILESEXTS)),
	    fsCtrlContextHelp(IDC_GRPEXTS, LS(L_GRPFILESEXTS)),    fsCtrlContextHelp(IDOK, LS(L_ACCEPTCHANGES)),
...
{
	LPCSTR ppsz[] = {"%server%", "%path_on_server%", "%year%",   "%month%",
	...
	if (str.IsEmpty() == FALSE && str[str.GetLength() - 1] != '\\') str += '\\';
