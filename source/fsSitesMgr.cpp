	...
	for (size_t i = 0; i < m_vSites.size(); i++)
	{
		...
		if (fsIsServersEqual(site->strName, pszSite, site->dwValidFor & SITE_VALIDFOR_SUBDOMAINS))
		{
			if (bAllReq)
			{
				...
				if ((site->dwValidFor & dwValidFor) == dwValidFor) return m_vSites[i];
			}
			else
			{

				if (site->dwValidFor & dwValidFor) return m_vSites[i];
			}
		}
	}
...
{
	if (m_pfnEvents) m_pfnEvents(ev, site, m_lpEventsParam);
}
...
BOOL fsSitesMgr::SaveToFile(HANDLE hFile)
{
	...
	if (!WriteFile(hFile, &hdr, sizeof(hdr), &dw, NULL)) return FALSE;

	if (!WriteFile(hFile, &cPermSites, sizeof(cPermSites), &dw, NULL)) return FALSE;

	BOOL bMaySavePwds = _App.SM_KeepPasswords() && _App.SM_DontSavePwdsToDisk() == FALSE;

	for (int i = 0; i < GetSiteCount(); i++)
	{
		fsSiteInfo* site = GetSite(i);

		if (site->bTemp) continue;

		if (!fsSaveStrToFile(site->strName, hFile)) return FALSE;

		if (!fsSaveStrToFile(bMaySavePwds ? site->strPassword : NULL, hFile)) return FALSE;

		if (!fsSaveStrToFile(bMaySavePwds ? site->strUser : NULL, hFile)) return FALSE;

		UINT nGrpId = site->pGroup ? site->pGroup->nId : (UINT)-1;
		if (!WriteFile(hFile, &nGrpId, sizeof(UINT), &dw, NULL)) return FALSE;

		if (!WriteFile(hFile, &site->dwFtpFlags, sizeof(site->dwFtpFlags), &dw, NULL)) return FALSE;

		if (!WriteFile(hFile, &site->cConnsNow, sizeof(site->cConnsNow), &dw, NULL)) return FALSE;

		if (!WriteFile(hFile, &site->cMaxConns, sizeof(site->cMaxConns), &dw, NULL)) return FALSE;

		if (!WriteFile(hFile, &site->dwValidFor, sizeof(site->dwValidFor), &dw, NULL)) return FALSE;
	}
...
BOOL fsSitesMgr::LoadFromFile(HANDLE hFile)
{
	...
	if (!ReadFile(hFile, &cSites, sizeof(cSites), &dw, NULL) || dw != sizeof(cSites)) return FALSE;
	...
	for (int i = 0; i < cSites; i++)
	{
		...
		else
		{
			if (hdr.wVer < 3)
			{
				...
				if (!ReadFile(hFile, &sitev2, sizeof(sitev2), &dw, NULL) || dw != sizeof(sitev2)) return FALSE;

				if (!fsReadStrFromFile(&sitev2.strGroup.pszString, hFile)) return FALSE;
		...
		if (!fsReadStrFromFile(&site->strName.pszString, hFile)) return FALSE;

		if (!fsReadStrFromFile(&site->strPassword.pszString, hFile)) return FALSE;

		if (!fsReadStrFromFile(&site->strUser.pszString, hFile)) return FALSE;

		if (hdr.wVer >= 3)
		{
			UINT nId;
			if (!ReadFile(hFile, &nId, sizeof(nId), &dw, NULL)) return FALSE;
			site->pGroup = nId != (UINT)-1 ? _DldsGrps.FindGroup(nId) : NULL;

			if (!ReadFile(hFile, &site->dwFtpFlags, sizeof(site->dwFtpFlags), &dw, NULL)) return FALSE;

			if (!ReadFile(hFile, &site->cConnsNow, sizeof(site->cConnsNow), &dw, NULL)) return FALSE;

			if (!ReadFile(hFile, &site->cMaxConns, sizeof(site->cMaxConns), &dw, NULL)) return FALSE;

			if (!ReadFile(hFile, &site->dwValidFor, sizeof(site->dwValidFor), &dw, NULL)) return FALSE;
		}
...
bool fsSitesMgr::loadObjectItselfFromStateBuffer(LPBYTE pbtBuffer, LPDWORD pdwSize, DWORD dwVer)
{
	...
	if (!getVarFromBuffer(cSites, pbtCurrentPos, pbtBuffer, *pdwSize)) return false;
	...
	for (int i = 0; i < cSites; i++)
	{
		...
		if (dwVer <= 1)
		{
			...
			if (!getVarFromBuffer(sitev1, pbtCurrentPos, pbtBuffer, *pdwSize)) return false;
		...
		else
		{
			if (dwVer < 3)
			{
				...
				if (!getVarFromBuffer(sitev2, pbtCurrentPos, pbtBuffer, *pdwSize)) return false;

				if (!getStrFromBuffer(&sitev2.strGroup.pszString, pbtCurrentPos, pbtBuffer, *pdwSize)) return false;
		...
		if (!getStrFromBuffer(&site->strName.pszString, pbtCurrentPos, pbtBuffer, *pdwSize)) return false;

		if (!getStrFromBuffer(&site->strPassword.pszString, pbtCurrentPos, pbtBuffer, *pdwSize)) return false;

		if (!getStrFromBuffer(&site->strUser.pszString, pbtCurrentPos, pbtBuffer, *pdwSize)) return false;

		if (dwVer >= 3)
		{

			UINT nId;
			if (!getVarFromBuffer(nId, pbtCurrentPos, pbtBuffer, *pdwSize)) return false;
			site->pGroup = nId != (UINT)-1 ? _DldsGrps.FindGroup(nId) : NULL;

			if (!getVarFromBuffer(site->dwFtpFlags, pbtCurrentPos, pbtBuffer, *pdwSize)) return false;

			if (!getVarFromBuffer(site->cConnsNow, pbtCurrentPos, pbtBuffer, *pdwSize)) return false;

			if (!getVarFromBuffer(site->cMaxConns, pbtCurrentPos, pbtBuffer, *pdwSize)) return false;

			if (!getVarFromBuffer(site->dwValidFor, pbtCurrentPos, pbtBuffer, *pdwSize)) return false;
		}
...
void fsSitesMgr::CheckGroups()
{
	...
	for (size_t j = 0; j < v.size(); j++) SiteUpdated(v[j]);
}
