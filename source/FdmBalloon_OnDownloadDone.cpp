...
{
	if (CFdmBalloonContent::OnCreate(lpCreateStruct) == -1) return -1;
...
void CFdmBalloon_OnDownloadDone::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (bClose) ((CFdmBalloon*)GetParent()->GetParent())->CloseBalloon(true);
}
...
int CFdmBalloon_OnDownloadDone::FindLinkIndexUnderCursor()
{
	...
	{
		if (PtInRect(&m_vLinksPos[i], pt)) return i;
	}
