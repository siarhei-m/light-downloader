...
class vmsCrashCatcher
{
  ...
  protected:
	...
	static DWORD WINAPI _threadCreateDump(LPVOID lp)
	{
		...
		HANDLE hFile = CreateFile(tszTmpFile, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, 0, NULL);
		...
		if (nSize)
		{
			pthis->m_dwpFaultModuleCrashAddress =
			    (DWORD_PTR)pthis->m_pEP->ExceptionRecord->ExceptionAddress - (DWORD_PTR)mbi.AllocationBase;
	...
	static LONG WINAPI _UnhandledExceptionFilter(_EXCEPTION_POINTERS* info)
	{
		...
		if (_b) return m_pPrevFilter ? m_pPrevFilter(info) : EXCEPTION_EXECUTE_HANDLER;
		...
		WaitForSingleObject(CreateThread(NULL, 0, _threadCreateDump, m_pThis, 0, NULL), INFINITE);
