...
bool vmsPersistableDescEventWrapper::loadObjectItselfFromStateBuffer(LPBYTE pbtBuffer, LPDWORD pdwSize, DWORD dwVer)
{
	...
	if (!getVarFromBuffer(deEvent, pbtCurrentPos, pbtBuffer, *pdwSize)) return false;

	if (!getStrFromBuffer(&deEvent.pszEvent, pbtCurrentPos, pbtBuffer, *pdwSize)) return false;
...
void fsEventsMgr::clear()
{
	...
	for (int i = m_vEvents.size() - 1; i >= 0; i--) delete[] m_vEvents[i]->deEvent.pszEvent;
	setDirty();
...
BOOL fsEventsMgr::Save(HANDLE hFile)
{
	...
	if (!WriteFile(hFile, &cRecs, sizeof(cRecs), &dw, NULL)) return FALSE;
	...
	for (int i = 0; i < cRecs; i++)
	{
		...
		if (!WriteFile(hFile, ev, sizeof(fsDescEvent), &dw, NULL)) return FALSE;

		if (!fsSaveStrToFile(ev->pszEvent, hFile)) return FALSE;
	}
...
BOOL fsEventsMgr::Load(HANDLE hFile)
{
	...
	if (!ReadFile(hFile, &cRecs, sizeof(cRecs), &dw, NULL) || dw != sizeof(cRecs)) return FALSE;
...
bool fsEventsMgr::loadObjectItselfFromStateBuffer(LPBYTE pbtBuffer, LPDWORD pdwSize, DWORD dwVer)
{
	...
	if (!getVarFromBuffer(cRecs, pbtCurrentPos, pbtBuffer, *pdwSize)) return false;
