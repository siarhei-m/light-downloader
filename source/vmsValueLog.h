...
class vmsValueLog
{
  public:
	...
	{
		if (!cValues) return ValueUnknown;
		if (bCheckCompleteness && m_vValues.size() < Completeness) return ValueUnknown;
		...
		{
			if (!m_vValues[i].bValid) continue;
	...
	void AddValue(ValueType val)
	{
		...
		{
			for (size_t i = m_vValues.size() - Validness - 1; i < m_vValues.size(); i++) CheckValueValid(i);
		}
		if (m_vValues.size() > MaxSize) m_vValues.erase(m_vValues.begin(), m_vValues.begin() + MaxSize / 3);
	}

  protected:
	void CheckValueValid(size_t nIndex)
	{
		if (m_vValues[nIndex].bValid) return;
		int cNearItems = 0;
		size_t nStart = nIndex > Validness ? nIndex - Validness : 0;
		size_t nEnd = nIndex + Validness;
		if (nEnd >= m_vValues.size()) nEnd = m_vValues.size() - 1;
		...
		for (size_t i = nStart; i <= nEnd; i++)
		{
			...
			{
				if (valItem2 / (valItem2 - valItem) >= 10) cNearItems++;
			}
		}
		if (cNearItems >= Validness) m_vValues[nIndex].bValid = true;
	}
...
template <int iExploreDuringMin, int iExploreDuringMax, int iMinIntervalBetween, int iMaxIntervalBetween,
          class ValueLog>
class vmsMaximumPossibleValueExplorerBase
{
  public:
	void CheckNeedStartExplore()
	{
		...
		if (m_tcExplore < tcCurrent) StartExplore();
	}
  ...
  public:
	...
	{
		if (!m_bExploring) StartExplore();
	}

	void ForceStopExplore()
	{
		if (m_bExploring) StopExplore(true);
	}
  ...
  public:
	vmsMaximumPossibleValueExplorerBase(ValueLog* pValues, float fIntervalIncreasePow = 1.0f)
	    : m_pValues(pValues), m_tcExplore(0), m_tcExploreStarted(0), m_cIneffectualExplorings(0), m_bExploring(false),
	      m_uMaxValueBeforeExplore(0), m_bEffectualExploring(0), m_fIntervalIncreasePow(fIntervalIncreasePow)
	{
	}
  ...
  protected:
	...
	{
		assert(!m_bExploring);
		if (m_bExploring) return false;
		m_bEffectualExploring = false;
		if (m_uMaxValueBeforeExplore == ValueLog::getValueUnknown()) return false;
		m_bExploring = true;
	...
	void StopExplore(bool bForcing = false)
	{
		...
		if (uInterval > iMaxIntervalBetween) uInterval = iMaxIntervalBetween;
...
template <int iExploreDuringMin, int iExploreDuringMax, int iMinIntervalBetween, int iMaxIntervalBetween,
          UINT64 uMinAdjustingValue, class ValueLog>
class vmsMaximumPossibleValueExplorer
    : public vmsMaximumPossibleValueExplorerBase<iExploreDuringMin, iExploreDuringMax, iMinIntervalBetween,
                                                 iMaxIntervalBetween, ValueLog>
{
	using Base = vmsMaximumPossibleValueExplorerBase<iExploreDuringMin, iExploreDuringMax, iMinIntervalBetween,
	                                                 iMaxIntervalBetween, ValueLog>;
  ...
  public:
	...
	vmsMaximumPossibleValueExplorer(ValueLog* pValues, float fIntervalIncreasePow = 1.0f)
	    : Base(pValues, fIntervalIncreasePow), m_uAdjusting(0)
	{
	}
  ...
  protected:
	virtual bool StartExplore()
	{
		assert(!this->m_bExploring);
		if (this->m_bExploring) return false;
		this->m_uMaxValueBeforeExplore = this->m_pValues->getCurrentMaximumValue();
		m_uAdjusting = max(uMinAdjustingValue, UINT64(this->m_uMaxValueBeforeExplore * 0.2));
		return Base::StartExplore();
	}
	bool CheckNeedStopExplore_NeedContinue()
	{
		UINT64 uCurrentMaxValue = this->m_pValues->getCurrentMaximumValue();
		if (uCurrentMaxValue >= (this->m_uMaxValueBeforeExplore + UINT64(m_uAdjusting * 0.3)) ||

		    (this->m_bEffectualExploring == false && uCurrentMaxValue >= UINT64(this->m_uMaxValueBeforeExplore * 1.05)))
		{
			m_uAdjusting *= 2;
			return true;
		}
...
template <class ValueType, int iExploreDuringMin, int iExploreDuringMax, int iMinIntervalBetween,
          int iMaxIntervalBetween, class ValueLog>
class vmsMaximumPossibleValueExplorer2
    : public vmsMaximumPossibleValueExplorerBase<iExploreDuringMin, iExploreDuringMax, iMinIntervalBetween,
                                                 iMaxIntervalBetween, ValueLog>
{
	using Base = vmsMaximumPossibleValueExplorerBase<iExploreDuringMin, iExploreDuringMax, iMinIntervalBetween,
  ...
  protected:
	...
	{
		UINT64 uCurrentMaxValue = this->m_pValues->getCurrentMaximumValue();
		if (uCurrentMaxValue > (UINT64)(this->m_uMaxValueBeforeExplore * 1.05))
		{

			this->m_uMaxValueBeforeExplore = uCurrentMaxValue;
			return true;
		}
  ...
  public:
	vmsMaximumPossibleValueExplorer2(ValueType* pValueToExplore, ValueLog* pValues, float fIntervalIncreasePow = 1.0f)
	    : Base(pValues, fIntervalIncreasePow), m_pValueToExplore(pValueToExplore)
	{
	}
  ...
  protected:
	virtual bool StartExplore()
	{
		assert(!this->m_bExploring);
		if (this->m_bExploring) return false;
		this->m_uMaxValueBeforeExplore = *m_pValueToExplore;
		return Base::StartExplore();
	}
