...
#define CHECK_BOUNDS(need)                                                                                             \
	if (need < 0 || need > int(*lpdwSize) - (pB - LPBYTE(lpBuffer))) return FALSE;
...
void vmsDownloadMgrEx::GetSplittedSectionsList(std::vector<vmsSectionInfo>& v)
{
	try
	{
		...
		for (size_t i = 0; i < num; i++)
		{
			...
			if (sectLast != NULL && (sectLast->uDCurrent == sect.uDStart || sectLast->uDCurrent + 1 == sect.uDStart))
			{
...
void vmsDownloadMgrEx::Do_OpenFolder()
{
	...
	{
		CString strFileName((LPCSTR)get_OutputFilePathName());
