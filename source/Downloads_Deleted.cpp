...
BOOL CDownloads_Deleted::Create(CWnd* pParent)
{
	...
	if (FALSE == CListCtrlEx::Create(LVS_REPORT | LVS_OWNERDRAWFIXED | LVS_SHOWSELALWAYS, rc, pParent, 0x5A9))
		return FALSE;
...
void CDownloads_Deleted::UpdateDownload(int iItem)
{
	...
	ssFile << CDownloads_Tasks::GetFileName(dld) << " (" << dld->pMgr->get_URL() << ")";
...
{
	if (m_sortMode != LCSM_ASCENDING) m_sortMode = LCSM_ASCENDING;
...
void CDownloads_Deleted::Sort()
{
	...
	{
		DeleteAllItems();
		for (int i = _DldsMgr.Get_DeletedDownloadCount() - 1; i >= 0; i--) AddDownload(_DldsMgr.Get_DeletedDownload(i));
	}
}
