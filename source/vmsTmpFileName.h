...
class vmsTmpFileName
{
  public:
	...
	~vmsTmpFileName()
	{
		if (!m_tstrTmpFileName2.empty()) DeleteFile(m_tstrTmpFileName2.c_str());
		if (!m_tstrTmpFileName1.empty()) DeleteFile(m_tstrTmpFileName1.c_str());
	}

	operator LPCTSTR() const
	{
		return m_tstrTmpFileName2.empty() ? m_tstrTmpFileName1.c_str() : m_tstrTmpFileName2.c_str();
	}
...
{
  public:
	vmsTmpDirName(bool bDeleteDirWithContents = true) : m_bDeleteDirWithContents(bDeleteDirWithContents)
	{
