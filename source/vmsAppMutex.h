...
class vmsAppMutex
{
  public:
	...
	void Create()
	{
		...
		SECURITY_ATTRIBUTES attr = vmsLowLabelSecurityAttributes();
		m_hmxApp = CreateMutex(&attr, TRUE, m_tstrName.c_str());

		// m_hmxApp = CreateMutex (ver.dwMajorVersion > 5 ? &vmsLowLabelSecurityAttributes () : NULL, TRUE,
		// m_tstrName.c_str ());
