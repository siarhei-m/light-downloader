...
CWaitDlg::CWaitDlg(CWnd* pParent) : CDialog(CWaitDlg::IDD, pParent)
{
...
UINT CWaitDlg::StartWaiting(LPCSTR pszMessage, LPTHREAD_START_ROUTINE procWait, BOOL bEnableCancel, LPVOID lpParam1,
                            LPVOID lpParam2, LPVOID lpParam3, LPVOID lpParam4, LPVOID lpParam5, LPVOID lpParam6,
                            BOOL bModal)
{
	m_info.hwndDlg = NULL;
...
BOOL CWaitDlg::OnInitDialog()
{
	...
	DWORD dw;
	CloseHandle(CreateThread(NULL, 0, m_pfnThread, &m_info, 0, &dw));
...
void CWaitDlg::OnTimer(UINT_PTR)
{
	if (m_info.bWaitDone)
	{
		EndDialog(m_info.bNeedStop == FALSE ? IDOK : IDCANCEL);
		if (m_bNeedRelease) delete this;
		return;
	}
