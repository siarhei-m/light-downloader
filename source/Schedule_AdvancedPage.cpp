...
BOOL CSchedule_AdvancedPage::OnInitDialog()
{
	...
	CheckDlgButton(IDC_USEWAIT, m_task->uWaitForConfirmation == UINT_MAX || m_task->uWaitForConfirmation == 0
	                                ? BST_UNCHECKED
	                                : BST_CHECKED);
...
void CSchedule_AdvancedPage::ApplyLanguage()
{
	fsDlgLngInfo lnginfo[] = {
	    fsDlgLngInfo(IDC__CONFIRMATION, L_CONFIRMATION), fsDlgLngInfo(IDC_USECONFIRMATION, L_CONFIRMREQ),
	    fsDlgLngInfo(IDC_USEWAIT, L_WAITFORCONFIRM),     fsDlgLngInfo(IDC__SECONDS, L_SECS),
	    fsDlgLngInfo(IDC_DISABLE, L_DISAFTEREXEC),       fsDlgLngInfo(IDC_DELETE, L_DELAFTEREXEC),
	};
